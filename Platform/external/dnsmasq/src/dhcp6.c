...
void dhcp6_packet(time_t now)
{
  ...
  {
    my_syslog(MS_DHCP | LOG_INFO, _("[dhcp6_packet]Fail to iface_enumerate"));
    return;
  }
  
  lease_prune(NULL, now); /* lose any expired leases */

  my_syslog(MS_DHCP | LOG_INFO, _("[dhcp6_packet]dhcp6_reply send %d[%s]"),port,ifr.ifr_name); 
  ...
  my_syslog(MS_DHCP | LOG_WARNING, _("port value is %d"),port);
