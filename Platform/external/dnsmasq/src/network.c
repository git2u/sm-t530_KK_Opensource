...
int set_servers(const char *servers)
{
  ...
  //<RNTFIX:: Uses samsung's tethering implementation_jhPark
  ...
  //>RNTFIX
    ...
    my_syslog(MS_DHCP | LOG_INFO, _("[Check Set_Server]saddr: %s"),saddr);
      ...
      if ((addr.in.sin_addr.s_addr = inet_addr(saddr)) != (in_addr_t) -1)
	{
      ...
      my_syslog(MS_DHCP | LOG_INFO, _("[Set_Server4]")); 
    }
#ifdef HAVE_IPV6
      else if (inet_pton(AF_INET6, saddr, &addr.in6.sin6_addr) > 0)
	{
      ...
      my_syslog(MS_DHCP | LOG_INFO, _("[Set_Server6]")); 
    }
#endif /* IPV6 */
    else
    {
      my_syslog(MS_DHCP | LOG_INFO, _("[Set_Server]Invaild %s"), saddr); 
      continue;
    }
  ...
  inet_ntop(AF_INET6, &new_servers->source_addr.in6.sin6_addr, dst_addr2, ADDRSTRLEN);
  inet_ntop(AF_INET, &daemon->servers->addr.in.sin_addr, dst_addr3, ADDRSTRLEN); 
  inet_ntop(AF_INET, &new_servers->source_addr.in.sin_addr, dst_addr4, ADDRSTRLEN);

  my_syslog(MS_DHCP | LOG_INFO, _("[Set_Server6]daemon->servers.addr: %s"),dst_addr1);
  my_syslog(MS_DHCP | LOG_INFO, _("[Set_Server6]servers.source_addr: %s"),dst_addr2);
  my_syslog(MS_DHCP | LOG_INFO, _("[Set_Server4]daemon->servers.addr: %s"),dst_addr3);
  my_syslog(MS_DHCP | LOG_INFO, _("[Set_Server4]servers.source_addr: %s"),dst_addr4);
