  ...
  for (opt_cfg = daemon->dhcp_opts6; opt_cfg; opt_cfg = opt_cfg->next)
  {
    ...
    if (opt_cfg->opt == OPTION6_DNS_SERVER)
    {
      struct in6_addr *a = (struct in6_addr *)opt_cfg->val;
        ...
        inet_ntop(AF_INET6, &parm.link_local, dst_addr2, ADDRSTRLEN); 
        
        my_syslog(MS_DHCP | LOG_INFO, "opt_cfg->val address: %s",dst_addr1);
        my_syslog(MS_DHCP | LOG_INFO, "parm.link_local address: %s",dst_addr2);
        ...
        for (i = 0; i < opt_cfg->len; i += IN6ADDRSZ, a++)
          if (IN6_IS_ADDR_UNSPECIFIED(a))
          {
            ...
            my_syslog(MS_DHCP | LOG_INFO, "opt6 was set with link local");
          }
          else
          {
            put_opt6(a, IN6ADDRSZ);
            my_syslog(MS_DHCP | LOG_INFO, "opt6 was set with a");
          }
      }
  ...
  if (!done_dns)
    {
      ...
      my_syslog(MS_DHCP | LOG_INFO, "opt6 was set with link_local because the value of done dns is 0");
    }
...
// for ipv6 tethering
        ...
        if (do_prefix)
        {
            ...
            { 
                ...
                inet_ntop(AF_INET6, &param->link_local, link_local_addr, ADDRSTRLEN); 
                
                my_syslog(MS_DHCP | LOG_INFO, "RTR-ADVERT(%s) %s (%s)", param->if_name, link_local_addr, daemon->addrbuff); 	
            }    
        }
    }
    return 1;
