			   ...
			   struct in6_addr *fallback, size_t sz, int is_unicast, time_t now)
{
  ...
  my_syslog(MS_DHCP | LOG_WARNING, _("dhcp6_reply through %s"), iface_name);
			     ...
			     int interface, char *iface_name, struct in6_addr *fallback, void *inbuff, size_t sz, int is_unicast, time_t now)
{
  ...
  my_syslog(MS_DHCP | LOG_WARNING, _("dhcp6_maybe_relay [msg_type is %d]"), msg_type);
			  ...
			  int interface, char *iface_name, struct in6_addr *fallback, void *inbuff, size_t sz, int is_unicast, time_t now)
{
  ...
  my_syslog(MS_DHCP | LOG_WARNING, _("dhcp6_no_relay through %s"), iface_name);
  ...
  my_syslog(MS_DHCP | LOG_INFO, _("[dhcp6_no_relay]msg_type is %d"), msg_type); 
  ...
  for (opt_cfg = daemon->dhcp_opts6; opt_cfg; opt_cfg = opt_cfg->next)
	...
	continue;
      ...
      my_syslog(MS_DHCP | LOG_INFO, "opt_cfg->opt is %d",opt_cfg->opt);
      if (opt_cfg->opt == OPTION6_DNS_SERVER)
      {
        done_dns = 1;
        my_syslog(MS_DHCP | LOG_INFO, _("done_dns was set to TRUE")); 
        if (opt_cfg->len == 0)
          continue;
      }
      ...
      if (opt_cfg->flags & DHOPT_ADDR6)
    	{
    	  int j;
        char dst_addr2[256] = {'\0',};
        char dst_addr3[256] = {'\0',};
        
    	  struct in6_addr *a = (struct in6_addr *)opt_cfg->val;

        inet_ntop(AF_INET6, fallback, dst_addr2, ADDRSTRLEN); 
        inet_ntop(AF_INET6, &context->local6, dst_addr3, ADDRSTRLEN); 

        my_syslog(MS_DHCP | LOG_INFO, _("fallback address: %s"),dst_addr2);
        my_syslog(MS_DHCP | LOG_INFO, _("context->local6: %s"),dst_addr3);

        for (j = 0; j < opt_cfg->len; j+=IN6ADDRSZ, a++)
        {
          char dst_addr1[256] = {'\0',};
          
          inet_ntop(AF_INET6, a, dst_addr1, ADDRSTRLEN); 
          my_syslog(MS_DHCP | LOG_INFO, _("opt_cfg->val address: %s(%d)"),dst_addr1, j);
          ...
          if (IN6_IS_ADDR_UNSPECIFIED(a))
          {
            if (IN6_IS_ADDR_UNSPECIFIED(&context->local6))
            {
              ...
              my_syslog(MS_DHCP | LOG_INFO, _("opt6 was set with fallback")); 
            }
            else
            {
              put_opt6(&context->local6, IN6ADDRSZ);
              my_syslog(MS_DHCP | LOG_INFO, _("opt6 was set with context->local6")); 
            }
          }
          else
          {
            put_opt6(a, IN6ADDRSZ);
            my_syslog(MS_DHCP | LOG_INFO, _("opt6 was set with a")); 
          }
        }
    	}
      else if (opt_cfg->val)
      {
        ...
        //my_syslog(MS_DHCP | LOG_INFO, _("opt_cfg->val is: %s[%d]"),opt_cfg->val[opt_cfg->len]);
        my_syslog(MS_DHCP | LOG_INFO, _("opt_cfg->val was set to opt6"));
      }
      end_opt6(o);
    }
    ...
    if(!done_dns && !IN6_IS_ADDR_UNSPECIFIED(&daemon->servers->addr.in6.sin6_addr))
    {
      ...
      inet_ntop(AF_INET6, &daemon->servers->addr.in6.sin6_addr, dst_addr1, ADDRSTRLEN); 
      inet_ntop(AF_INET6, &daemon->servers->source_addr.in6.sin6_addr, dst_addr2, ADDRSTRLEN);

      my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]daemon->servers.addr: %s"),dst_addr1);
      my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]servers.source_addr: %s"),dst_addr2);
          ...
          !IN6_IS_ADDR_UNSPECIFIED(fallback)))
    {
      ...
      char dst_addr2[256] = {'\0',};
      char dst_addr3[256] = {'\0',};
      char dst_addr4[256] = {'\0',};

      inet_ntop(AF_INET6, fallback, dst_addr1, ADDRSTRLEN); 
      inet_ntop(AF_INET6, &context->local6, dst_addr2, ADDRSTRLEN); 
      inet_ntop(AF_INET6, &daemon->servers->addr.in6.sin6_addr, dst_addr3, ADDRSTRLEN); 
      inet_ntop(AF_INET6, &daemon->servers->source_addr.in6.sin6_addr, dst_addr4, ADDRSTRLEN);
      
      my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]fallback address: %s"),dst_addr1);
      my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]context->local6: %s"),dst_addr2);
      my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]daemon->servers.addr: %s"),dst_addr3);
      my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]servers.source_addr: %s"),dst_addr4);

      o = new_opt6(OPTION6_DNS_SERVER);
      if (IN6_IS_ADDR_UNSPECIFIED(&context->local6))
      {
        put_opt6(fallback, IN6ADDRSZ);
        my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]opt6 was set with fallback")); 
      }
      else
      {
        put_opt6(&context->local6, IN6ADDRSZ);
        my_syslog(MS_DHCP | LOG_INFO, _("[done_dns is false]opt6 was set with context->local6")); 
      }
      end_opt6(o); 
    }
