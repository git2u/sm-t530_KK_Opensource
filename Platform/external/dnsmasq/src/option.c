...
#  endif
      ...
      else
	{
	  ...
	  if (!(d = canonicalise_opt(arg)))
        ...
        my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt 's' - %d[%s]"),msize, arg);
  ...
  my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt 'F' - dhcp-range"));
	...
	if (k < 2)
      ...
      my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt 's' - v4"));
...
// ] for ipv6 tethering
      ...
      for(i=0; i<k; i++)
      {
          my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt 'F' - Check v6[%s][%d]"), a[i], i);
      }

      if(!daemon->dhcp6)
        my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]daemon->dhcp6 is empty"));
    ...
    {  
      ...
      my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt leasepos[static]"));
    }
		else if (strcmp(a[leasepos], "ra-only") == 0 || strcmp(a[leasepos], "slaac") == 0 )
    {  
		  new->flags |= CONTEXT_RA_ONLY;
      my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt leasepos[ra-only][%d], a[leasepos] = %s"),leasepos,a[leasepos]);
     
    }
		else if (strcmp(a[leasepos], "ra-names") == 0)
    {  
		  new->flags |= CONTEXT_RA_NAME;
      my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt leasepos[ra-names]"));
    }
		else if (strcmp(a[leasepos], "ra-stateless") == 0)
    {  
		  new->flags |= CONTEXT_RA_STATELESS | CONTEXT_DHCP;
      my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt leasepos[ra-stateless]"));
    }
		else if (leasepos == 1 && inet_pton(AF_INET6, a[leasepos], &new->end6))
    {  
		  new->flags |= CONTEXT_DHCP; 
      my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt leasepos[CONTEXT_DHCP]"));
    }
    else  
		  break;
	      }
    ...
    my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt create daemon->dhcp6"));
	      } 
	    else
	      {
    ...
    my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt create daemon->ra_contexts"));
	      }
...
// ] for ipv6 tethering
  ...
  my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]one_opt 'F' - v6 time[%s][%d][%d]"), a[leasepos], leasepos, k);
...
void read_opts(int argc, char **argv, char *compile_opts)
{
  ...
  while (1) 
      ...
      else if (option == 'w')
	{
      ...
      {
        my_syslog(MS_DHCP | LOG_INFO, _("[DHCPv6]display_opts6"));
        display_opts6();
      }
#endif
	  else
