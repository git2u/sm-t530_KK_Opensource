...
void GLES2Implementation::RenderbufferStorageMultisampleIMG(
    GLenum target, GLsizei samples, GLenum internalformat, GLsizei width,
    GLsizei height) {
  GPU_CLIENT_SINGLE_THREAD_CHECK();
  GPU_CLIENT_LOG("[" << GetLogPrefix() << "] glRenderbufferStorageMultisampleIMG(" << GLES2Util::GetStringRenderBufferTarget(target) << ", " << samples << ", " << GLES2Util::GetStringRenderBufferFormat(internalformat) << ", " << width << ", " << height << ")");  // NOLINT
  if (samples < 0) {
    SetGLError(
  ...
  helper_->RenderbufferStorageMultisampleIMG(
      target, samples, internalformat, width, height);
  CheckGLError();
}
...
void GLES2Implementation::FramebufferTexture2DMultisampleIMG(
    GLenum target, GLenum attachment, GLenum textarget, GLuint texture,
    GLint level, GLsizei samples) {
  GPU_CLIENT_SINGLE_THREAD_CHECK();
  GPU_CLIENT_LOG("[" << GetLogPrefix() << "] glFramebufferTexture2DMultisampleIMG(" << GLES2Util::GetStringFrameBufferTarget(target) << ", " << GLES2Util::GetStringAttachment(attachment) << ", " << GLES2Util::GetStringTextureTarget(textarget) << ", " << texture << ", " << level << ", " << samples << ")");  // NOLINT
  if (samples < 0) {
    SetGLError(
        GL_INVALID_VALUE, "glFramebufferTexture2DMultisampleIMG", "samples < 0");  // NOLINT
    return;
  }
  helper_->FramebufferTexture2DMultisampleIMG(
      target, attachment, textarget, texture, level, samples);
  CheckGLError();
}
