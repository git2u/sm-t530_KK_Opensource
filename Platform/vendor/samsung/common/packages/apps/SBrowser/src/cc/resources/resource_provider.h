...
class CC_EXPORT ResourceProvider {
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
    ,AndroidNativeBuffer	// Need this to signal to createGLTexture that this is native buffer backed texture
#endif 
  };
  enum ResourceType {
    GLTexture = 1,
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  bool allowNativeBufferCreation();
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  uint8_t* MapPixelBuffer(ResourceId id, size_t* stride);
#else
 ...
 private:
  struct Resource {
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
    // Add constructor which can store graphic buffer handles
    Resource(unsigned texture_id, gfx::Size size, GLenum format, GLenum filter, android::sp<android::GraphicBuffer> android_graphic_buffer);
#endif
    unsigned gl_id;
    ...
    ResourceType type;
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
    // Add here storage for handle
    android::sp<android::GraphicBuffer> android_graphic_buffer;
#endif
  };
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  int max_grbuffer_resources_;
  int num_grbuffer_resources_;
  int max_fds_;
  struct pollfd fd_status_[1024];
#endif
