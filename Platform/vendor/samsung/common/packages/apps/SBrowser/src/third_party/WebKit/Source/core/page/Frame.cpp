...
#if defined(SBROWSER_LOCALSTORAGE_LIMIT_PER_ORIGIN)
void Frame::setLocalStorageAlloted(long alloted, bool add)
{
    if(add)
        m_localStorageAlloted  += alloted;
    else {
        m_localStorageAlloted -= alloted;
		if(m_localStorageAlloted < 0)  //Fix PLM: P130430-0096
			m_localStorageAlloted = 0;
    }
}
#endif
