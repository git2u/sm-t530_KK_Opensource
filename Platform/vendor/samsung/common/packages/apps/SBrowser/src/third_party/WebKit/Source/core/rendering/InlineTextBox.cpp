...
void InlineTextBox::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset, LayoutUnit /*lineTop*/, LayoutUnit /*lineBottom*/)
{
    ...
    if (renderer() && renderer()->node())
        shadowAncestor = renderer()->node()->deprecatedShadowAncestorNode();
    ...
    if (shadowAncestor && (shadowAncestor->renderer()->isTextField() || shadowAncestor->renderer()->isTextArea())) {
        if (selectionState() != RenderObject::SelectionBoth){
            ...
            defaultSelection  = (startObj->node()->isInShadowTree()&& endObj->node()->isInShadowTree());
        }
        ...
        if (useCustomUnderlines) {
            ...
            for (size_t index = 0; index < numUnderlines; ++index) {
                ...
                if (underline.startOffset <= end()) {
                    if( underline.endHighlightOffset!= -1&&
                        underline.startHighlightOffset != -1 &&
                        underline.startHighlightOffset < underline.endHighlightOffset
                        )
                    {
                        paintCompositionBackground(context, boxOrigin, styleToUse, font,
                            underline.startHighlightOffset,
                            underline.endHighlightOffset,
                            0xFF66CDAA);
                        break;
                    }
                } else
...
#if defined(ENABLE_JPN_COMPOSING_REGION)
// For JPN_IME<<
void InlineTextBox::paintCompositionBackground(GraphicsContext* context, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font, int startPos, int endPos, Color color)
#else
...
#if defined(SBROWSER_PLM_P130827_05910)  && defined(SBROWSER_TEXT_SELECTION)  
...
#if defined(SBROWSER_PLM_P130827_05910)&& defined(SBROWSER_TEXT_SELECTION)    
   IntRect markerRect = enclosingIntRect(font.selectionRectForText(run, IntPoint(x(), root()->selectionTop(true)), selHeight, sPos, ePos));
#else
