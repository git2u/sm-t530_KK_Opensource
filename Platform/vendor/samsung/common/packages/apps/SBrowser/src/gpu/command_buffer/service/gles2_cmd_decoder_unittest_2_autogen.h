...
TEST_F(GLES2DecoderTest2, RenderbufferStorageMultisampleIMGValidArgs) {
  EXPECT_CALL(
      *gl_, RenderbufferStorageMultisampleIMG(
          GL_RENDERBUFFER, 2, GL_RGBA4, 4, 5));
  SpecializedSetup<cmds::RenderbufferStorageMultisampleIMG, 0>(true);
  cmds::RenderbufferStorageMultisampleIMG cmd;
  cmd.Init(GL_RENDERBUFFER, 2, GL_RGBA4, 4, 5);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
}

TEST_F(GLES2DecoderTest2, RenderbufferStorageMultisampleIMGInvalidArgs0_0) {
  EXPECT_CALL(*gl_, RenderbufferStorageMultisampleIMG(_, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::RenderbufferStorageMultisampleIMG, 0>(false);
  cmds::RenderbufferStorageMultisampleIMG cmd;
  cmd.Init(GL_FRAMEBUFFER, 2, GL_RGBA4, 4, 5);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES2DecoderTest2, RenderbufferStorageMultisampleIMGInvalidArgs1_0) {
  EXPECT_CALL(*gl_, RenderbufferStorageMultisampleIMG(_, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::RenderbufferStorageMultisampleIMG, 0>(false);
  cmds::RenderbufferStorageMultisampleIMG cmd;
  cmd.Init(GL_RENDERBUFFER, -1, GL_RGBA4, 4, 5);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}

TEST_F(GLES2DecoderTest2, RenderbufferStorageMultisampleIMGInvalidArgs3_0) {
  EXPECT_CALL(*gl_, RenderbufferStorageMultisampleIMG(_, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::RenderbufferStorageMultisampleIMG, 0>(false);
  cmds::RenderbufferStorageMultisampleIMG cmd;
  cmd.Init(GL_RENDERBUFFER, 2, GL_RGBA4, -1, 5);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}

TEST_F(GLES2DecoderTest2, RenderbufferStorageMultisampleIMGInvalidArgs4_0) {
  EXPECT_CALL(*gl_, RenderbufferStorageMultisampleIMG(_, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::RenderbufferStorageMultisampleIMG, 0>(false);
  cmds::RenderbufferStorageMultisampleIMG cmd;
  cmd.Init(GL_RENDERBUFFER, 2, GL_RGBA4, 4, -1);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
// TODO(gman): FramebufferTexture2DMultisampleEXT

TEST_F(GLES2DecoderTest2, FramebufferTexture2DMultisampleIMGValidArgs) {
  EXPECT_CALL(
      *gl_, FramebufferTexture2DMultisampleIMG(
          GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5, 6));
  SpecializedSetup<cmds::FramebufferTexture2DMultisampleIMG, 0>(true);
  cmds::FramebufferTexture2DMultisampleIMG cmd;
  cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5, 6);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_NO_ERROR, GetGLError());
}

TEST_F(GLES2DecoderTest2, FramebufferTexture2DMultisampleIMGInvalidArgs0_0) {
  EXPECT_CALL(
      *gl_, FramebufferTexture2DMultisampleIMG(_, _, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::FramebufferTexture2DMultisampleIMG, 0>(false);
  cmds::FramebufferTexture2DMultisampleIMG cmd;
  cmd.Init(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5, 6);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES2DecoderTest2, FramebufferTexture2DMultisampleIMGInvalidArgs0_1) {
  EXPECT_CALL(
      *gl_, FramebufferTexture2DMultisampleIMG(_, _, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::FramebufferTexture2DMultisampleIMG, 0>(false);
  cmds::FramebufferTexture2DMultisampleIMG cmd;
  cmd.Init(GL_READ_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5, 6);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES2DecoderTest2, FramebufferTexture2DMultisampleIMGInvalidArgs2_0) {
  EXPECT_CALL(
      *gl_, FramebufferTexture2DMultisampleIMG(_, _, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::FramebufferTexture2DMultisampleIMG, 0>(false);
  cmds::FramebufferTexture2DMultisampleIMG cmd;
  cmd.Init(
      GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_PROXY_TEXTURE_CUBE_MAP, 4, 5,
      6);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_ENUM, GetGLError());
}

TEST_F(GLES2DecoderTest2, FramebufferTexture2DMultisampleIMGInvalidArgs5_0) {
  EXPECT_CALL(
      *gl_, FramebufferTexture2DMultisampleIMG(_, _, _, _, _, _)).Times(0);
  SpecializedSetup<cmds::FramebufferTexture2DMultisampleIMG, 0>(false);
  cmds::FramebufferTexture2DMultisampleIMG cmd;
  cmd.Init(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, 4, 5, -1);
  EXPECT_EQ(error::kNoError, ExecuteCmd(cmd));
  EXPECT_EQ(GL_INVALID_VALUE, GetGLError());
}
