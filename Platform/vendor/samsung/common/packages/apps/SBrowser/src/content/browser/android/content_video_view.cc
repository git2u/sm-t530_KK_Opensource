...
#ifdef SBROWSER_MULTIINSTANCE_VIDEO
  JNIEnv* env = AttachCurrentThread();
  if(Java_ContentVideoView_assignDelegate(env)) {
	  ...
	  if (j_content_video_view_.is_null()) {
        ...
        Java_ContentVideoView_openVideo(AttachCurrentThread(),
          ...
          Java_ContentVideoView_notifyDestroyContentVideoView(env);
          j_content_video_view_.Reset(Java_ContentVideoView_createContentVideoView(
          env, reinterpret_cast<jint>(this)));
  }
#else
...
void ContentVideoView::OnStart() {
  if (!j_content_video_view_.is_null())
    Java_ContentVideoView_onStart(AttachCurrentThread(),
                                            j_content_video_view_.obj());
}
#endif
