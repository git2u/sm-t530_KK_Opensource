...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
// Constructor with graphicbuffer handle
ResourceProvider::Resource::Resource(unsigned texture_id, gfx::Size size, GLenum format, GLenum filter, android::sp<android::GraphicBuffer> android_gr_buffer)
    : gl_id(texture_id),
      gl_pixel_buffer_id(0),
      gl_upload_query_id(0),
      pixels(NULL),
      ...
      filter(filter),
      type(GLTexture),
      android_graphic_buffer(android_gr_buffer)
{
}
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
bool ResourceProvider::allowNativeBufferCreation()
{
	if (num_grbuffer_resources_ < max_grbuffer_resources_) // check here if we have GrBf quota remaining
	{
		poll(fd_status_, max_fds_, 0);

		int fd_count = max_fds_;
		for(int i=0; i < max_fds_; i++)
		{
			if(fd_status_[i].revents & POLLNVAL)
				fd_count--;
		}
        // check if there is enough FD headroom
		if(fd_count < (max_fds_ - 200)) { 
			return true;
		}
	}
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  // ...except for Android native buffer -based textures
  Resource resource;
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
    if (hint == AndroidNativeBuffer &&
    	allowNativeBufferCreation()) { // check here if we have GrBf quota remaining
    	android::sp<android::GraphicBuffer> graphic_buffer = new android::GraphicBuffer((uint32_t)size.width(),
    										(uint32_t)size.height(), android::PIXEL_FORMAT_RGBA_8888,
    										android::GraphicBuffer::USAGE_SW_WRITE_OFTEN |
    										android::GraphicBuffer::USAGE_SW_READ_OFTEN |
    										android::GraphicBuffer::USAGE_HW_TEXTURE);
    	// Flatten for IPC
    	// KK changes
    	size_t flattened_bytes = graphic_buffer->getFlattenedSize();
    	size_t fd_count = graphic_buffer->getFdCount();

    	size_t fd_bytes = fd_count * 4; //fd_count * 32bit int
    	size_t flattened_size = flattened_bytes + fd_bytes + 4; // add one int for fd count indicator

    	int32 fds[fd_count];
    	uint8_t flattened_grbuffer[flattened_size];
    	int* fdArray = fds;
    	void* buffer_in = flattened_grbuffer;
    	size_t fd_count_in = fd_count;
    	size_t flattened_bytes_in = flattened_bytes;
    	graphic_buffer->flatten(buffer_in, flattened_bytes_in, fdArray, fd_count_in);

    	int32* fdstorage = (int32*)(&flattened_grbuffer[flattened_bytes]);
    	size_t i;
    	for (i = 0; i < fd_count; i++) {
    		fdstorage[i] = fds[i];
    	}
    	fdstorage[i] = fd_count; //last int tells the fd count

    	// NOTE! we now call texImage2D with the flattened grbuffer in void* pixels field, and indicate that we are using
    	// graphicbuffer allocated memory by setting border = flattened_size for informing command buffer client side.
    	// (According to spec it is always zero otherwise.)
    	GLC(context3d, context3d->texImage2D(GL_TEXTURE_2D, 0, format, size.width(), size.height(), flattened_size, format, GL_UNSIGNED_BYTE, (void*)flattened_grbuffer));

		// Texture allocation in v27 seems to be deferred. In case of Android
		// native buffers we allocate immediately and set allocated flag to true,
		// also set pending_set_pixels to false
		resource = Resource(texture_id, size, format, GL_LINEAR, graphic_buffer); // JKK need to store buffer in resource object for later access
		num_grbuffer_resources_++;
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  if (resource->android_graphic_buffer != NULL) {
	  num_grbuffer_resources_--;
  }
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
	  ,max_grbuffer_resources_(0),
      num_grbuffer_resources_(0) {
#else
	{
#endif
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
	if (CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableNativeBufferTextures))
	{
		// reset the pollfd array for FD count polling
		memset(fd_status_, 0, sizeof(struct pollfd) * 1024);
		for(int i=0; i < 1024; i++)
		{
			fd_status_[i].fd = i;
		}

		// get the number of max FDs for process
		struct rlimit limit;
		getrlimit(RLIMIT_NOFILE, &limit);
		max_fds_ = limit.rlim_cur < 1024 ? limit.rlim_cur : 1024;

		// get grbuffer limit from command line (if set)
		std::string temp = CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kEnableNativeBufferTextures);
		max_grbuffer_resources_ = atoi(temp.c_str());

		// if limit is not explicitly set:
		if (max_grbuffer_resources_ == 0)
		{
			// check how many file descriptors does one grbuffer use on current hardware
			android::sp<android::GraphicBuffer> graphic_buffer = new android::GraphicBuffer(64, 64,
					android::PIXEL_FORMAT_RGBA_8888, android::GraphicBuffer::USAGE_SW_WRITE_OFTEN |
					android::GraphicBuffer::USAGE_SW_READ_OFTEN |	android::GraphicBuffer::USAGE_HW_TEXTURE);
			...
			if (valid_fd_count > 0)
			{
				...
				if (max_fd_count < 0)
					max_fd_count = 0;
				max_grbuffer_resources_ = max_fd_count / valid_fd_count;
			}
			else
			{
				max_grbuffer_resources_ = INT_MAX; // no limitations if grbuffer takes zero fd and
												   // no explicit limit set (mali case)
			}
		}
	}
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
uint8_t* ResourceProvider::MapPixelBuffer(ResourceId id, size_t* stride) 
#else
...
#endif
{
  ...
  if (resource->gl_id) {
	...
	{
		uint8_t* tmp;
        resource->android_graphic_buffer->lock(android::GraphicBuffer::USAGE_SW_WRITE_OFTEN |
    					android::GraphicBuffer::USAGE_SW_READ_OFTEN |
    					android::GraphicBuffer::USAGE_HW_TEXTURE, (void**)&tmp);
        *stride = (size_t)resource->android_graphic_buffer->getStride() * 4; //stride in pixels * 32bit
		return tmp;
	}
	else
	{
...
void ResourceProvider::UnmapPixelBuffer(ResourceId id) {
  ...
  if (resource->gl_id) {
	...
	if (resource->android_graphic_buffer != NULL)
	{
        resource->android_graphic_buffer->unlock();
	}
	else
	{
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  if (resource->gl_id  && resource->android_graphic_buffer == NULL) {
#else
  if (resource->gl_id) {
#endif
    WebGraphicsContext3D* context3d = output_surface_->context3d();
