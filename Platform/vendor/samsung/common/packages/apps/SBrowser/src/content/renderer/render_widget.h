...
struct ViewMsg_HideUrlBarCmd_Params;
#endif
...
#if defined(SBROWSER_HIDE_URLBAR)
  void HideUrlBarEventNotify(int event, int event_data1, int event_data2);
#endif
...
void FlingAnimationCompleted(bool page_end);
  ...
  void SetDeferCommitsOnGesture(bool deferCommits);
#endif
 protected:
  ...
  void OnGetBitmapForCopyImage(int x,int y);
  #endif
  void OnCursorVisibilityChange(bool is_visible);
  void OnMouseCaptureLost();
  virtual void OnSetFocus(bool enable);
#if defined(SBROWSER_TEXT_SELECTION)	
  virtual void onUnmarkSelection();
#endif

#if defined(SBROWSER_SBR_NATIVE_CONTENTVIEW)  && defined(SBROWSER_ENTERKEY_LONGPRESS)
  virtual void OnLongPressOnFocussed(const WebKit::WebInputEvent* longpress_event);
#endif
...
#if defined(SBROWSER_HIDE_URLBAR)
  void OnHideUrlBarCmd(ViewMsg_HideUrlBarCmd_Params params);
#endif

#if defined(SBROWSER_FLUSH_PENDING_MESSAGES)
  void OnFlushPendingMessages();
#endif
  ...
  void SuspendDiferredInputJS();
  void ResumeDiferredInputJS();
  void OnDeferredInputJS(bool isJsDeferred);
#endif
  ...
  void resetInputMethodOnTap(const WebKit::WebInputEvent &input_event);
#endif
...
#if defined(SBROWSER_KEYLAG)
  bool change_listener_registered_; 
  #endif
...
#if defined(SBROWSER_KEYLAG)
    base::OneShotTimer<RenderWidget> deferred_input_js_timer_;
#endif
  ...
  bool block_updating_input_state_;
#endif
