...
void BookmarkNode::Initialize(int64 id) {
  ...
  editable_ = 1; //1-> Bookmark is editable, 0-> Bookmark is not editable
  operator_bookmark_ = 0;
#endif
  meta_info_str_.clear();
}
...
#if defined(SBROWSER_OPERATOR_BOOKMARKS)
void BookmarkModel::UpdateBookmarkNode(const BookmarkNode* node) {
...
int BookmarkModel::GetBookmarkNodeCountOfType(const BookmarkNode * node, BookmarkNode::Type type)
{
	int url_nodes_count = 0;
	if(!node)
		return url_nodes_count;
	...
	for(int i = 0; i < node->child_count(); i++)
	{
		...
		if(child_node)
		{
			...
			{
				url_nodes_count += GetBookmarkNodeCountOfType(child_node,  type);
			}
		}
	}
	return url_nodes_count;
