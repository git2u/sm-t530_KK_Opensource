...
void StartClientCertificateRequest(
  ...
  if (!chrome::android::
#if defined(SBROWSER_CAC)
      SbrSSLClientCertificateRequest_selectClientCertificate(
#else
...
#if defined(SBROWSER_CAC)
void OnSystemRequestCompletionExt(
    JNIEnv* env,
    jclass clazz,
    jint request_id,
    jobjectArray encoded_chain_ref,
    jobject private_key_ref) {
  OnSystemRequestCompletion(env, clazz, request_id, encoded_chain_ref, private_key_ref);
}
#else
