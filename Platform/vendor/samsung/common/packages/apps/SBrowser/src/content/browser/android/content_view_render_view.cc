...
void ContentViewRenderView::SetCurrentContentView(
  ...
  #if defined(SBROWSER_GRAPHICS_UI_COMPOSITOR_REMOVAL) && defined(SBROWSER_GRAPHICS_UI_COMPOSITOR_REMOVAL_MULTINSTANT)
    
    if(content_view)
    {
    	//LOG(INFO)<<"Sandeep Setting root Layer to Compositor "<<compositor_.get();
    	SbrContentViewCore* sbr_core = content_view->GetSbrContentViewCore();
	if(sbr_core)
	{
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
        MSC_METHOD("UI","CC","SetCurrentContentView",compositor_.get());
#endif
		sbr_core->SetWindow((gfx::AcceleratedWidget)(compositor_.get()));
		sbr_core->OnCompositingSurfaceUpdated();
	}
	else
		LOG(ERROR)<<"Single_compositor:No Content View";
    }
    else
        LOG(ERROR)<<"Single_compositor:No Content View";
  #endif
