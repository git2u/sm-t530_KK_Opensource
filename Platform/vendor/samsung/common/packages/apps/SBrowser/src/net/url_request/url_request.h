...
#undef HTTP_ATOM
...
#if defined(SBROWSER_SESSION_CACHE)
	// This flag will be set true only on back forward navigation.
	// For fresh load/reload it should be false.
	void	set_back_forward_navigation(bool is_back_forward){ 
		is_back_forward_navigation_ = is_back_forward;
	}
	bool is_back_forward_navigation() const{
		return is_back_forward_navigation_;
	}
#endif
 ...
 private:
  ...
  // This flag will be set true only on back forward navigation.
  // For fresh load/reload it should be false.
  bool	is_back_forward_navigation_;
#endif
