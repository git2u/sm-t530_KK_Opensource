...
int GpuChannel::GetNativeBitmap(BitmapParams imageParams, int32 surface_id, void** buffer)
{
    int ret = -1;
  DCHECK(surface_id);
  for (StubMap::iterator iter(&stubs_); !iter.IsAtEnd(); iter.Advance()) {
    if (iter.GetCurrentValue()->surface_id() == surface_id) {
     VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"FoundStub for surface: "<<surface_id;
     ret = iter.GetCurrentValue()->GetNativeBitmap(imageParams, buffer);
    }
  }
  if (ret == -1)
      VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"can't find stub for surface_id: "<< surface_id;
