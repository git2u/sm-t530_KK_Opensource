...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_METHOD_NORMAL("UI","CC",__FUNCTION__,client);
#endif
...
void CompositorImpl::Composite() {
	...
	{
		OnViewContextSwapBuffersPosted(); //Notification required by Chrome on Android
...
void CompositorImpl::SetWindowSurface(ANativeWindow* window) {
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_METHOD("UI","CC",__FUNCTION__,window);
#endif
  ...
  if(single_compositor)
  {
  	  ...
  	  SurfaceViewManager::GetInstance()->SetNativeWindow((gfx::AcceleratedWidget)this,window);
#else
	  SurfaceViewManager::GetInstance()->SetNativeWindow((gfx::AcceleratedWidget)NULL,window);
#endif
  }
  #endif
}
