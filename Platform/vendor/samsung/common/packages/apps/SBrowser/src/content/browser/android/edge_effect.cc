...
void EdgeEffect::Pull(base::TimeTicks current_time, float delta_distance) {
  ...
  edge_scale_y_ = edge_scale_y_start_
      = Clamp(distance * kPullDistanceEdgeFactor, kHeldEdgeScaleY, 0.5f);
...
void EdgeEffect::ApplyToLayers(gfx::SizeF size, Edge edge) {
  ...
  glow_->CalculateContentsScale(1.f, false,
  ...
  // SAMSUNG CHANGE : Remove unneccessary drawing update >>
  if(glow_bottom<3 ||glow_alpha_<0.1f)
    return;
  // SAMSUNG CHANGE : Remove unneccessary drawing update <<
  ...
  edge_->CalculateContentsScale(1.f, false,
                                &dummy_scale_x, &dummy_scale_y,
                                &edge_image_bounds);
