...
// changing the scrollbar opaque from translusent to avoid patent issue
  settings.solid_color_scrollbar_color = SkColorSetARGB(255, 128, 128, 128);
  ...
  settings.enable_hide_urlbar = false;
  if(cmd->HasSwitch(cc::switches::kEnableHideUrlBar)) {
    std::string hide_urlbar_str(cmd->GetSwitchValueASCII(cc::switches::kEnableHideUrlBar));
    settings.enable_hide_urlbar = (hide_urlbar_str == "1");
  }
  ...
  if(cmd->HasSwitch(switches::kGpuDriverVendor))
  {
  	std::string vendor(cmd->GetSwitchValueASCII(switches::kGpuDriverVendor)) ;

	settings.is_imagination_ = vendor.find("imagination") != std::string::npos;	
  }
#endif /* SBROWSER_GL_OUT_OF_MEMORY_PATCH */
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
  settings.enable_edge_gloweffect = false;
  if(cmd->HasSwitch(cc::switches::kEnableEdgeGlowEffect)) {
    std::string edge_gloweffect_str(cmd->GetSwitchValueASCII(cc::switches::kEnableEdgeGlowEffect));
    settings.enable_edge_gloweffect = (edge_gloweffect_str == "1");
  }
...
void RenderWidgetCompositor::UpdateCompositingLayersIfNeeded() {
  widget_->webwidget()->updateCompositingLayersIfNeeded();
}
#endif
...
void RenderWidgetCompositor::setGestureState(bool gestureActive) { 
      layer_tree_host_->setGestureState(gestureActive);
}
#endif
...
void RenderWidgetCompositor::SetDeferCommitsOnGesture(bool deferCommits) {
  widget_->SetDeferCommitsOnGesture(deferCommits);
}
#endif 

#if defined(SBROWSER_HIDE_URLBAR)
void RenderWidgetCompositor::SetUrlBarBitmap(SkBitmap* bitmap) {
  layer_tree_host_->SetUrlBarBitmap(bitmap);
}

void RenderWidgetCompositor::SetUrlBarActive(bool active, bool override_allowed) {
  layer_tree_host_->SetUrlBarActive(active, override_allowed);
}

void RenderWidgetCompositor::HideUrlBarEventNotify(int event, int event_data1, int event_data2) {
  widget_->HideUrlBarEventNotify(event, event_data1, event_data2);
}
#endif   //SBROWSER_HIDE_URLBAR

#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE)
void RenderWidgetCompositor::FlingAnimationCompleted(bool page_end){
widget_->FlingAnimationCompleted(page_end);
}
#endif

#if defined(SBROWSER_ADJUST_WORKER_THREAD_PRIORITY)
void RenderWidgetCompositor::setHighPriorityToRasterWorker(bool flag) {   
   if(layer_tree_host_) layer_tree_host_->SetHighPriorityToRasterWorker(flag);
}
#endif
}  // namespace content
