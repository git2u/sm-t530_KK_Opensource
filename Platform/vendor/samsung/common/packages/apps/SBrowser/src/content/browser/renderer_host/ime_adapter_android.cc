...
void ImeAdapterAndroid::SetComposingTextWithSubComposingRegion(JNIEnv* env, jobject, jstring text,
                                         int new_cursor_pos,int newRegionStart, int newRegsionEnd ) {
  ...
  underlines.push_back(
      WebKit::WebCompositionUnderline(0, text16.length(), SK_ColorBLACK,
                                      false,newRegionStart,newRegsionEnd));
...
#endif
void ImeAdapterAndroid::SetComposingText(JNIEnv* env, jobject j , jstring text,
                                         int new_cursor_pos) {
#if defined(ENABLE_JPN_COMPOSING_REGION)
SetComposingTextWithSubComposingRegion( env, j , text  , new_cursor_pos, -1, -1 ) ;
#else
  RenderWidgetHostImpl* rwhi = RenderWidgetHostImpl::From(
      rwhva_->GetRenderWidgetHost());
  if (!rwhi)
...
void ImeAdapterAndroid::SetComposingRegion(JNIEnv*, jobject,
  ...
  // For JPN_IME<<
  underlines.push_back(
      WebKit::WebCompositionUnderline(0, end - start, SK_ColorBLACK, false, -1, -1 ));
  // For JPN_IME>>
#else
  underlines.push_back(
...
void ImeAdapterAndroid::DirectPaste(JNIEnv* env, jobject, jstring text) {
  RenderWidgetHost* rwhi = RenderWidgetHostImpl::From(
      rwhva_->GetRenderWidgetHost());
  if (!rwhi)
    return;
...
void ImeAdapterAndroid::InformJSDeferrState(JNIEnv* env , jobject obj, bool JSDeferrState ){
#if defined(SBROWSER_KEYLAG)
	rwhva_->InformJSDeferrState(JSDeferrState);
#endif
}
}  // namespace content
