...
public:
    ...
    static bool inline IsArchitectureCompatible() {
        ...
        if (-1 == compat) {
            ...
            for (int i = 0; COMPAT_LIB_NAMES[i]; i++) {
                if (::dlopen(COMPAT_LIB_NAMES[i], RTLD_LAZY)) {
                    compat = 1;
                    break;
                }
            }
...
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    Node* nextElement() const { return m_nextElement; }
#endif  
    ...
    bool isEqualNode(const Node*) const;
#else
...
#if COMPILER_SUPPORTS(BUILTIN_PREFETCH) && defined(SBROWSER_QCOM_DOM_PREFETCH)
    ALWAYS_INLINE Node* traversePrevious() {
        if (previousSibling()) {
        ...
        // ContainerNode is a subclass of Node, so we shouldn't have to cast,
        // but it's a forward declaration.
        return (Node *)parentNode();
    }

    ALWAYS_INLINE Node* previousPostOrder(const Node* current) {
        if (current->lastChild())
            return current->lastChild();
        if (current->previousSibling())
            return current->previousSibling();

        for (current = (Node *)current->parentNode(); current; current = (Node *)current->parentNode()) {
            if (current->previousSibling())
                return current->previousSibling();
        }
        return 0;
    ...
    ALWAYS_INLINE void updatePrefetchTarget() {
        if (IsArchitectureCompatible() && m_next) {
            int skew;
            Node* from = this;
            Node* n = previousPostOrder(from);
            for (skew = 5; skew && n; skew--) {
                from = n;
                n = previousPostOrder(from);
            }
...
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    void setNextElement(Node* next) { m_nextElement = next; }
#endif
   ...
   #if defined(SBROWSER_NAVER_POPUPZOOMER_FIX)
   void setRequestFromPopupZoomer(bool requestFromPopupZoomer){ isRequestFromPopupZoomer = requestFromPopupZoomer;}
   #endif
