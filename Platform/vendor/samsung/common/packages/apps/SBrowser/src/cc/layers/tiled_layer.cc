...
void TiledLayer::SetNeedsDisplayRect(const gfx::RectF& dirty_rect) {
    ...
    if(m_gestureActive)
    {
        m_pausedAnimationRect.Union(dirty_rect); 
        return;
    }
    if(!m_pausedAnimationRect.IsEmpty())
    {
        InvalidateContentRect(LayerRectToContentRect(m_pausedAnimationRect));
        ContentsScalingLayer::SetNeedsDisplayRect(m_pausedAnimationRect);
        m_pausedAnimationRect = gfx::RectF();
    }
#endif	
...
#if defined( SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
void TiledLayer::setGestureState(bool gestureActive)
{ 
    m_gestureActive = gestureActive;
    if(!m_gestureActive){
        SetNeedsDisplayRect(gfx::RectF());
    }    
}
#endif
