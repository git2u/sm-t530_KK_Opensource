...
SearchProvider::NavigationResult::NavigationResult(
    ...
    bool from_keyword_provider,
    int relevance, bool from_search_results)
    : Result(from_keyword_provider, relevance),
      url_(url),
      formatted_url_(AutocompleteInput::FormattedStringWithEquivalentMeaning(
          url, provider.StringForURLDisplay(url, true, false))),
      description_(description), from_search_results_(from_search_results) {
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  search_term_results_.clear();
  search_term_results_from_db_.clear();
  LOG(INFO)<<"SmartSuggest: Getting search results for: "<<input_.text();
  url_db->GetSearchTermsResults(input_.text(), &search_term_results_from_db_);
  LOG(INFO)<<"SmartSuggest: #results found: "<<search_term_results_from_db_.size();

  for(unsigned i = 0; i < search_term_results_from_db_.size(); i++) {
	  LOG(INFO)<<"SmartSuggest: search_term_results: "
			  <<search_term_results_from_db_.at(i).url_.spec()
			  <<"description: "<< search_term_results_from_db_.at(i).description_
			  << "relevance:  "<<search_term_results_from_db_.at(i).relevance_;


	  search_term_results_.push_back(NavigationResult(*this,
			  search_term_results_from_db_.at(i).url_,
			  search_term_results_from_db_.at(i).description_,
			  false,
			  search_term_results_from_db_.at(i).relevance_, true) );
  }
#endif
}
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  AddNavigationResultsToMatches(search_term_results_, false);
#endif
...
AutocompleteMatch SearchProvider::NavigationToMatch(
  ...
  AutocompleteMatch match(this, navigation.relevance(), false,
                            navigation.get_from_search_result() ?
                            AutocompleteMatch::SEARCH_RESULT_NAVSUGGEST :
