...
#if 0 //defined(SBROWSER_COMMIT_BLOCK_ONTOUCH)
        if(m_needsCommitBlock)
///            return ACTION_NONE;
#endif
      if (output_surface_state_ != OUTPUT_SURFACE_ACTIVE &&
...
#if defined(SBROWSER_COMMIT_BLOCK_ONTOUCH)
void SchedulerStateMachine::setNeedsCommitBlock(bool blockState)
{
    m_needsCommitBlock = blockState;
}
#endif
}  // namespace cc
