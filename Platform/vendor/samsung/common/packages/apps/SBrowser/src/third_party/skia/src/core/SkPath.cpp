...
void SkPath::arcTo(SkScalar x1, SkScalar y1, SkScalar x2, SkScalar y2,
                   SkScalar radiusX, SkScalar radiusY) {
    SkVector    before, after;
    ...
    SkScalar distx = SkScalarMulDiv(radiusX, SK_Scalar1 - cosh, sinh);
    if (distx < 0) {
        distx = -distx;
    }

	SkScalar disty = SkScalarMulDiv(radiusY, SK_Scalar1 - cosh, sinh);
    if (disty < 0) {
        disty = -disty;
    }

    SkScalar xx = x1 - SkScalarMul(distx, before.fX);
    SkScalar yy = y1 - SkScalarMul(disty, before.fY);
    SkRotationDirection arcDir;
    ...
    matrix.setScale(radiusX, radiusY);
    matrix.postTranslate(xx - SkScalarMul(radiusX, before.fX),
                         yy - SkScalarMul(radiusY, before.fY));
...
#ifdef SBROWSER_SELECTIVE_AA_PATCH_V2
    int32_t packed = ((fIsCurves & 1) << kIsCurves_SerializationShift) |
...
#ifdef SBROWSER_SELECTIVE_AA_PATCH_V2
    fIsCurves = (packed >> kIsCurves_SerializationShift) & 1;
#endif
