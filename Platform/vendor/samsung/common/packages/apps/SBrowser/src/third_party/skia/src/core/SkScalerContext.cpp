...
#if defined(SBROWSER_EMOJI_FONT_SUPPORT)
    if((this->getGlyphContext(*glyph)->isColoredBitmap() == false) && fGenerateImageFromPath) {
#else
...
#if defined(SBROWSER_EMOJI_FONT_SUPPORT)
    if((this->getGlyphContext(*glyph)->isColoredBitmap() == false) && fMaskFilter) {
#else
    if (fMaskFilter) {
#endif
        SkMask      src, dst;
...
#if defined(SBROWSER_EMOJI_FONT_SUPPORT)
   virtual bool isColoredBitmap() SK_OVERRIDE {return false;}
#endif
};
