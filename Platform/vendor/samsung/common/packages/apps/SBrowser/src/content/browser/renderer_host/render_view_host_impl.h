...
void OnReceiveBitmapForDragging(const SkBitmap& bitmap,int width, int height);
#endif
#if defined (SBROWSER_SAVEPAGE_IMPL)
  void ScrapPageSavedAs(const base::FilePath::StringType& pure_file_name) ;
#endif
#if defined (SBROWSER_SBR_NATIVE_CONTENTVIEW)
	void LoadDataWithBaseUrl(const std::string& data,
 ...
 protected:
  ...
  void OnShowPopup(const ViewHostMsg_ShowPopup_Params& params);
   ...
   void OnSoftBitmap(const SkBitmap& bitmap);
   void ReCaptureSoftBitmap(int x, int y, int height, float pageScaleFactor, bool fromPageLoadFinish);
   void OnUpdateReCaptureSoftBitmap(const SkBitmap& bitmap, bool fromPageLoadFinish);
#endif

 #if defined (SBROWSER_PRINT_IMPL)
  void OnPrintBegin(int totalPageCount );
  void OnBitmapForPrint(const SkBitmap&  bitmap, int responseCode);
#endif
#if defined(SBROWSER_TEXT_SELECTION)  
  void OnSelectedBitmap(const SkBitmap& Bitmap);
  void OnPointOnRegion(bool isOnRegion);
  void OnReceiveSelectionVisibility(bool isVisible);
void  OnUpdateSelectionRect(const gfx::Rect& selectionRect);
#endif
#if defined(SBROWSER_FP_SUPPORT)
  void OnNotifyAutofillBegin();
#endif
#if defined (SBROWSER_SCROLL_INPUTTEXT)
void ScrollEditWithCursor(int scrollX);
void OnTextFieldBoundsChanged(const gfx::Rect&  input_edit_rect, bool  isScrollEnable);
#endif

#if defined (SBROWSER_MULTIINSTANCE_DRAG_DROP)
void HandleSelectionDrop(int x, int y, string16& text);
#endif

#if defined (SBROWSER_HANDLE_MOUSECLICK_CTRL)
void HandleMouseClickWithCtrlkey(int x, int y);
void OnOpenUrlInNewTab(const string16& mouseClickUrl);
  ...
  void OnSelectedMarkup(const string16& markup);
#endif
// SAMSUNG : SmartClip >>
...
void moveToPrevInput();
void moveToNextInput();
#endif 
#if defined (SBROWSER_SAVEPAGE_IMPL)
  void OnBitmapForScrap(const SkBitmap&  bitmap, bool response);
  ...
  void OnReceiveBitmapFromCache(const SkBitmap& bitmap);
   ...
   MessageFlushObserver* msgFlushObserver_;
   void FlushPendingMessages(MessageFlushObserver* observer);
   void OnDidFlushPendingMessages();
#endif
 ...
 private:
  ...
  bool getFPAuthStatusFlag();
  void setFPAuthStatusFlag(bool fpAuthStatusFlag);
#endif
...
#if defined(SBROWSER_FPAUTH_IMPL)
  //flag to check whether FPAuthStatus was called.
  bool fpAuthStatusFlag_;
#endif
