...
//throw away struct for wrapping data from db.
public:
struct NavigationResultForUrlDb {

	NavigationResultForUrlDb(const GURL& url,
	                 const string16& description,
	                 int relevance): url_(url),
...
private:
//NavigationResult is private by default
#endif
  class NavigationResult : public Result {
   public:
    ...
    NavigationResult(const AutocompleteProvider& provider,
                         ...
                         const string16& description,
                         bool from_keyword_provider,
                         int relevance, bool from_search_results = false);
#else
    NavigationResult(const AutocompleteProvider& provider,
                        const GURL& url,
                        const string16& description,
    ...
    virtual int CalculateRelevance(
        ...
        bool keyword_provider_requested) const OVERRIDE;
#if SBROWSER_OMNIBOX_SMART_SUGGEST
    void set_from_search_result(bool from_search_results){
    	from_search_results_ = from_search_results;
    }
    bool get_from_search_result() const { return from_search_results_; }
#endif
   private:
    ...
    string16 description_;
#if SBROWSER_OMNIBOX_SMART_SUGGEST
    bool from_search_results_; // to identify if navigation suggest is coming from search result //smart suggest
#endif
  };
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  typedef std::vector<history::KeywordSearchTermRow> HistorySearchResults;
  typedef std::vector<NavigationResultForUrlDb> NavigationResultsForUrlDb;
#endif
 ...
 #if SBROWSER_OMNIBOX_SMART_SUGGEST
  void AddSearchNavigationResultsToMap(const NavigationResult& results,
		  	  	  	  	  	  	  MatchMap* map);
#endif
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  NavigationResults search_term_results_;
  NavigationResultsForUrlDb search_term_results_from_db_;
#endif
