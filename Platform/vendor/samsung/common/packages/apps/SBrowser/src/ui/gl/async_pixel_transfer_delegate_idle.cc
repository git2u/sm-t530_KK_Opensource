...
// ref-counted ashmmem by closing renderers.
   ...
   if (!shm_mem.get()) {
        ...
        LOG(ERROR)<<"[AsyncPixelTransferDelegateIdle][AsyncNotifyCompletion] Failed mmap()";
        return;
    }
...
// ref-counted ashmmem by closing renderers.
   ...
   if (!shm_mem.get()) {
        ...
        // TODO: post message to renderer that no more shared memory will be alocated.
        LOG(ERROR)<<"[AsyncPixelTransferDelegateIdle][AsyncTexImage2D] Failed mmap()";
        return;
    }
...
// ref-counted ashmmem by closing renderers.
   ...
   if (!shm_mem.get()) {
        ...
        // TODO: post message to renderer that no more shared memory will be alocated.
        LOG(ERROR)<<"[AsyncPixelTransferDelegateIdle][AsyncTexSubImage2D] Failed mmap()";
        return;
    }
