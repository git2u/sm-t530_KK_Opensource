...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)	
  if(layer_tree_host_impl_->deactiveGlowOnTouchStart() && layer_tree_host_->settings().enable_edge_gloweffect && layer_tree_host_->IsGlowAnimationActive()) {
       layer_tree_host_->ReleaseEdgeGlow(ThreadProxy::AXIS_XY);
	   SetGlowStatus(false);
	   layer_tree_host_impl_->setDeactiveGlowOnTouchStart(false);
  }
#endif  
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
void ThreadProxy::OnOverscrollOnImplThread(gfx::Vector2dF accumulated_root_overscroll_, 
                                  gfx::Vector2dF current_fling_velocity_, bool hScroll, bool vScroll, gfx::SizeF size, float WebLayerY) {
  DCHECK(IsImplThread());
  TRACE_EVENT0("cc", "ThreadProxy::OnOverscrollOnImplThread");
  Proxy::MainThread()->PostTask(
      base::Bind(&ThreadProxy::DidOverscroll, main_thread_weak_ptr_, 
                accumulated_root_overscroll_, current_fling_velocity_, hScroll, vScroll, size, WebLayerY));
}

void ThreadProxy::DropEdgeGlowEffect(int axis) {
  DCHECK(IsImplThread());
  TRACE_EVENT0("cc", "ThreadProxy::DropEdgeGlowEffect");
  Proxy::MainThread()->PostTask(
      base::Bind(&ThreadProxy::ReleaseEdgeGlow, main_thread_weak_ptr_, axis));
}
#endif
#if defined(SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
void ThreadProxy::setGestureStateOnImplThread(bool gestureActive)
{
   DCHECK(IsImplThread());
   Proxy::MainThread()->PostTask(base::Bind(&ThreadProxy::setGestureState, base::Unretained(this), gestureActive));
//      m_mainThreadProxy->postTask(FROM_HERE, base::Bind(&ThreadProxy::setGestureState, base::Unretained(this), gestureActive));
}
#endif
...
void ThreadProxy::BeginFrame(
  ...
  layer_tree_host_->UpdateCompositingLayersIfNeeded();
#endif
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)	
  if (layer_tree_host_->settings().impl_side_painting &&
      layer_tree_host_->BlocksPendingCommit() && !layer_tree_host_->IsGlowAnimationActive()) {
#else	  
if (layer_tree_host_->settings().impl_side_painting && layer_tree_host_->BlocksPendingCommit()) {
#endif	  
    ...
    commit_completion_event_on_impl_thread_ = NULL;
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
	if( layer_tree_host_->settings().enable_edge_gloweffect ){
		  signal_event_++;
		  base::TimeDelta delay = base::TimeDelta::FromSeconds(1);
		  Proxy::ImplThread()->PostDelayedTask(
		  base::Bind(&ThreadProxy::ForceCommitUnblock,
					 weak_factory_on_impl_thread_.GetWeakPtr(), signal_event_),
		  delay);
	}
...
ThreadProxy::ScheduledActionDrawAndSwapInternal(bool forced_draw) {
  ...
  if (draw_frame) {
    ...
    VLOG(0)<<"cc_logs: "<<"Any problem with Draw-Frame vs-time= ?"<<scheduler_on_impl_thread_->LastVSyncTime().ToInternalValue();
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
void ThreadProxy::DidOverscroll(gfx::Vector2dF accumulated_root_overscroll_, 
                                gfx::Vector2dF current_fling_velocity_, bool hScroll, bool vScroll, gfx::SizeF size, float WebLayerY) {
  DCHECK(IsMainThread());
  if (!layer_tree_host_)
    return;
  SetGlowStatus(true);
  layer_tree_host_->DidOverscroll(accumulated_root_overscroll_, current_fling_velocity_, hScroll, vScroll, size, WebLayerY);
}

void ThreadProxy::ReleaseEdgeGlow(int axis) {
  DCHECK(IsMainThread());
  if (!layer_tree_host_)
    return;
  if(!layer_tree_host_->IsGlowAnimationActive())
    return; 
  layer_tree_host_->ReleaseEdgeGlow(axis);
  SetGlowStatus(false);
}

void ThreadProxy::SetGlowStatus(bool glowState) {
  DCHECK(IsImplThread());
  if (!layer_tree_host_impl_)
      return;
  layer_tree_host_impl_->SetGlowStatus(glowState);
}
#endif

#if defined (SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
void ThreadProxy::setGestureState(bool gestureState)
{
    DCHECK(IsImplThread());
    if (!layer_tree_host_)
        return;
    layer_tree_host_->setGestureState(gestureState);
}
#endif
    ...
    if (render_vsync_notification_enabled_) {
      ...
      frame_rate_controller.reset(
          new FrameRateController(DelayBasedTimeSource::Create(
              ...
              display_refresh_interval, Proxy::ImplThread()), Proxy::ImplThread()));
#else
...
void ThreadProxy::InitializeOutputSurfaceOnImplThread(
  ...
  *success = layer_tree_host_impl_->InitializeRenderer(output_surface.Pass());
  ...
  {
  	//Temporary Hack to wait for actual surface availability in longer span.
  	// The actual issue should be fixed by the UI team. They should create the tab only after surface view creation.
  	//This change should not be used in the Product. Till development this can be used.
  	//Maximum Buffer for Resource or Surface available is 200 MilliSeconds * 5 = 1 Sec
  	//Original buffer was 5 * ScheduleCompositeTime = 60 Milli Seconds
  	usleep(200000);
  }
  #endif
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
void ThreadProxy::ForceCommitUnblock(int signal_event) {
  if ((layer_tree_host_->settings().enable_edge_gloweffect) && (completion_event_for_commit_held_on_tree_activation_)) {
    if (signal_event == signal_event_) {
      completion_event_for_commit_held_on_tree_activation_->Signal();
      completion_event_for_commit_held_on_tree_activation_ = NULL;
      LOG(INFO) << __FUNCTION__ << "EdgeGlowEffectLog" << "*** UNBLOCKED FORCEFULLY ***";
    }
  }
}
#endif
...
void ThreadProxy::StartScrollbarAnimationOnImplThread() {
  layer_tree_host_impl_->decrcounter();
  if (layer_tree_host_impl_->getcounter()>0)
  return;
  layer_tree_host_impl_->StartScrollbarAnimation(
      layer_tree_host_impl_->CurrentFrameTimeTicks());
}
#if defined(SBROWSER_COMMIT_BLOCK_ONTOUCH)
void ThreadProxy::setCommitBlock(bool blockState)
{
     DCHECK(IsMainThread());
//    TRACE_EVENT("CCThreadProxy::setCommitBlock", this, 0);
//    TRACE_METHOD_ARGS("CCThreadProxy::setCommitBlock");
    //Proxy::implThread()->postTask(createCCThreadTask(this, &ThreadProxy::setCommitBlockOnImplThread,blockState));
     Proxy::ImplThread()->PostTask(base::Bind(&ThreadProxy::setCommitBlockOnImplThread, base::Unretained(this),blockState));
}
void ThreadProxy::setCommitBlockOnImplThread(bool blockState)
{

     DCHECK(IsImplThread());
    //TRACE_EVENT("CCThreadProxy::setCommitBlockOnImplThread", this, 0);
    //TRACE_METHOD_ARGS("CCThreadProxy::setCommitBlockOnImplThread");

    scheduler_on_impl_thread_->setNeedsCommitBlock(blockState);
}

/*
std::string ThreadProxy::getDumpLayersTree()
{
...
void ThreadProxy::DeferCommitsOnImplThread(bool active) {
  DCHECK(IsImplThread());
  Proxy::MainThread()->PostTask(
      base::Bind(&ThreadProxy::DeferCommitsOnGesture, main_thread_weak_ptr_, active));
}

void ThreadProxy::DeferCommitsOnGesture(bool active) {
  DCHECK(IsMainThread());
  layer_tree_host_->SetDeferCommits(active);
}
#endif


#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE)
void ThreadProxy::FlingAnimationCompletedOnImplThread(bool page_end){
  DCHECK(IsImplThread());
  Proxy::MainThread()->PostTask(
      base::Bind(&ThreadProxy::FlingAnimationCompleted, main_thread_weak_ptr_, page_end));
}

void ThreadProxy::FlingAnimationCompleted(bool page_end){
  DCHECK(IsMainThread());
  layer_tree_host_->FlingAnimationCompleted(page_end);
}
#endif

#if defined(SBROWSER_HIDE_URLBAR)
void ThreadProxy::DidHideUrlBarEvent(int event, int event_data1, int event_data2) {
  DCHECK(IsMainThread());
  layer_tree_host_->DidHideUrlBarEvent(event, event_data1, event_data2);
}

void ThreadProxy::DidHideUrlBarEventOnImplThread(int event, int event_data1, int event_data2) {
  DCHECK(IsImplThread());
  Proxy::MainThread()->PostTask(
      base::Bind(&ThreadProxy::DidHideUrlBarEvent, main_thread_weak_ptr_, event, event_data1, event_data2));
}

void ThreadProxy::HideUrlBarCmdReq(bool urlbar_active, bool override_allowed) {
  DCHECK(IsMainThread());
  Proxy::ImplThread()->PostTask(
      base::Bind(&ThreadProxy::HideUrlBarCmdReqOnImplThread, impl_thread_weak_ptr_, urlbar_active, override_allowed));
}

void ThreadProxy::HideUrlBarCmdReqOnImplThread(bool urlbar_active, bool override_allowed) {
  DCHECK(IsImplThread());
  layer_tree_host_impl_->SetUrlBarActive(urlbar_active, override_allowed);
}

#endif
#if defined(SBROWSER_ADJUST_WORKER_THREAD_PRIORITY)
void ThreadProxy::SetHighPriorityToRasterWorker(bool flag) {
    TRACE_EVENT1("cc","ThreadProxy::SetHighPriorityToRasterWorker", "flag", (flag ? "true" : "false"));
    DCHECK(IsMainThread());
    Proxy::ImplThread()->PostTask(base::Bind(
      &ThreadProxy::SetHighPriorityToRasterWorkerOnImplThread, impl_thread_weak_ptr_, flag));
}

void ThreadProxy::SetHighPriorityToRasterWorkerOnImplThread(bool flag) {
    DCHECK(IsImplThread());
    TRACE_EVENT1("cc","ThreadProxy::SetHighPriorityToRasterWorkerOnImplThread", "flag", (flag ? "true" : "false"));
    if(layer_tree_host_impl_) 
        layer_tree_host_impl_->SetHighPriorityToRasterWorker(flag); 
}
#endif
}  // namespace cc
