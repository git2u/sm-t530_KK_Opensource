...
#if defined(SBROWSER_GRAPHICS_GETBITMAP)
int GpuChannelManager::GetNativeBitmap(BitmapParams imageParams,
                                        int32 view_or_surface_id,int32 renderer_id, void** buffer)
{
  GpuChannelMap::const_iterator iter = gpu_channels_.find(renderer_id);
  int surface_id = GpuSurfaceTracker::Get()->LookupSurfaceForRenderer(
          renderer_id, view_or_surface_id) ;//view_or_surface_id;
  if (iter != gpu_channels_.end()) {
    if (!surface_id) {
      LOG(WARNING) <<"GetBitmap"<<"GpuChannelManager:SetNativeWindow can't find surface";
      return -1;
    }
    scoped_refptr<GpuChannel> channel = iter->second;
    return channel->GetNativeBitmap(imageParams, surface_id, buffer);
  } else {
    LOG(WARNING) <<"GetBitmap"<<"GpuChannelManager::SetNativeWindow can't find render "
        << renderer_id;
    return -1;
  }
 }
