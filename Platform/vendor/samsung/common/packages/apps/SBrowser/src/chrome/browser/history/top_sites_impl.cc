...
#if defined(SBROWSER_CQ_MPSG100014065)
//Added for debugging
// void PrintTopSites(const MostVisitedURLList& list) {
//   VLOG(0) << "VLOG " << __FUNCTION__ << " =============== START";
//   for(size_t i = 0; i < list.size(); i++) {
//     VLOG(0) << " VLOG url = " << list[i].url.spec();
//     VLOG(0) << " VLOG title = " << list[i].title;
//   }
//   VLOG(0) << "VLOG " << __FUNCTION__ << " =============== END";
// }

bool IsNewTopSitesValid(const MostVisitedURLList& list) {
  for(size_t i = 0; i < list.size(); i++) {
    if(list[i].url.is_empty()) {
      return true;
    }
  }
  return false;
...
#if defined(SBROWSER_QUICK_LAUNCH)
bool TopSitesImpl::IsBlacklistedFromRedirect(const GURL& gurl,
    GURL* out_url) {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  const DictionaryValue* blacklist =
      profile_->GetPrefs()->GetDictionary(prefs::kNtpMostVisitedURLsBlacklist);

  MostVisitedURLList most_visited_list = cache_->top_sites();
  if(!most_visited_list.empty()) {
    for (size_t i = 0; i < most_visited_list.size(); ++i) {
      const history::MostVisitedURL most_visited_url = most_visited_list[i];
      if(most_visited_url.url == gurl) {
        if(blacklist && blacklist->HasKey(GetURLHash(most_visited_url.url))) {
          *out_url = most_visited_url.url;
          return true;
        }
      }
      for (size_t i = 0; i < most_visited_url.redirects.size(); i++) {
        if (most_visited_url.redirects[i] == gurl) {
          if(blacklist && blacklist->HasKey(GetURLHash(most_visited_url.url))) {
            *out_url = most_visited_url.url;
            return true;
          }
  ...
  // Adding the search engine hard coded here because whenever we load the google.com
  // its coming with different ids and comparing is getting failed always
  // hence once remove google.com will never be coming into the most visited
  // list.
  GURL search_engine_url1("http://www.google.com/");
  std::string search_engine_title("google");
  std::string requested_url = gurl.spec();
  std::size_t loc = requested_url.find(search_engine_title);

  if(loc != std::string::npos ) {
    if(blacklist && blacklist->HasKey(GetURLHash(search_engine_url1))) {
      *out_url = search_engine_url1;
      return true;
    }
  }
...
bool TopSitesImpl::SetPageThumbnailNoDB(
  ...
  NotifyTopSitesThumbnailChanged();
#endif
  return true;
}
...
#if defined(SBROWSER_QUICKLAUNCH_OBSERVER)
  VLOG(0) << " VLOG delay in minutes = " << minutes;
#endif
...
#if defined(SBROWSER_CQ_MPSG100014065)
  if(IsNewTopSitesValid(new_top_sites)) {
    top_sites.clear();
    FilterTopSitesList(new_top_sites, &top_sites);
  }
#endif
...
#if defined(SBROWSER_QUICKLAUNCH_OBSERVER)
void TopSitesImpl::NotifyTopSitesThumbnailChanged() {
  VLOG(0) << " VLOG " << __FUNCTION__;
  content::NotificationService::current()->Notify(
      chrome::NOTIFICATION_TOP_SITES_THUMBNAIL_CHANGED,
      content::Source<TopSites>(this),
      content::NotificationService::NoDetails());
}
#endif
...
#if defined(SBROWSER_CQ_MPSG100014065)
void TopSitesImpl::FilterTopSitesList(const MostVisitedURLList& old_list,
                                   MostVisitedURLList* new_list) {
  for(size_t i = 0; i < old_list.size(); i++) {
    if(old_list[i].url.is_empty()) {
      continue;
    }
...
#if defined(SBROWSER_QUICKLAUNCH_OBSERVER)
void TopSitesImpl::UpdateTopSitesWithHistory() {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  if (loaded_ && temp_images_.size()) {
    timer_.Stop();
    StartQueryForMostVisited();
