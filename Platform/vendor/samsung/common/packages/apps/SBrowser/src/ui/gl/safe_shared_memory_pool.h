...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
  friend class SafeSharedMemoryPool;
  static ScopedSafeSharedMemory* create(SafeSharedMemoryPool* pool,
  	                 base::SharedMemory* memory,
  	                 size_t shm_size);
#endif
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
  ScopedSafeSharedMemory(const base::SharedMemoryHandle&);
#endif
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
  ScopedSafeSharedMemory* AcquireScopedSafeSharedMemory(base::SharedMemory* memory, size_t size);
#endif
