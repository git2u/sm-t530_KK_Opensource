...
#define GPU_COMMAND_BUFFER_CLIENT_GLES2_CMD_HELPER_AUTOGEN_H_
  ...
  void RenderbufferStorageMultisampleIMG(
      GLenum target, GLsizei samples, GLenum internalformat, GLsizei width,
      GLsizei height) {
    gles2::cmds::RenderbufferStorageMultisampleIMG* c =
        GetCmdSpace<gles2::cmds::RenderbufferStorageMultisampleIMG>();
    if (c) {
      c->Init(target, samples, internalformat, width, height);
    }
  }
  ...
  void FramebufferTexture2DMultisampleIMG(
      GLenum target, GLenum attachment, GLenum textarget, GLuint texture,
      GLint level, GLsizei samples) {
    gles2::cmds::FramebufferTexture2DMultisampleIMG* c =
        GetCmdSpace<gles2::cmds::FramebufferTexture2DMultisampleIMG>();
    if (c) {
      c->Init(target, attachment, textarget, texture, level, samples);
    }
  }
