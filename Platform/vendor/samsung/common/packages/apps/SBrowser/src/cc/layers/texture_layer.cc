...
// SAMSUNG: Ignore blocking in Single Buffer >>
// WAS: scoped_refptr<TextureLayer> TextureLayer::Create(TextureLayerClient* client) {
  // WAS: return scoped_refptr<TextureLayer>(new TextureLayer(client, false));
...
scoped_refptr<TextureLayer> TextureLayer::CreateForMailbox(
  ...
  return scoped_refptr<TextureLayer>(new TextureLayer(client, true,
      /*SAMSUNG: Ignore blocking in Single Buffer*/ false));
}

// SAMSUNG: Ignore blocking in Single Buffer >>
// WAS: TextureLayer::TextureLayer(TextureLayerClient* client, bool uses_mailbox)
...
// SAMSUNG: Ignore blocking in Single Buffer <<
    : Layer(),
      ...
      texture_id_(0),
      own_mailbox_(false),
      single_buffer_(single_buffer) { // SAMSUNG: Ignore blocking in Single Buffer 
...
// SAMSUNG: Ignore blocking in Single Buffer >>
  if(!CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableSingleBufferHint)) {
	single_buffer_ = false; //Single Buffer Hint disabled via command line flag
  }
// SAMSUNG: Ignore blocking in Single Buffer  <<
}
...
TextureLayer::~TextureLayer() {
  if (layer_tree_host()) {
    if (texture_id_ && /*SAMSUNG: Ignore blocking in Single Buffer */ !single_buffer_)
...
void TextureLayer::SetTextureId(unsigned id) {
  ...
  if (texture_id_ == id)
    return;
  if (texture_id_ && layer_tree_host() && /*SAMSUNG: Ignore blocking in Single Buffer */ !single_buffer_)
    layer_tree_host()->AcquireLayerTextures();
  texture_id_ = id;
  SetNeedsCommit();
}
...
void TextureLayer::WillModifyTexture() {
  if (layer_tree_host() && (DrawsContent() || content_committed_) && /*SAMSUNG: Ignore blocking in Single Buffer */ !single_buffer_) {
    layer_tree_host()->AcquireLayerTextures();
    content_committed_ = false;
  }
}
...
void TextureLayer::SetLayerTreeHost(LayerTreeHost* host) {
  if (texture_id_ && layer_tree_host() && host != layer_tree_host() && /*SAMSUNG: Ignore blocking in Single Buffer */ !single_buffer_)
    layer_tree_host()->AcquireLayerTextures();
