...
void  BrowsingDataRemover::GetCurrentCacheSize() { 

  BrowserThread::PostTask(
      BrowserThread::IO, FROM_HERE,
      base::Bind(&BrowsingDataRemover::GetCurrentCacheSizeOnIOThread,
                 base::Unretained(this)));
	
}
void BrowsingDataRemover::GetCurrentCacheSizeOnIOThread() {
  // This function should be called on the IO thread.
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::IO));
  DCHECK(main_context_getter_);
	net::HttpTransactionFactory* factory = main_context_getter_->GetURLRequestContext()->http_transaction_factory();
        disk_cache::Backend* disk_cache = factory->GetCache()->GetCurrentBackend();
	if (disk_cache) {
      ...
      base::Bind(&BrowsingDataRemover::OnGetCurrentCacheSize,
                 base::Unretained(this), stats[3].second)); //stats[3] is cache size
  	}
}
void BrowsingDataRemover::OnGetCurrentCacheSize(std::string currentCacheSize) {
	 DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
	FOR_EACH_OBSERVER(Observer, observer_list_, OnGetCurrentCacheSize(currentCacheSize));
}
