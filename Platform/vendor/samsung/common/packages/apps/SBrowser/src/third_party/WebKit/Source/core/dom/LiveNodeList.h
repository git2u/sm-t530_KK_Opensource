...
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    ALWAYS_INLINE Vector<Node* >& cachedNodes() const { return m_cachedNodes; }
#endif
...
private:
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    template <bool Compatible> Node* itemBeforeOrAfterCachedItem(unsigned offset, ContainerNode* root) const;
    template <bool Compatible> Node* traverseChildNodeListForwardToOffset(unsigned offset, Node* currentNode, unsigned& currentOffset) const;    
#else
...
#if defined(SBROWSER_QCOM_DOM_PREFETCH) 
    mutable Vector<Node* > m_cachedNodes;
#endif   
