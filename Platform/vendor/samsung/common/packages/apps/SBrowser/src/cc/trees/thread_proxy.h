...
class ThreadProxy : public Proxy,
 ...
 public:
  ...
  virtual void HideUrlBarCmdReq(bool urlbar_active, bool override_allowed) OVERRIDE;
#endif
  ...
  virtual void OnOverscrollOnImplThread(gfx::Vector2dF accumulated_root_overscroll_, 
                        gfx::Vector2dF current_fling_velocity_, bool hScroll, bool vScroll, gfx::SizeF size, float WebLayerY) OVERRIDE;
  virtual void DropEdgeGlowEffect(int axis) OVERRIDE;
  ...
  virtual void RequestScrollbarAnimationOnImplThread(base::TimeDelta delay)
    ...
    virtual void setGestureStateOnImplThread(bool gestureActive) OVERRIDE;
#endif
  virtual void DidReceiveLastInputEventForVSync(
      base::TimeTicks frame_time) OVERRIDE;
#if defined(SBROWSER_HIDE_URLBAR)
   virtual void DidHideUrlBarEventOnImplThread(int event, int event_data1, int event_data2) OVERRIDE;
#endif
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
   virtual void DeferCommitsOnImplThread(bool active) OVERRIDE;
#endif
#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE)
virtual void FlingAnimationCompletedOnImplThread(bool page_end) OVERRIDE;
#endif
...
///    virtual std::string getDumpLayersTree();
#endif
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
  void DidOverscroll(gfx::Vector2dF accumulated_root_overscroll_, 
                        gfx::Vector2dF current_fling_velocity_,bool hScroll, bool vScroll, gfx::SizeF size, float WebLayerY);
  void ReleaseEdgeGlow(int axis);
  void SetGlowStatus(bool glowState);
#endif
...
#if defined(SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
    void setGestureState(bool gestureState);
#endif
#if defined(SBROWSER_HIDE_URLBAR)
    void DidHideUrlBarEvent(int event, int event_data1, int event_data2);
#endif
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
   void DeferCommitsOnGesture(bool active);
#endif

#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE)
void FlingAnimationCompleted(bool page_end);
#endif
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
  void ForceCommitUnblock(int signal_event);
#endif
  void DidSwapUseIncompleteTileOnImplThread();
  void StartScrollbarAnimationOnImplThread();
  void MainThreadHasStoppedFlingingOnImplThread();
#if defined(SBROWSER_HIDE_URLBAR)
  void HideUrlBarCmdReqOnImplThread(bool urlbar_active, bool override_allowed);
#endif
#if defined(SBROWSER_ADJUST_WORKER_THREAD_PRIORITY)
   virtual void SetHighPriorityToRasterWorkerOnImplThread(bool flag);
#endif
