...
// JKK: required includes. also see command_buffer_service.gypi for include folders
/* EGLImage extension helpers. */

//#define GRALLOC_DUMMY_WRITING
#define GRALLOC_MODIFICATION

#include "third_party/android_tools/ndk/platforms/android-14/arch-arm/usr/include/EGL/eglext.h"
#include <dlfcn.h>
...
static void WrappedTexImage2D(
    ...
    if (border != 0) { 
        // We have incoming native handle
		// create a buffer from an existing handle. assume that "pixels" now holds flattened buffer object
		// and "border" holds size (as received from flatten() )
		DCHECK(texture);
		...
		android::sp<android::GraphicBuffer> gr_buffer = new GraphicBuffer;

		int32 fd_count = texture->getFileDescriptorCount();
		int32 fds[fd_count];

		for (int i = 0; i < fd_count; i++)
		{
			fds[i] = texture->getFileDescriptorAt(i);
		}

		// KK changes
		size_t border_size = border ;
		size_t fdCount = fd_count ;
		int const* fdArray = fds;
		gr_buffer->unflatten(pixels, border_size, fdArray, fdCount);

		// Actual mapping from grbuffer to texture. hard-coded EGL enum values to minimize KHR header includes

		const EGLint attrs[] = {EGL_IMAGE_PRESERVED_KHR, 0, /*EGL_NONE*/0x3038, /*EGL_NONE*/0x3038};
		EGLImageKHR pEGLImage = _eglCreateImageKHR(eglGetDisplay((EGLNativeDisplayType)0), /*EGL_NO_CONTEXT*/(EGLContext)0 , EGL_NATIVE_BUFFER_ANDROID, gr_buffer->getNativeBuffer(),attrs);
		_glEGLImageTargetTexture2DOES(target, pEGLImage);
	    _eglDestroyImageKHR(eglGetDisplay((EGLNativeDisplayType)0), pEGLImage);

    }
    else { 
    // Normal texture memory allocation
	glTexImage2D(
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
  explicit GLES2DecoderImpl(ContextGroup* group, content::GpuCommandBufferStub* gpuCmdBufferStub = NULL);
#else
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
  virtual void NotifyMMapFailure() OVERRIDE;
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  virtual void SetFileDescriptors(uint32 texid, int* fd_array, int fd_count) OVERRIDE;
#endif
 private:
...
#if !defined(SBROWSER_DISABLE_ROBUSTNESS_EXT)
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
  content::GpuCommandBufferStub* gpuCmdBufferStub;
#endif
...
bool BackRenderbuffer::AllocateStorage(const gfx::Size& size, GLenum format,
  ...
  if (samples <= 1) {
    ...
    if (GLES2Decoder::IsAngle()) {
      ...
      if (decoder_->GetGLContext()->HasExtension("GL_IMG_multisampled_render_to_texture"))
      {
          VLOG(0) << "Using glRenderbufferStorageMultisampleIMG" ;
      ...
      {
        VLOG(0) << "Using glRenderbufferStorageMultisampleEXT" ;
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
GLES2Decoder* GLES2Decoder::Create(ContextGroup* group, content::GpuCommandBufferStub* gpuCmdBufferStub) {
  return new GLES2DecoderImpl(group, gpuCmdBufferStub);
}
#else
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
GLES2DecoderImpl::GLES2DecoderImpl(ContextGroup* group, content::GpuCommandBufferStub* gpuBufferStub)
#else
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
      gpuCmdBufferStub(gpuBufferStub),
#endif
...
bool GLES2DecoderImpl::Initialize(
  ...
  if (offscreen) {
    if (attrib_parser.samples_ > 0 && attrib_parser.sample_buffers_ > 0 &&
      ...
      offscreen_target_samples_ = std::min(attrib_parser.samples_,
                                           max_sample_count);
    } else if (attrib_parser.samples_ > 0 && attrib_parser.sample_buffers_ > 0 &&
        features().multisample_render_to_texture) {
      GLint max_sample_count = 1;
      glGetIntegerv(0x9135, &max_sample_count);
      offscreen_target_samples_ = std::min(attrib_parser.samples_,
                                           max_sample_count);  
    }else {
  ...
  async_pixel_transfer_delegate_.reset(
      gfx::AsyncPixelTransferDelegate::Create(context.get(), this));
#else
  async_pixel_transfer_delegate_.reset(
...
void GLES2DecoderImpl::DoRenderbufferStorageMultisample(
    GLenum target, GLsizei samples, GLenum internalformat,
    GLsizei width, GLsizei height) {
  if (!features().chromium_framebuffer_multisample && !features().multisample_render_to_texture) {
    LOCAL_SET_GL_ERROR(
        GL_INVALID_OPERATION,
  ...
  if (IsAngle()) {
    ...
    if (context_->HasExtension("GL_IMG_multisampled_render_to_texture")){
...
void  GLES2DecoderImpl::DoFramebufferTexture2DMultisample(
       ...
       GLuint client_texture_id, GLint level, GLsizei samples)
{
     VLOG(0) << "***** GLES2DecoderImpl::DoFramebufferTexture2DMultisample" ;
     if (!features().multisample_render_to_texture) {
       LOCAL_SET_GL_ERROR(GL_INVALID_OPERATION, "FramebufferTexture2DMultisample" , 
     ...
     if (!texture_manager()->ValidForTarget(textarget, level, 0, 0, 1)) {
         LOCAL_SET_GL_ERROR(GL_INVALID_VALUE, "FramebufferTexture2DMultisample" , 
                    "level out of range");
         return;
     }     

    GLenum error = GL_NO_ERROR;
     if (context_->HasExtension("GL_EXT_multisampled_render_to_texture")){
         LOCAL_COPY_REAL_GL_ERRORS_TO_WRAPPER("glFramebufferTexture2DMultisampleEXT");
         glFramebufferTexture2DMultisampleEXT(
            target, attachment, textarget, service_id, level, samples);
         error = LOCAL_PEEK_GL_ERROR("glFramebufferTexture2DMultisampleEXT");
     } else {
         LOCAL_COPY_REAL_GL_ERRORS_TO_WRAPPER("glFramebufferTexture2DMultisampleIMG");
         glFramebufferTexture2DMultisampleIMG(
            target, attachment, textarget, service_id, level, samples);
         error = LOCAL_PEEK_GL_ERROR("glFramebufferTexture2DMultisampleIMG");
     }
...
// LOCAL_PERFORMANCE_WARNING("Some textures are unrenderable."); // commented for  avoiding crash  due to OOM
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  if (border != 0) { 
      //Avoid possible validation errors if border != 0 ie. we have graphicbuffer texture
	  if (!ValidateTexImage2D("glTexImage2D", target, level, internal_format,
	      width, height, 0, format, type, pixels, pixels_size)) {
	    return;
	  }
  } else {
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  // Here also need to check if border == 0. If not then we have graphicbuffer-based texture
  // that is already filled with pixels
  if (!teximage2d_faster_than_texsubimage2d_ && level_is_same && pixels && (border == 0)) {
#else
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
    // In GraphicBuffer case, the shared buffer size is passed in the border attribute
	if (border != 0) 
	{
		pixels = GetSharedMemoryAs<const void*>(
			pixels_shm_id, pixels_shm_offset, border);
	}
	else
	{
#endif
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
void GLES2DecoderImpl::SetFileDescriptors(uint32 texid, int* fd_array, int fd_count)
{
	Texture* texture = GetTexture(texid);
	if (texture)
	{
		texture->setFileDescriptors(fd_count, fd_array);
	}
}
#endif
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
void GLES2DecoderImpl::NotifyMMapFailure() {
    gpuCmdBufferStub->Send(new GpuHostMsg_MMapFailed(gpuCmdBufferStub->route_id()));
}
#endif
