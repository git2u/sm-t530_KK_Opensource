...
ScopedJavaLocalRef<jobject> CreateJavaBitmapWith565Config(const gfx::Size& size) {

    return ui::Java_BitmapHelper_createBitmapWith565Config(AttachCurrentThread(),
      size.width(), size.height());
}

ScopedJavaLocalRef<jobject> ConvertToJavaBitmapWith565Config(const SkBitmap* skbitmap) {
  
  DCHECK(skbitmap);
  DCHECK_EQ(skbitmap->bytesPerPixel(), 2);

  // Create a temporary java bitmap.
  ScopedJavaLocalRef<jobject> jbitmap =
      CreateJavaBitmapWith565Config(gfx::Size(skbitmap->width(), skbitmap->height()));
...
SkBitmap ConvertToSkBitmap(ScopedJavaLocalRef<jobject> jbitmap) {
#else
