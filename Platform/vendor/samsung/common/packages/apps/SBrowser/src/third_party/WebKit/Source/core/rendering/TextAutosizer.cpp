...
#if defined(SBROWSER_TEXT_AUTOSIZER)
    //Deepak Changes Start
    float mul = m_document->settings()->textAutosizingFontScaleFactor();
    if (mul*1.00f >= multiplier*0.85f)
        multiplier = mul*1.00f;
    else
        multiplier = multiplier*0.85f;
    //Deepak Changes End
#endif
...
#if defined(SBROWSER_TEXT_AUTOSIZER)
        if (rendererNode && rendererNode->hasTagName(liTag)) {
            Node* childNode = renderer->firstChild() ? renderer->firstChild()->generatingNode() : 0;
            if (childNode && childNode->hasTagName(aTag)) {
                AtomicString linkURL = toElement(childNode)->getAttribute(hrefAttr);
                if (linkURL.startsWith("http://media.daum.net/v/"))
                    return true;
            }
        }
#endif
