...
class URLRequestHttpJob::HttpTransactionDelegateImpl
 ...
 public:
  ...
  virtual int GetRouteID() const OVERRIDE {
    const content::ResourceRequestInfoImpl* req_info =
        content::ResourceRequestInfoImpl::ForRequest(request_);
    if(req_info)
      return req_info->GetRouteID();
    //FIXME : Figure out a method to get the route id always!!!
    return -1;
  }
  ...
  virtual int GetChildID() const OVERRIDE {
    const content::ResourceRequestInfoImpl* req_info =
        content::ResourceRequestInfoImpl::ForRequest(request_);
    if(req_info)
      return req_info->GetChildID();
    //FIXME : Figure out a method to get the child id always!!!
    return -1;
