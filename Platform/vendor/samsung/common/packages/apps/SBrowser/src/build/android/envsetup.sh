...
#############################################
# Workaround to apply USE_CCACHE.
# buildscript/build_env.default will export the ccache executable path as CCACHE_EXE.
if [[ "$USE_CCACHE" == "1" ]]; then
  ANDROID_GOMA_WRAPPER="$CCACHE_EXE"
elif [[ -d $GOMA_DIR ]]; then
