...
Document::Document(Frame* frame, const KURL& url, bool isXHTML, bool isHTML,bool isWML)
#else
...
#endif
//SAMSUNG_CHANGES
#if ENABLE(SBROWSER_FONT_BOOSTING_PREFERENCES)
...
void Document::updateStyleIfNeeded()
{
    ...
    if(m_frame && m_frame->animation() && m_frame->animation()->isSetDelayLayoutAndUpdateStyle()) {
        TRACE_EVENT0("webkit", "Document::updateStyleIfNeeded : scheduleStyleRecalc");
        scheduleStyleRecalc();
        return;
    }
#endif
...
Document* Document::parentDocument() const
{
    if (!m_frame)
        return 0;
#if defined(SBROWSER_PLM_P140212_00046)
    if (!m_frame->tree())
        return 0;
#endif
...
// SAMSUNG CHANGE : For keylagging perfomance >>
#if defined (SBROWSER_KEYLAGGING_PERFORMANCE)
void Document::setUpdateStyleIfNeeded(bool updateStyleIfNeeded)
{
    m_updateStyleIfNeeded = updateStyleIfNeeded;
}
#endif
// SAMSUNG CHANGE : For keylagging perfomance <<
