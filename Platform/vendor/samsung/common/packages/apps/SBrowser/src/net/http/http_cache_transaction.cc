...
#if defined(SBROWSER_SESSION_CACHE)
		,store_response_in_session_cache_(false)
		,active_response_to_update_(NULL)
#endif
	  {
...
#if defined(SBROWSER_SESSION_CACHE)
  if ((mode_ == NONE || result < 0) && (!store_response_in_session_cache_))
#else
...
int HttpCache::Transaction::DoCacheWriteDataComplete(int result) {
  ...
  if (result == 0) {
#if defined(SBROWSER_SESSION_CACHE)
  // As the store_response_in_session_cache_ has been set so
  // we will take control of the response :)
  // We have a http cache entry ready, so just keep it alive till
  // we have the complete response.

  if(SessionCacheStorage::GetInstance()->IsSessionCacheEnabled() &&
      store_response_in_session_cache_ && request_->url == url_to_store_) {
      // tell the response that collation is done, can do book-keeping now.
      SessionCacheStorage::GetInstance()
      ->UpdateCacheStorageData(active_response_to_update_->response_body_length());
      //reset the state so that we are ready for the next transaction
      store_response_in_session_cache_ = false;
      url_to_store_ = GURL();
      active_response_to_update_ = NULL;

      // doom the entry, by forcing a false
      // on request itself an entry is created, we have to make sure that
      // it does not stay.
      DoneWritingToEntry(false);
      return result;
    }
...
#if defined(SBROWSER_SESSION_CACHE)
  // As the store_response_in_session_cache has been set so
  // we will take control of the response :)
  // We have a http cache entry ready, so just keep it alive till
  // we have the complete response.
  SessionCacheStorage* session_cache = SessionCacheStorage::GetInstance();
  if (session_cache->IsSessionCacheEnabled() && store_response_in_session_cache_ &&
      request_->url == url_to_store_) {
    //store the entry in session cache
    if (!partial_.get() || !data_len) {
      // add the data to session cache
      if (!active_response_to_update_) {
        DCHECK(transaction_delegate_);
        const net::SessionCacheID cache_id(transaction_delegate_->GetRouteID(),
                                           transaction_delegate_->GetChildID());
        active_response_to_update_ = session_cache->LoadResponse(url_to_store_,
                                                                 cache_id);
      }
      if (active_response_to_update_ && data_len) {
        if (active_response_to_update_->isResponseBodyMarkedForClearing()) {
          //clear up old data so that we can store the new one.
          active_response_to_update_->clearCachedResponse();
        }
        active_response_to_update_->collateResponseBody(data, data_len);
      }
    } else {
      // FIXME::need to figure out what has to be done!!
      LOG(INFO) << "Session Cache: We should not be here!!!! ";
      LOG(INFO) << "Session Cache: FIXME Check what is to be done for this url : " << request_->url;
  }
  return data_len;
}
#endif 
...
#else
    std::string mimeType;
    response_.headers->GetMimeType(&mimeType);
    std::transform(mimeType.begin(), mimeType.end(), mimeType.begin(), ::tolower);

  if (cache_->mode() != RECORD &&
      response_.headers->HasHeaderValue("cache-control", "no-store")
     && (mimeType.find("text/cache-manifest") == string::npos)) {
    SessionCacheStorage* session_cache = SessionCacheStorage::GetInstance();
    if (session_cache->IsSessionCacheEnabled() &&
      request_->url.SchemeIs("http")) {
      LOG(INFO) << "Session Cache: Mime type : " << mimeType;
      LOG(INFO) << "Session Cache: Storing this url to session cache : " << request_->url;
      
      // so some one has taken the pain to get the contents from network
      // ,so now we need to update the session cache entry
      // The header response will be updated in the interceptor, so we are
      // reseting the response body only

      url_to_store_ = request_->url;

      DCHECK(transaction_delegate_);
      const net::SessionCacheID cache_id(transaction_delegate_->GetRouteID(),
                                         transaction_delegate_->GetChildID());
      active_response_to_update_
              = session_cache->LoadResponse(url_to_store_,
                                            cache_id);
      if (active_response_to_update_) {
        active_response_to_update_->markResponseBodyForClearing();
        //active_response_to_update_->clearCachedResponse();
      } else {
        net::HttpResponseInfo* rinfo = new net::HttpResponseInfo(response_);
        active_response_to_update_ = new SessionCacheResponseInfo(
                    url_to_store_, rinfo, cache_id);
        active_response_to_update_->AddRef();
        session_cache->AddToCache(url_to_store_,
                                  active_response_to_update_,
                                  cache_id);
      }

      //we will mark this url and response for storing in session cache
      store_response_in_session_cache_ = true;
    } else {
      //we do not have session cache, so doom the entry
      DoneWritingToEntry(false);
      ReportCacheActionFinish();
      if (net_log_.IsLoggingAllEvents())
        net_log_.EndEvent(NetLog::TYPE_HTTP_CACHE_WRITE_INFO);
    ...
    return OK;
  } else if (net::IsCertStatusError(response_.ssl_info.cert_status)) {
    DoneWritingToEntry(false);
    ReportCacheActionFinish();
    if (net_log_.IsLoggingAllEvents())
      net_log_.EndEvent(NetLog::TYPE_HTTP_CACHE_WRITE_INFO);
...
#if defined(SBROWSER_SESSION_CACHE)
  //keep the entry live as we need it.
  if(!store_response_in_session_cache_){
