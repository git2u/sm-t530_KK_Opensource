...
SBROWSER_PACKAGE_NAMES = {
   'android-sbrowser-samsung-chrome': 'com.sec.android.app.sbrowser'
}

ALL_BROWSER_TYPES = ','.join(['android-content-shell'] +
                             CHROME_PACKAGE_NAMES.keys() +
			     SBROWSER_PACKAGE_NAMES.keys())
...
# Add SBROWSER Portable Browser for benchmark
SBROWSER_ANDROID_BROWSER_PACKAGE = 'com.sec.android.app.sbrowser'
SBROWSER_ANDROID_BROWSER_ACTIVITY = 'com.sec.android.app.sbrowser.SBrowserMainActivity'
SBROWSER_ANDROID_BROWSER_COMMAND_LINE = '/data/local/tmp/chromium-testshell-command-line'
SBROWSER_ANDROID_BROWSER_DEVTOOLS_REMOTE_PORT = (
...
def FindAllAvailableBrowsers(options, logging=real_logging):
  ...
  for name, package in CHROME_PACKAGE_NAMES.iteritems():
    if 'package:' + package in packages:
      ...
      possible_browsers.append(b)
   #Check for SBrowser packages
  for name, package in SBROWSER_PACKAGE_NAMES.iteritems():
    if 'package:' + package in packages:
      b = PossibleAndroidBrowser(name,
                                 options, adb,
                                 package, True,
                                 SBROWSER_ANDROID_BROWSER_COMMAND_LINE,
                                 SBROWSER_ANDROID_BROWSER_ACTIVITY,
                                 SBROWSER_ANDROID_BROWSER_DEVTOOLS_REMOTE_PORT)
      possible_browsers.append(b)
