...
void SkBBoxRecord::drawText(const void* text, size_t byteLength, SkScalar x, SkScalar y,
    ...
    SkScalar pad = (metrics.fBottom - metrics.fTop) /* / 2 */; // SAMSUNG CHANGE : increase pad double due to korean text clipped issue.
    bbox.fLeft  -= pad;
    bbox.fRight += pad;
...
void SkBBoxRecord::drawPosText(const void* text, size_t byteLength,
    ...
    SkScalar pad = (metrics.fTop - metrics.fBottom) /* / 2 */; // SAMSUNG CHANGE : increase pad double due to korean text clipped issue.
    bbox.fLeft += pad;
    bbox.fRight -= pad;
