...
#if defined(SBROWSER_QCOM_DOM_CACHED_SELECTOR)    
    SelectorQueryResultsCache::getInstance()->invalidateAll();
#endif
...
#if defined(SBROWSER_QCOM_DOM_CACHED_SELECTOR)    
    SelectorQueryResultsCache *resultsCache = SelectorQueryResultsCache::getInstance();

    PassRefPtr<NodeList> toReturn = resultsCache->getQuery(this, selectors);
    if (toReturn != 0)
        return toReturn;    
#endif
...
#if defined(SBROWSER_QCOM_DOM_CACHED_SELECTOR)    
    PassRefPtr<NodeList> toReturn2 = selectorQuery->queryAll(this);
    return resultsCache->addQuery(this, selectors, toReturn2);
#else
