...
#if defined(SBROWSER_CUSTOM_HTTP_REQUEST_HEADER)
  if (hasCustomHttpAcceptLanguageHeader()) {
    request_headers_.RemoveHeader(HttpRequestHeaders::kAcceptLanguage);
	request_headers_.MergeFrom(getCustomHttpAcceptLanguageHeader());
  }
  if (hasCustomHttpRequestHeader()) {
    request_headers_.MergeFrom(getCustomHttpRequestHeader());
  }
  // SAMSUNG CHANGE : remove UAP header if UA is not mobile
  // this feature is under SBROWSER_CUSTOM_HTTP_REQUEST_HEADER
  // because UAP can't be exist w/o SBROWSER_CUSTOM_HTTP_REQUEST_HEADER feature
  std::string value;
...
int HttpNetworkTransaction::HandleCertificateRequest(int error) {
  ...
  if (!found_cached_cert || !client_cert)
#else
  if (!found_cached_cert)
#endif
    return error;
#if defined(SBROWSER_CAC)
  LOG(INFO) << "HttpNetworkTransaction::HandleCertificateRequest() cached client cert found! client_cert:" << client_cert;
#endif
