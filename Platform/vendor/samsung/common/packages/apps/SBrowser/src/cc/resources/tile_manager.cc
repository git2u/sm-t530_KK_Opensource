...
inline TileManagerBin BinFromTilePriority(const TilePriority& prio) {
  ...
  const float kBackflingGuardDistancePixels = 1582.0f;//314.0f;
#else
  const float kBackflingGuardDistancePixels = 942.0f;//314.0f;
#endif
  // SAMSUNG CHANGE : Power consumption(reduce tile & fling speed)<<
  ...
  if (prio.time_to_visible_in_seconds == 0 ||
    ...
    return NOW_BIN;
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  //Allow tile binning to SOON bins so we can allocate native buffers for NOW and SOON bins
  //Just allocating to NOW bins is not good as they cover only viewport plus a small buffer zone
  if ((prio.resolution == NON_IDEAL_RESOLUTION) && !TileManager::zerocopy_)
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  TileManager::zerocopy_ = CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableNativeBufferTextures);
#endif
}
...
void TileManager::AssignBinsToTiles() {
  ...
  for (TileVector::iterator it = tiles_.begin();
	...
	TileManagerBin gpu_memmgr_stats_bin = NEVER_BIN;
    ...
    gpu_memmgr_stats_bin = BinFromTilePriority(tile->combined_priority());
	...
	if (tile->drawing_info().requires_resource()) {
		...
		if (tile->drawing_info().memory_state_ != NOT_ALLOWED_TO_USE_MEMORY)
            memory_used_bytes_ += tile->bytes_consumed_if_allocated();	
	}
...
void TileManager::CheckForCompletedTileUploads() {
  while (!tiles_with_pending_upload_.empty()) {
    ...
    //bool was_forced = tile->drawing_info().forced_upload_;
...
void TileManager::ForceTileUploadToComplete(Tile* tile) {
  ...
  if (tile->drawing_info().resource_ &&
    ...
    DidFinishTileInitialization(tile);
	DCHECK(tile->drawing_info().IsReadyToDraw());
  }
...
void TileManager::GetMemoryStats(
  ...
  *memory_used_bytes = memory_used_bytes_;
}
...
void TileManager::AddRequiredTileForActivation(Tile* tile) {
	DCHECK(std::find(tiles_that_need_to_be_initialized_for_activation_.begin(),
		tiles_that_need_to_be_initialized_for_activation_.end(), tile) == 
...
void TileManager::AssignGpuMemoryToTiles() {
  ...
  for (TileVector::iterator it = tiles_.begin();
     ...
     if (!drawing_info.requires_resource())
       ...
       if (drawing_info.memory_state_ == USING_UNRELEASABLE_MEMORY) {
        if (tile->required_for_activation()) {
         ...
         if (!tile->drawing_info().forced_upload_ &&
	...
	if (!drawing_info.resource_ && tile->required_for_activation())
	    AddRequiredTileForActivation(tile);
  }
...
bool TileManager::DispatchImageDecodeTasksForTile(Tile* tile) {
  ...
  for (PicturePileImpl::PixelRefIterator iter(tile->content_rect(),
    ...
    // SAMSUNG CHANGE : Apply opensoure 16332004 >>
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
    // Android native_buffer in use
    // If tiles are in bin NOW or SOON in either tree and texupload patch is enabled then try to allocate from native buffers
    // Resource pool can still deny this if GraphicBuffer count is too high or tile dimensions are other than default
    // If tiles are in bin NOW or SOON in either tree and texupload patch is enabled then try to allocate from native buffers
    // Resource pool can still deny this if GraphicBuffer count is too high or tile dimensions are other than default
      ,(((tile->managed_state_.bin[0]<EVENTUALLY_BIN) || (tile->managed_state_.bin[1]<EVENTUALLY_BIN)) ?
    		  TileManager::zerocopy_ :
    		  0)
#endif
  );
...
void TileManager::DispatchOneRasterTask(scoped_refptr<Tile> tile) {
  ...
  buffer_data.stride = 0;
  buffer_data.buffer = resource_pool_->resource_provider()->MapPixelBuffer(
      resource_id, &buffer_data.stride);

  // skia requires that our buffer be 4-byte aligned
  CHECK(!(reinterpret_cast<intptr_t>(buffer_data.buffer) & 3));
...
void TileManager::OnRasterTaskCompleted(
  ...
  if (tile->drawing_info().memory_state_ == USING_RELEASABLE_MEMORY) {
	...
	if (tile->required_for_activation() && client_->ShouldForceTileUploadsRequiredForActivationToComplete())
		ForceTileUploadToComplete(tile);
...
void TileManager::DidFinishTileInitialization(Tile* tile) {
  if (tile->priority(ACTIVE_TREE).distance_to_visible_in_pixels == 0)
    did_initialize_visible_tile_ = true;
  	if (tile->required_for_activation()) {
		DCHECK(std::find(tiles_that_need_to_be_initialized_for_activation_.begin(),
			tiles_that_need_to_be_initialized_for_activation_.end(), tile) != tiles_that_need_to_be_initialized_for_activation_.end());
			tiles_that_need_to_be_initialized_for_activation_.erase(tile);
	}
}
...
void TileManager::RunRasterTask(
  ...
  SkBitmap bitmap;
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
   if (TileManager::zerocopy_)
     bitmap.setConfig(SkBitmap::kARGB_8888_Config, rect.width(), rect.height(), buffer_data.stride);
   else
#endif
     bitmap.setConfig(SkBitmap::kARGB_8888_Config, rect.width(), rect.height());
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
   bitmap.setPixels(buffer_data.buffer);
#else
...
void TileManager::RunImageDecodeTask(
  ...
  if(!pixel_ref  ||  pixel_ref->getRefCnt() <= 0)
  {
  	VLOG(0)<<"cc_logs: "<<"RunImageDecodeTask - pixel_ref is  Invalid ";
  	return ;
  }
...
void TileManager::SetHighPriorityToRasterWorker(bool flag, TileManager* tile_manager) {
  TRACE_EVENT2("cc","TileManager::SetHighPriorityToRasterWorker", "flag", (flag ? "true" : "false"), "tile_manager", tile_manager);

  if(tile_manager && tile_manager->raster_worker_pool_){
#if defined(SBROWSER_INCREASE_RASTER_THREAD)
    if(flag){
      tile_manager->SetNumRasterThreads(4);
    } else {
      tile_manager->SetNumRasterThreads(1);
    }
#endif // defined(SBROWSER_INCREASE_RASTER_THREAD)
    tile_manager->raster_worker_pool_->SetThreadHighPriority(flag);
  }        
}
#endif

#if defined(SBROWSER_INCREASE_RASTER_THREAD)
void TileManager::SetNumRasterThreads(size_t num_threads){

    //Tear-Down existing Raster Worker Pool
    raster_worker_pool_->Shutdown();
    AbortPendingTileUploads();
    //Re-create Raster Worker Pool
    raster_worker_pool_ = RasterWorkerPool::Create(this, num_threads);
    //update max pending tasks	    
    max_pending_tasks_ = num_threads*	kMaxNumPendingTasksPerThread;
