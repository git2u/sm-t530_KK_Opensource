...
class InMemoryHistoryBackend : public content::NotificationObserver {
 ...
 private:
  ...
  // Handler for NOTIFICATION_HISTORY_SEARCH_TERMS_RESULTS_UPDATED
  // To receive notification from history to update in-memory db of search results
  void OnSearchTermsResultsUpdated(const KeywordSearchTermDetails& details);
#endif
