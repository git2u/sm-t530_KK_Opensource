...
inline Node* LiveNodeListBase::traverseChildNodeListForwardToOffset(unsigned offset, Node* currentNode, unsigned& currentOffset) const
{
    ...
    ASSERT(currentOffset < offset);
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    Vector<Node* >& cachedNodesList = cachedNodes(); 
#endif
    while ((currentNode = currentNode->nextSibling())) {
        ...
        currentOffset++;
        if (Compatible) {
            if (currentOffset >= cachedNodesList.size())
                cachedNodesList.resize(currentOffset + 1);
            cachedNodesList.data()[currentOffset] = currentNode;
        }
        if (currentOffset == offset)
            return currentNode;
#else
...
{
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    bool Compatible = Node::IsArchitectureCompatible();
    Vector<Node* >& cachedNodesList = cachedNodes();
    if (Compatible) {
        Node* result;
        if (offset < cachedNodesList.size()) {
            result = cachedNodesList[offset];
            if (result)
                return result;
        }
    }
...
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    if (Compatible && isItemCacheValid()) {
        if (cachedItemOffset() >= cachedNodesList.size())
            cachedNodesList.resize(cachedItemOffset() + 1);
        cachedNodesList.data()[cachedItemOffset()] = cachedItem();
    }
#endif
...
#if defined(SBROWSER_QCOM_DOM_PREFETCH)
    if (Compatible)
        return itemBeforeOrAfterCachedItem<true>(offset, root);
    else
        return itemBeforeOrAfterCachedItem<false>(offset, root);   
#else
...
inline Node* LiveNodeListBase::itemBeforeOrAfterCachedItem(unsigned offset, ContainerNode* root) const
{
...
#if defined(SBROWSER_QCOM_DOM_PREFETCH)              
            if (Compatible) {
                cachedNodesList.data()[currentOffset] = currentItem;
            }   
#endif       
            if (currentOffset == offset) {
    ...
    if (type() == ChildNodeListType)
#if defined(SBROWSER_QCOM_DOM_PREFETCH)  
        currentItem = traverseChildNodeListForwardToOffset<Compatible>(offset, currentItem, currentOffset);
#else
