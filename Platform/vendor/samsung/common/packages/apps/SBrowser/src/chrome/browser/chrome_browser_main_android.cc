...
void ChromeBrowserMainPartsAndroid::PreEarlyInitialization() {
...

  {
    ...
    #if defined(SBROWSER_GRAPHICS_UI_COMPOSITOR_REMOVAL)
	/*  Enabling the UI Compositor via command line file only
	*    Following code will be un-commented once runtime flag is enabled in Mainline*/
  if(!(CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableSingleCompositor)))
  {
	CommandLine::ForCurrentProcess()->AppendSwitch(switches::kEnableSingleCompositor);

  }
  if(!(CommandLine::ForCurrentProcess()->HasSwitch(switches::kDisableSurfaceWorkaround)))
  {
	CommandLine::ForCurrentProcess()->AppendSwitch(switches::kEnsureSurfaceWorkaround);
  }
  
  if(!(CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableSingleCompositor)))
  {
