...
bool TopSitesLikelyImpl::IsBlacklistedFromRedirect(const GURL& gurl,
    GURL* out_url) {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  const DictionaryValue* blacklist =
      profile_->GetPrefs()->GetDictionary(prefs::kNtpMostVisitedURLsBlacklist);
...
void TopSitesLikelyImpl::FilterTopSitesList(const MostVisitedURLList& old_list,
...
void TopSitesLikelyImpl::UpdateTopSitesWithHistory() {
//NotImplemented.
}
#endif
