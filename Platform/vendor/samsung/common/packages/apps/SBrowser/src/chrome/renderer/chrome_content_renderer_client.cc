...
#define LOGE_NOJUNG(...) __android_log_print(ANDROID_LOG_DEBUG, "ChromeContentRendererClient", __VA_ARGS__)
...
void ChromeContentRendererClient::GetNavigationErrorStrings(
  ...
  if (error_html) {
    ...
    if (extension && !extension->from_bookmark()) {
      ...
      if( ((strcmp(SecNativeFeature::getInstance()->getString(CscFeatureTagWeb_CustomizeErrorPage), "SKO") == 0) ||
      	(strcmp(SecNativeFeature::getInstance()->getString(CscFeatureTagWeb_CustomizeErrorPage), "KTO") == 0) ||
      	(strcmp(SecNativeFeature::getInstance()->getString(CscFeatureTagWeb_CustomizeErrorPage), "LUO") == 0)) && (language.compare("ko") == 0) ) {
      	LOGE_NOJUNG("Error Reason : %d", error.reason);
      	switch(error.reason)
      	{      		
    	  ...
    	  resource_id = IDR_NET_ERROR_HTML_KOR;
      			break;
      		default:
      			break;      			
      	}    	  
