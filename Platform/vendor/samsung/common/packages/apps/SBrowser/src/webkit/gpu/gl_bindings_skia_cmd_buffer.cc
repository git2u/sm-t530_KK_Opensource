...
GrGLInterface* CreateCommandBufferSkiaGLBinding() {
  ...
  else if (GrGLHasExtensionFromString("GL_EXT_multisampled_render_to_texture", extStr))
  {
    interface->fRenderbufferStorageMultisample =  glRenderbufferStorageMultisampleEXT;
    interface->fFramebufferTexture2DMultisample = glFramebufferTexture2DMultisampleEXT;
  }
  else if (GrGLHasExtensionFromString("GL_IMG_multisampled_render_to_texture", extStr))
  {
