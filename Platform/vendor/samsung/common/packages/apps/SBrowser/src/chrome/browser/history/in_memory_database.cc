...
//Create the search terms results table.
  if (!InitSearchTermsResultsTable()) {
    NOTREACHED() << "Unable to create search terms results";
    db_.Close();
    return false;
  }
#endif
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  if (!db_.Execute(
      "INSERT OR IGNORE INTO urls SELECT DISTINCT u.id, u.url, u.title, u.visit_count, "
      "u.typed_count, u.last_visit_time, u.hidden, u.favicon_id "
      "FROM history.urls u JOIN history.keyword_search_terms kst, "
	  "history.search_terms_results sr "
      "WHERE u.typed_count = 0 AND (u.id = kst.url_id OR u.id = sr.url_id)")) {
    // Unable to get data from the history database. This is OK, the file may
    // just not exist yet.
	  LOG(ERROR)<<"Could not load in memory url db";
  }
#else
  if(!db_.Execute(
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  //copy search terms and results url into memory
  //in G we trust. log time as here as well.
  begin_load = base::TimeTicks::Now();
  if (!db_.Execute(
        "INSERT INTO search_terms_results SELECT * FROM "
        "history.search_terms_results")) {
      // Unable to get data from the history database. This is OK, the file may
      // just not exist yet.
	  LOG(ERROR)<<"insert into in memory search_terms_results failed ";
    }
    end_load = base::TimeTicks::Now();
#endif
