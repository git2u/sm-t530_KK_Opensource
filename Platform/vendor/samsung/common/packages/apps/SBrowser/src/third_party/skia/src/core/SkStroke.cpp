...
void SkPathStroker::fixFirstPoint() {

    SkVector normal;
    SkVector unitNormal;
    preJoinTo(fFirstPt, &normal, &unitNormal, true);

    if (!is_clockwise(normal, fFirstNormal))
    {
        SkTSwap<SkPath>(fInner, fOuter);
        normal.negate();
        fFirstNormal.negate();

    }
    SkPoint first;
    if (SkStrokerPriv::correctInnerPoint(first, fFirstPt, normal, fFirstNormal) ) {
        SkPathRef::Editor ed(&fInner.fPathRef);
        SkPoint* pt = ed.atPoint(0);
        *pt = first;
    }
    fOuter.lineTo(fFirstPt.fX + normal.fX, fFirstPt.fY + normal.fY);
    postJoinTo(fFirstPt, normal, unitNormal);
}
#endif
