...
void ConvertBookmarkNode(
  ...
  jnode->Reset(
      Java_BookmarkNode_create(
          ...
          parent_node.obj(),node->Editable(),node->OperatorBookmark()));
#else
  jnode->Reset(
      Java_BookmarkNode_create(
          env, node->id(), (jint) node->type(), title.obj(), url.obj(),
...
void ChromeBrowserProvider::Observe(
  ...
  if (type == chrome::NOTIFICATION_HISTORY_URL_VISITED ||
    ...
    if (obj.is_null())
      return;
    //Bookmark Changed notification should not be sent on History data change.
    //Java_ChromeBrowserProvider_onBookmarkChanged(env, obj.obj());
  } else if (type ==
      chrome::NOTIFICATION_HISTORY_KEYWORD_SEARCH_TERM_UPDATED) {
    JNIEnv* env = AttachCurrentThread();
...
//This API needed to access the private bookmark_model_ to its child
//classes.
BookmarkModel* ChromeBrowserProvider::GetBookmarkModel() {
  return bookmark_model_;
}

#ifdef SBROWSER_QUICK_LAUNCH
//This API need to access the private top_sites_ to its child
//classes.
history::TopSites* ChromeBrowserProvider::GetTopSites() {
  return top_sites_;
}
#endif
