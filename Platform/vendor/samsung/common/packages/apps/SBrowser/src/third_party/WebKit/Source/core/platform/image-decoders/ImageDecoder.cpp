...
#if defined(SBROWSER_WBMP_SUPPORT)
bool read_mbf(char* contnets, unsigned* value,unsigned* bytes_read)
{
    char* temp = contnets;
    unsigned n=0;
    unsigned i=0;
...
bool matchesWBMPSignature(char* contents,unsigned* w, unsigned* h)
{
   	char* temp = contents;
	
    unsigned width =0;
	unsigned height =0;
	unsigned num_bytes_read_w =0;
	unsigned num_bytes_read_h =0;
	...
	if(temp[1] & 0x9F)
        ...
        if (!read_mbf((temp+2), &width,&num_bytes_read_w) || width > 0xFFFF) {
            return false;
        }
        if (!read_mbf((temp+2+num_bytes_read_w), &height,&num_bytes_read_h) || height > 0xFFFF) {
            return false;
        }
		*w= width;
		*h = height;
...
{
#if defined(SBROWSER_WBMP_SUPPORT)
    static const unsigned lengthOfWBMPLongestSignature = 12; 
    char WBMPcontents[lengthOfWBMPLongestSignature];
    unsigned WBMPlength = copyFromSharedBuffer(WBMPcontents, lengthOfWBMPLongestSignature, data, 0);
    if (WBMPlength < lengthOfWBMPLongestSignature)
        return nullptr;
        unsigned   width = 0;
        unsigned   height = 0;
     if (matchesWBMPSignature(WBMPcontents,&width,&height))
     {
       if(width == 0 || height == 0)
            return nullptr;
        OwnPtr<WBMPImageDecoder> wbmp_Image_decoder = adoptPtr(new WBMPImageDecoder(alphaOption, gammaAndColorProfileOption));
        wbmp_Image_decoder->setSize(width,height);
        return wbmp_Image_decoder.release();
     }
#endif
