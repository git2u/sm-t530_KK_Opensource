...
//#define MASK_FILTER_IMAGE_CACHE 1  //SRUK DISABLE, DUE TO NEW SELECTIVE AA PATCH

#ifdef MASK_FILTER_IMAGE_CACHE
#include "base/logging.h"
#include <unordered_map>

typedef std::tr1::unordered_map <unsigned, unsigned> MaskImageCache;
MaskImageCache g_MaskImageCache;
#endif //MASK_FILTER_IMAGE_CACHE
...
unsigned computeCrc(const unsigned char *buffer, int size) {
...
#ifdef MASK_FILTER_IMAGE_CACHE
	int size = sizeof(uint8_t)*desc.fHeight* dstM.fRowBytes;
	unsigned crc =  computeCrc(dstM.fImage,size);	
	
	MaskImageCache::iterator it = g_MaskImageCache.find(crc);
	
 	if (it == g_MaskImageCache.end()) {			
		g_MaskImageCache.insert(std::pair<unsigned,unsigned>(crc, 0));	
