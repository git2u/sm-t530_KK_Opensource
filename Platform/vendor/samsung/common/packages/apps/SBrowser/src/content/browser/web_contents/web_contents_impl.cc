...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
   MSC_METHOD_NORMAL("UI", "WC", "Create", "");
#endif
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
    MSC_METHOD_NORMAL("UI", "WC", "Create", "Guest");
#endif
...
void WebContentsImpl::Init(const WebContents::CreateParams& params) {
  ...
  if( java_bridge_dispatcher_host_manager_->GetRenderViewStatus() ) {
	  //LOG(INFO) << "WebContentsImpl::WebContents is renewal but render_view is not created.";
	  //LOG(INFO) << "WebContentsImpl::So call RenderViewCreated forcibly";
	  java_bridge_dispatcher_host_manager_->RenderViewCreated(render_manager_.current_host());
  }
#endif
...
void WebContentsImpl::CreateNewWindow(
  ...
  if (delegate_) {
    ...
    delegate_->WebContentsCreatedEx( this, 
...
bool WebContentsImpl::IsWMLPage(){
     return contents_mime_type_ == "text/vnd.wap.wml";
}
#endif
...
#if defined (SBROWSER_SAVEPAGE_IMPL)
 void WebContentsImpl::ScrapPageSavedAs(const base::FilePath::StringType& pure_file_name){
  LOG(INFO)<<"SCRAP :: WebContentsImpl::ScrapPageSavedAs";
  if (GetRenderViewHost())
    GetRenderViewHostImpl()->ScrapPageSavedAs(pure_file_name);
  }
#endif
...
void WebContentsImpl::DidNavigateMainFramePostCommit(
  ...
  if (details.is_navigation_to_different_page()) {
    ...
    // page.
    LOG(INFO) << "URLUpdate WebContentsImpl::DidNavigateMainFramePostCommit";
...
void WebContentsImpl::RenderViewTerminated(RenderViewHost* rvh,
                                           ...
                                           int error_code) {
LOG(INFO)  << "TouchLockup WebContentsImpl::RenderViewTerminated";
  if (rvh != GetRenderViewHost()) {
...
void WebContentsImpl::UpdateTargetURL(int32 page_id, const GURL& url) {
	...
	LOG(INFO) << "URLUpdate WebContentsImpl::UpdateTargetURL url is called";
#else
	LOG(INFO) << "URLUpdate WebContentsImpl::UpdateTargetURL url = " << url.spec();
#endif
  if (delegate_)
