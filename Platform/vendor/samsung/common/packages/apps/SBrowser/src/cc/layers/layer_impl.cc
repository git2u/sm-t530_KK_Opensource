...
#if defined(SBROWSER_HIDE_URLBAR)
void LayerImpl::DumpLayerPropertiesEx(std::string* str, int indent) {
  std::string indent_str = IndentString(indent);
  str->append(indent_str);
  base::StringAppendF(str, "layer ID: %d\n", layer_id_);
  ...
  str->append(indent_str);
  base::StringAppendF(str, "opacity: %f\n", draw_properties_.opacity);
  ...
  str->append(indent_str);
  const gfx::Transform& scr_transform = draw_properties_.screen_space_transform;
  base::StringAppendF(str,
                      "screenSpaceTransform: %f, %f, %f, %f  //  %f, %f, %f, %f"
                      "  //  %f, %f, %f, %f  //  %f, %f, %f, %f\n",
                      scr_transform.matrix().getDouble(0, 0),
                      scr_transform.matrix().getDouble(0, 1),
                      scr_transform.matrix().getDouble(0, 2),
                      scr_transform.matrix().getDouble(0, 3),
                      scr_transform.matrix().getDouble(1, 0),
                      scr_transform.matrix().getDouble(1, 1),
                      scr_transform.matrix().getDouble(1, 2),
                      scr_transform.matrix().getDouble(1, 3),
                      scr_transform.matrix().getDouble(2, 0),
                      scr_transform.matrix().getDouble(2, 1),
                      scr_transform.matrix().getDouble(2, 2),
                      scr_transform.matrix().getDouble(2, 3),
                      scr_transform.matrix().getDouble(3, 0),
                      scr_transform.matrix().getDouble(3, 1),
                      scr_transform.matrix().getDouble(3, 2),
                      scr_transform.matrix().getDouble(3, 3));
  ...
  str->append(indent_str);
  base::StringAppendF(
      str, "is_clipped: %s\n", draw_properties_.is_clipped ? "yes" : "no");

  str->append(indent_str);
  gfx::Rect temp_rect = draw_properties_.clip_rect;
  base::StringAppendF(
      str, "clip_rect: %d, %d, %d, %d\n", temp_rect.x(), temp_rect.y(), temp_rect.width(), temp_rect.height());

  str->append(indent_str);
  base::StringAppendF(
      str, "content_bounds: %d, %d\n", draw_properties_.content_bounds.width(), draw_properties_.content_bounds.height());

  str->append(indent_str);
  base::StringAppendF(
      str, "contents_scale_x: %f, contents_scale_y: %f\n", draw_properties_.contents_scale_x, draw_properties_.contents_scale_y);

  str->append(indent_str);
  temp_rect = draw_properties_.visible_content_rect;
  base::StringAppendF(
      str, "visible_content_rect: %d, %d, %d, %d\n", temp_rect.x(), temp_rect.y(), temp_rect.width(), temp_rect.height());

  str->append(indent_str);
  temp_rect = draw_properties_.drawable_content_rect;
  base::StringAppendF(
      str, "drawable_content_rect: %d, %d, %d, %d\n", temp_rect.x(), temp_rect.y(), temp_rect.width(), temp_rect.height());

  str->append(indent_str);
  base::StringAppendF(
      str, "num_descendants_that_draw_content: %d \n", draw_properties_.num_descendants_that_draw_content);

  str->append(indent_str);
  base::StringAppendF(
      str, "descendants_can_clip_selves: %s \n", draw_properties_.descendants_can_clip_selves ? "yes" : "no" );

  str->append(indent_str);
  base::StringAppendF(
      str, "force_render_surface: %s \n", force_render_surface_ ? "yes" : "no" );

  str->append(indent_str);
  base::StringAppendF(
      str, "anchor_point: %f, %f\n", anchor_point_.x(), anchor_point_.y());

}

void LayerImpl::DumpLayerEx(std::string* str, int indent) {
  str->append(IndentString(indent));
  base::StringAppendF(str, "%s(%s)\n", LayerTypeAsString(), debug_name_.data());
  DumpLayerPropertiesEx(str, indent+2);
  if (replica_layer_) {
    str->append(IndentString(indent+2));
    str->append("Replica:\n");
    replica_layer_->DumpLayerEx(str, indent+3);
  }
  if (mask_layer_) {
    str->append(IndentString(indent+2));
    str->append("Mask:\n");
    mask_layer_->DumpLayerEx(str, indent+3);
  }
  for (size_t i = 0; i < children_.size(); ++i)
    children_[i]->DumpLayerEx(str, indent+1);
}
#endif
...
void LayerImpl::UpdateScrollbarPositions() {
  ...
  //It sometimes causes scrollbar animation to restart.
  //After scroll gesture, Scrollbar blinking issue may happens. 
  //We need to check whether this part is necessary or not.
...
CreateScrollbarAnimationControllerWithFade(LayerImpl* layer) {
  ...
  base::TimeDelta fadeout_length = base::TimeDelta::FromMilliseconds(1);
...
#if defined(SBROWSER_HIDE_URLBAR)
void LayerImpl::SetWebRoot(bool root) {
  web_root_ = root;
  if(root){
    layer_tree_impl_->set_web_root_layer(this);
  }
}
#endif
