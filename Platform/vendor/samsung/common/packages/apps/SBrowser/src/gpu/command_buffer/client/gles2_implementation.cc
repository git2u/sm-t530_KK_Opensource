...
#if 0 //#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  rlimit limit;
...
void GLES2Implementation::ShallowFlushCHROMIUM() {
  ...
  if (command_line.HasSwitch(switches::kDisableShallowFlush)) {
  } else {
    GPU_CLIENT_SINGLE_THREAD_CHECK();
...
#if defined(SBROWSER_NATIVE_TEXTURE_UPLOAD)
  // JKK: see if caller in resource_provider set GLenum border != 0, indicating that we have
  // GraphicBuffer -allocated texture already. If so then simply call server side

  if (border != 0) {
 
  	  ScopedTransferBufferPtr buffer(border, helper_, transfer_buffer_);
  	    if (!buffer.valid()) {
  	      return;
  	    }

  	  TextureUnit& unit = texture_units_[active_texture_unit_];
  	  GLenum texture_num = unit.bound_texture_2d;

  	  int32* fd_ptr = (int32*)((uint8_t*)pixels + border - 4); //last int contains the fd count
  	  int32 fd_count = *fd_ptr;

  	  fd_ptr -= fd_count; //move pointer to the beginning of the fd array

  	  helper_->SendFileDescriptors((uint32)texture_num, fd_count, fd_ptr);


  	  border -= (fd_count + 1) * 4; //do not transfer the fd data in buffer

   	  memcpy(buffer.address(), pixels, border);
  	    helper_->TexImage2D(
