...
void FeatureInfo::AddFeatures(const CommandLine& command_line) {
  ...
  VLOG(INFO) << "GL_VENDOR " << gl_strings[0];
  VLOG(INFO) << "GL_RENDERER " << gl_strings[1];
  if (!command_line.HasSwitch(switches::kGpuDriverBugWorkarounds) &&
    ...
    if (is_qualcomm) {
      ...
      workarounds_.flush_on_context_switch = false;
  ...
  if (extensions.Contains("GL_EXT_multisampled_render_to_texture"))
  {
    ...
    feature_flags_.multisample_render_to_texture = true;
    validators_.g_l_state.AddValue(GL_MAX_SAMPLES_EXT);
  }
  if (extensions.Contains("GL_IMG_multisampled_render_to_texture"))
  {
    AddExtensionString("GL_IMG_multisampled_render_to_texture");
    feature_flags_.multisample_render_to_texture = true;
    validators_.frame_buffer_target.AddValue(GL_FRAMEBUFFER);
    //validators_.g_l_state.AddValue(GL_MAX_SAMPLES_IMG);
    validators_.g_l_state.AddValue(0x9135);
  }
