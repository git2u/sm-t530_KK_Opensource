...
class BrowsingDataCookieHelper
 ...
 public:
  ...
  // Observer is notified when cookie information is retrieved
  class Observer {
   public:
    virtual void OnGetCurrentCookieCount(int cookieCount){}
    virtual void OnGetCookiesForUrl(const std::string cookies) {}
   protected:
    virtual ~Observer() {}
  };
...
#if defined (SBROWSER_CHROME_NATIVE_PREFERENCES)
  virtual void StartFetching(
      const base::Callback<void(const net::CookieList& cookies)>& callback, std::string url);
  ...
  #if defined (SBROWSER_CHROME_NATIVE_PREFERENCES)
  void  GetCurrentCookieCount(); 
  void  OnGetCurrentCookieCount(const net::CookieList& cookies);
  void GetCookiesForUrl(std::string curl);
  void OnGetCookiesForUrl(const net::CookieList& cookies);
...
#if defined (SBROWSER_CHROME_NATIVE_PREFERENCES)
 void FetchCookiesOnIOThreadwithUrl(std::string curl);
 void OnFetchCompleteForUrl(const net::CookieList& cookies);
 void NotifyInUIThreadForUrl(const net::CookieList& cookies);
#endif 
  ...
  #if defined (SBROWSER_CHROME_NATIVE_PREFERENCES)
   base::Callback<void(const net::CookieList& cookies)> cookie_completion_callback_;
  #endif // #if defined (SBROWSER_CHROME_NATIVE_PREFERENCES)
