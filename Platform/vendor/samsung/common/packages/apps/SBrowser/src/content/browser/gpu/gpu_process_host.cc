...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
    IPC_MESSAGE_HANDLER(GpuHostMsg_MMapFailed,
                        OnMMapFailed)
#endif
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
void GpuProcessHost::OnMMapFailed() {
  TRACE_EVENT0("gpu", "GpuProcessHost::OnMMapFailed::IO");

  BrowserThread::PostTask(
    BrowserThread::UI,
    FROM_HERE,
    base::Bind(&GpuProcessHost::OnBroadcastMMapFailed,
                    this));
}

void GpuProcessHost::OnBroadcastMMapFailed(const GpuProcessHost* gpu_host) {
  TRACE_EVENT0("gpu", "GpuProcessHost::OnBroadcastMMapFailed::UI");
