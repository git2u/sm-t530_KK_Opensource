...
class WorkerPool::Inner : public base::DelegateSimpleThread::Delegate {
 public:
  ...
  void SetThreadPriority(const std::string& thread_name_prefix, int nice_value);
#endif
 private:
...
#if defined(SBROWSER_ADJUST_WORKER_THREAD_PRIORITY)
void WorkerPool::Inner::SetThreadPriority(const std::string& thread_name_prefix, int nice_value) {
    int length = thread_name_prefix.length();        
    for(ScopedPtrDeque<base::DelegateSimpleThread>::const_iterator it = workers_.begin(); it != workers_.end(); it++) {        
        if((*it)->name_prefix().compare(0, length-1, thread_name_prefix, 0, length-1) == 0) {
            TRACE_EVENT2("cc","WorkerPool::Inner::SetThreadPriority adjust!", "name", (*it)->name_prefix().c_str(), "nice_value", nice_value);            
            setpriority(PRIO_PROCESS, (*it)->tid(), nice_value);
        }
    }
}
#endif
...
#if defined(SBROWSER_ADJUST_WORKER_THREAD_PRIORITY)
void WorkerPool::SetThreadPriority(const std::string& thread_name_prefix, int nice_value) {
    inner_->SetThreadPriority(thread_name_prefix, nice_value);
}
#endif
}  // namespace cc
