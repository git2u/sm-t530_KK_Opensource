...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
    case chrome::NOTIFICATION_HISTORY_SEARCH_TERMS_RESULTS_UPDATED:
      OnSearchTermsResultsUpdated(*content::Details<history::KeywordSearchTermDetails>(details).ptr());
      break;
#endif
    default:
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
void InMemoryHistoryBackend::OnSearchTermsResultsUpdated(
	const KeywordSearchTermDetails& details){
	URLRow url_row;
	LOG(INFO)<<"In memory db";
	LOG(INFO)<<"Result URL: "<<details.url;
	LOG(INFO)<<"Search terms: "<<details.term;

	if(db_->GetRowForURL(details.url, &url_row)){
		URLID url_id = url_row.id();
		LOG(INFO)<<"id of result url "<<url_id;
		// KeywordSearchTermRow search_term_row;
		//db_->GetKeywordSearchTermRow(url_id, search_term_row);
		db_->SetSearchTermsForURL(url_id, details.term);
	}
	else{
		LOG(INFO)<<"In memory db does not have this data";
		URLRow new_row(details.url);
		new_row.set_last_visit(base::Time::Now());
		URLID id = db_->AddURL(new_row);
		db_->SetSearchTermsForURL(id, details.term);
	}
}
#endif
