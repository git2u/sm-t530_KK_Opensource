...
class SSLContext {
 public:
  ...
  SSL_CTX* ssl_ctx() {
    if (ssl_cac_ctx_.get()) {
      LOG(INFO) << "ssl_cac_ctx_ used";
      return ssl_cac_ctx_.get();
    }
    LOG(INFO) << "ssl_ctx_ used";
    return ssl_ctx_.get();
  }
#endif
  // SAMSUNG_CHANGE : smartcard_certificate <<<
...
#if defined(SBROWSER_CAC)
  // SAMSUNG_CHANGE : smartcard_certificate >>>
  void ensureToUseCACEngine(const char* certificateAlias) {
    LOG (INFO) << "[CAC] SSLContext::ensureToUseCACEngine() START";
    if (ssl_cac_ctx_.get()) {
      LOG (INFO) << "[CAC] SSLContext::ensureToUseCACEngine() already registered";
      return;
    }

    int rv = registerEngine();
    if (rv != ENG_OK) {
      LOG(ERROR) << "[CAC] SSLContext::fail to registerEngine() - " << rv;
      return;
    }

    SSL_CTX* ctx;
    if (NULL != certificateAlias) {
      rv = setupCredentialsWithAlias(&ctx, certificateAlias);	
    } else {
      rv = setupCredentials(&ctx);
    }
    if (rv != ENG_OK) {
      LOG(ERROR) << "[CAC] SSLContext::fail to setupCredentials() - " << rv;
      deregisterEngine();
      return;
    }
    ...
    LOG (INFO) << "[CAC] SSLContext::ensureToUseCACEngine() start to use CAC context";
  }

  void deregisterCACEngine() {
    LOG (INFO) << "[CAC] SSLContext::deregisterCACEngine() START";
    if (ssl_cac_ctx_.get() == NULL) {
      LOG (INFO) << "[CAC] SSLContext::deregisterCACEngine() already deregistered";
      return;
    }

    ssl_cac_ctx_.reset(NULL);
    deregisterEngine();
    LOG (INFO) << "[CAC] SSLContext::deregisterCACEngine() END";
  }
  // SAMSUNG_CHANGE : smartcard_certificate <<<
#endif
 private:
...
SSLSessionCacheOpenSSL::Config
    SSLContext::kDefaultSessionCacheConfig = {
...
#if defined(SBROWSER_CAC)
// SAMSUNG_CHANGE : smartcard_certificate >>>
void SSLClientSocketOpenSSL::ensureToUseCACEngine(const char* certificateAlias) {
  SSLContext::GetInstance()->ensureToUseCACEngine(certificateAlias);
}
...
int SSLClientSocketOpenSSL::DoVerifyCert(int result) {
  LOG(INFO) << "SSLClientSocketOpenSSL::DoVerifyCert()";
  DCHECK(server_cert_);
