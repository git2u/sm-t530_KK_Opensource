...
std::string SysInfo::GetAndroidBuildPDA() {
  char os_build_pda_str[PROP_VALUE_MAX];
...
std::string SysInfo::GetAndroidCountryISO() {
  char os_build_CountryISO_str[PROP_VALUE_MAX];
  __system_property_get("ro.csc.countryiso_code", os_build_CountryISO_str);
  return std::string(os_build_CountryISO_str);
}
#endif
...
#if defined(SBROWSER_SYSINFO_GETANDROIDLANGUAGE)
std::string SysInfo::GetAndroidLanguage() {
	char language_info_str[PROP_VALUE_MAX];
	__system_property_get("persist.sys.language", language_info_str);
	return std::string(language_info_str);
}
#endif
