...
class AudioOutputDevice::AudioThreadCallback
 ...
 public:
  ...
  void update_tick_count(unsigned int input){wait_tick_count = input;}
  void update_delay_count(unsigned int input){wait_delay_milliseconds = input;}
#endif
...
#if defined(SBROWSER_WEBAUDIO_GLITCH_FIX)
  unsigned int wait_tick_count;
  unsigned int wait_delay_milliseconds;
#endif
...
#if defined(SBROWSER_WEBAUDIO_GLITCH_FIX)
  update_tick_count(0);// added for sbrowser webaudio
  update_delay_count(0);// added for sbrowser webaudio
#endif
}
...
void AudioOutputDevice::AudioThreadCallback::MapSharedMemory() {
  ...
  // added 4 extra bytes for sbrowser: kaustubh
#if defined(SBROWSER_WEBAUDIO_GLITCH_FIX)
  CHECK(shared_memory_.Map(TotalSharedMemorySizeInBytes(memory_length_ + sizeof(uint32))));
#else
...
void AudioOutputDevice::AudioThreadCallback::Process(int pending_data) {
  ...
  //SetActualFlagInBegining of buffer
#if defined(SBROWSER_WEBAUDIO_GLITCH_FIX)
  int shared_memory_size = num_frames * sizeof(*output_bus_->channel(0)) * output_bus_->channels();
  int lock_index = shared_memory_size + sizeof(*output_bus_->channel(0));
  uint32 local_tick_count = 0;
  uint32 local_delay_milliseconds = 0;
  ...
  if(ptr_shared_flag[lock_index] == 2){
      // wait loop
      while(ptr_shared_flag[lock_index] == 2){
          update_tick_count(local_tick_count++);
      }
      local_delay_milliseconds += local_tick_count * (pending_data / bytes_per_ms_);
      update_tick_count(local_delay_milliseconds);
  }else{
      // set the lock
      ptr_shared_flag[lock_index] = 1;
  }
#endif
...
#if defined(SBROWSER_WEBAUDIO_GLITCH_FIX)
  // Renderer usage done so release lock
  ptr_shared_flag[lock_index] = 0; //8196th byte
  // Renderer has filled the data filled
  ptr_shared_flag[lock_index+1] = 1; //8197th byte
  // wait loop
  while(ptr_shared_flag[lock_index] == 2){
      wait_tick_count++;
  }
  wait_delay_milliseconds += wait_tick_count * (pending_data / bytes_per_ms_);
#endif
}
