...
bool GpuCommandBufferStub::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(GpuCommandBufferStub, message)
    ...
    IPC_MESSAGE_HANDLER_GENERIC(GpuCommandBufferMsg_SendFileDescriptors,
		{
			DCHECK(message.HasFileDescriptors());
			PickleIterator iter(message);
			uint32 texid;
			message.ReadUInt32(&iter, &texid);


			const int maxfd = 5; //FileDescriptorSet::kMaxDescriptorsPerMessage;
			// Certain to be out of fd range
			// to replace zero descriptors to avoid duplicate releasing
			const int zeroFD = INT_MAX; 
			base::FileDescriptor descriptor;
			int fd_array[maxfd];
			uint32 zero_count;

			// read zero FD count
			message.ReadUInt32(&iter, &zero_count);

			// mark zero FDs first
			std::fill_n(fd_array, maxfd, -1);
			for (int i=0; i < (int)zero_count; i++)
			{
				uint32 zero_index;
				message.ReadUInt32(&iter, &zero_index);
				fd_array[zero_index] = zeroFD;
			}

			// read nonzero FDs and mark on correct positions in array
			int i = 0;
			while(i < maxfd && message.ReadFileDescriptor(&iter, &descriptor))
			{
				// jump over ready marked zero FDs
				while (fd_array[i] == zeroFD)
				{
					i++;
					DCHECK(i < maxfd);
				}

				fd_array[i++] = descriptor.fd;
			}
			// jump over possible trailing zero FDs to get correct FD count on i
			while (i < maxfd && fd_array[i] == zeroFD) { i++; }

			decoder_->SetFileDescriptors(texid, fd_array, i);
...
bool GpuCommandBufferStub::MakeCurrent() {
  ...
  if(single_compositor && surface_ && (!surface_->HaveContext()))
  {
	  //Restoring the relationship between Context and new egl window
	  LOG(INFO)<<"Single_Compositor:Restoring the context";
  ...
  #endif
  if (decoder_ && decoder_->MakeCurrent())
    return true;
...
void GpuCommandBufferStub::Destroy() {
  ...
  {
	  //Restoring the relationship between Context and new egl window
	  LOG(INFO)<<__FUNCTION__<<"Single_Compositor:gl_virtual setting the context in Destroy"<<surface_;
	  scoped_refptr<gfx::GLContext> context;
  	    context = channel_->share_group()->GetSharedContext();
	  if(context)
		context->MakeCurrent(surface_);
...
void GpuCommandBufferStub::OnInitialize(
...
#endif
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
    MSC_CALLBACK("RENDERER","GPU",__FUNCTION__,"OnScreen");
#endif
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  	MSC_CALLBACK("RENDERER","GPU",__FUNCTION__,"OffScreen");
#endif
  ...
  if ((CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    context = channel_->share_group()->GetSharedContext();
	#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
	MSC_METHOD_NORMAL("GPU","GPU","Shared Context ",context);
	#endif
    if (!context) {
...
void GpuCommandBufferStub::OnSetSurfaceVisible(bool visible) {
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
	MSC_CALLBACK("RENDERER","GPU",__FUNCTION__,visible);
#endif
  #if defined(SBROWSER_GRAPHICS_UI_COMPOSITOR_REMOVAL)
  if(single_compositor && visible)
  {
	  //Restoring the relationship between Context and new egl window
	  scoped_refptr<gfx::GLContext> context;
  	    context = channel_->share_group()->GetSharedContext();
  ...
  LOG(INFO)<<" Single_Compositor: Surface Visibility "<<surface_<<","<<visible;
  if(single_compositor && surface_)
  	surface_->SetVisibility(visible);
  #endif
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_CALLBACK("RENDERER","GPU",__FUNCTION__,"");
#endif
  if (!surface_)
    return;
...
#if defined(SBROWSER_GRAPHICS_GETBITMAP)
static void FlipImageVertically(int width, int height,void* buffer, int size)
{
    TRACE_EVENT0("gpu", "GpuCommandBufferStub::FlipImageVertically");
  uint8* scanline =  new unsigned char[width * size];
  uint8* framebuffer = (uint8*)buffer;
  if (!scanline)
    return;
  unsigned int row_bytes = width * size;
  ...
  for (unsigned int i = 0; i < count; i++) {
    ...
    delete []scanline;
   VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"Image Flipped Vertically";
}

static void getImageTypeFormat(int imageFormat, GLenum* type, GLenum* format)
{
    switch (imageFormat) {
        case FormatAlpha8:
            *format = GL_ALPHA;
...
static int getSize(int imageFormat)
{
    int size = 4;
    switch (imageFormat) {
        case FormatAlpha8:
        ...
        case FormatRGB565: // Fall Through
        case FormatARGB4444:
            size = 2;
            break;
...
int GpuCommandBufferStub::GetNativeBitmap(BitmapParams imageParams, void** buffer)
{
    TRACE_EVENT0("gpu", "GpuCommandBufferStub::GetNativeBitmap");
    GLenum error = GL_NO_ERROR;
    DCHECK(scheduler_.get());
    if (surface_ && surface_->IsOffscreen()) {
      VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"surface is offscreen";
      return -1;
    }

    if(decoder_ && MakeCurrent())
	VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"MakeCurrent is successfull";
    else
    {
	VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"MakeCurrent failed"<<"decoder "<<decoder_;
	return -1;
    }

    VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"x="<<imageParams.xPos<<"y="<<imageParams.yPos<<"width ="<<imageParams.width<<"height="<<imageParams.height;
    if(glGetError() == GL_NO_ERROR)
         VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"decoder makecurrent Succesfull NOERROR"<<buffer;
    else
        VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"decoder makecurrent FAILED **************"<<glGetError();
    glPixelStorei(GL_PACK_ALIGNMENT, 1);
    GLenum format;
    GLenum type;
    getImageTypeFormat(imageParams.imageFormat, &type, &format);
    VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"type ="<<type<<"format="<<format;
    TRACE_EVENT0("gpu", "GpuCommandBufferStub::glReadPixels");
    glReadPixels(imageParams.xPos, imageParams.yPos, imageParams.width, imageParams.height, format, type, *buffer);
    if((error = glGetError()) == GL_NO_ERROR) {
        FlipImageVertically(imageParams.width, imageParams.height,*buffer, getSize(imageParams.imageFormat));
        VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"glReadPixels Succesfull NOERROR"<<buffer;
    } else
        VLOG(0)<<"GetBitmap"<<__FUNCTION__<<"glReadPixels FAILED **************"<<error;
