...
ScopedSafeSharedMemory::create(SafeSharedMemoryPool* pool, base::SharedMemory* shared_memory, size_t shm_size) {
    if (pool)
        return pool->AcquireScopedSafeSharedMemory(shared_memory, shm_size);
    return NULL;
}


ScopedSafeSharedMemory::ScopedSafeSharedMemory(const base::SharedMemoryHandle& handle) 
 : safe_shared_memory_(NULL),
   original_handle_(handle),
   pool_(NULL) { }


ScopedSafeSharedMemory* 
SafeSharedMemoryPool::AcquireScopedSafeSharedMemory(base::SharedMemory* shared_memory, size_t shm_size) {
  ...
  if (it == memory_.end()) {
    ...
    if (!tracker.safe_shared_memory) {
        LOG(ERROR) << "[SafeSharedMemoryPool][AcquireScopedSafeSharedMemory] tracker.safe_shared_memory = NULL";
        return NULL;
    }
    ...
    // for newly created shared memory
    scoped_ptr<ScopedSafeSharedMemory> shm(new ScopedSafeSharedMemory(shared_memory->handle()));
    shm->pool_ = this;
    shm->safe_shared_memory_ = tracker.safe_shared_memory;
  ...
  scoped_ptr<ScopedSafeSharedMemory> shm(new ScopedSafeSharedMemory(shared_memory->handle()));
  shm->pool_ = this;
  shm->safe_shared_memory_ =  it->second.safe_shared_memory;
