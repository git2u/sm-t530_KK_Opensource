...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  // Sets search term used to navigate to this url id
  bool SetSearchTermsForURL(history::URLID, const string16&);
  // Get search term results row (id, terms, lower_terms, url_id) for url_id
  bool GetSearchTermResultsRow(URLID url_id, KeywordSearchTermRow* row);
  // Get search term results rows for particular term
  bool GetSearchTermResultsRows(const string16& term,
                                 std::vector<KeywordSearchTermRow>* rows);
  // Get URLs for this prefix search.
  void GetSearchTermsResults(
  		const string16& prefix,
  		std::vector<SearchProvider::NavigationResultForUrlDb>* matches);
  // Delete search term, unused for now
  bool DeleteSearchTermResults(const string16& term);
  // Create search terms to results table
  bool InitSearchTermsResultsTable();
  // Drop search term result table, unused for now.
  bool DropSearchTermsResultsTable();
#endif
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  //TODO(anish.p): this is unused for now. Used in deleting a history entry
  bool has_search_terms_results_;
#endif
