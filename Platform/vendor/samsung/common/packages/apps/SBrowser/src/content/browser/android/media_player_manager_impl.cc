...
bool MediaPlayerManagerImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP(MediaPlayerManagerImpl, msg)
    ...
    IPC_MESSAGE_HANDLER(MediaPlayerHostMsg_MediaPlayerSetVolume, OnSetVolume)
#endif
...
#if defined(SBROWSER_MEDIAPLAYER_SETVOLUME_IMPL)
void MediaPlayerManagerImpl::OnSetVolume(int player_id, float volume) {
  MediaPlayerBridge* player = GetPlayer(player_id);
  if (player)
...
#if defined(SBROWSER_MEDIAPLAYER_MULTIINSTANCE_DRAG_N_DROP_SUPPORT)  
  if (fullscreen_player_id_ != -1) {
    video_view_.DestroyContentVideoView();
    fullscreen_player_id_ = -1;
  }
...
void MediaPlayerManagerImpl::OnError(int player_id, int error) {
  LOG(INFO) << "[HTML5Media]CB_OnError. error:"  << error;
