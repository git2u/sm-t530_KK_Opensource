...
#ifdef GR_TRIANGULATE_PATH_RENDERER

static bool findIntersectPoint(const SkVector& oldNormal,
        const SkPoint& oldPoint,
        const SkVector& newNormal,
        const SkPoint& newPoint,
        SkPoint& intersectPoint)
{


    float a1 = - (oldNormal.x() / oldNormal.y() );
    float b1 = oldPoint.y() - a1 * oldPoint.x();
    float a2 = - (newNormal.x() / newNormal.y() );
    float b2 = newPoint.y() - a2 * newPoint.x();



    if (fabs(oldNormal.y()) < 10E-5 && fabs(newNormal.y()) < 10E-5) {
        return false;
    }
    if (fabs(oldNormal.y()) < 10E-5)  {
        intersectPoint.set(oldPoint.x(), a2 * oldPoint.x() + b2);
        return true;
    }
    if (fabs(newNormal.y()) < 10E-5)  {
        intersectPoint.set(newPoint.x(), a1 * newPoint.x() + b1);
        return true;
    }
    ...
    float intx = - (b2 - b1) / (a2 - a1);
    if (isnan(intx)) {
        return false;
    }
    intersectPoint.set(intx, a2 * intx + b2);
    return true;

}

bool SkStrokerPriv::correctInnerPoint(SkPoint& lastPoint, const SkPoint& pivot,
        const SkVector& before, const SkVector& after) {
    SkPoint    thisOffsetPoint;
    thisOffsetPoint.set(pivot.fX - after.fX, pivot.fY - after.fY);
    lastPoint.set(pivot.fX - before.fX, pivot.fY - before.fY);
    SkPoint intPoint;
    if (findIntersectPoint(before, lastPoint, after, thisOffsetPoint,    intPoint)) {
        lastPoint = intPoint;
        return true;
    } //else ignore
    return false;
...
static void HandleInnerJoin(SkPath* inner, const SkPoint& pivot, const SkVector& before, const SkVector& after)
{
    SkPoint lastPoint;
    if ( //inner->getLastPt(&lastPoint) &&
            SkStrokerPriv::correctInnerPoint(lastPoint, pivot, before, after) ) {
            inner->setLastPt(lastPoint);
    } //else ignore
}
                        ...
                        SkScalar radius, SkScalar invMiterLimit, bool, bool)
{
    ...
    SkVector    before;
    afterUnitNormal.scale(radius, &before);
    ...
    HandleInnerJoin(inner, pivot, before, after);
}
