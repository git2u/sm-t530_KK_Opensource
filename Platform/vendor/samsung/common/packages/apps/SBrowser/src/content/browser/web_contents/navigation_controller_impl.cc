...
void NavigationControllerImpl::ReloadInternal(bool check_for_repost,
  ...
  if (transient_entry_index_ != -1) {
    ...
    if (!active_entry)
      return;
	LOG(INFO) << "URLUpdate NavigationControllerImpl::ReloadInternal transient_entry_index not -1  calling LoadURL url ="<< active_entry->GetURL().spec();
  ...
  if (g_check_for_repost && check_for_repost &&
    ...
    if (site_instance &&
        ...
        LOG(INFO) << "URLUpdate NavigationControllerImpl::ReloadInternal site instance has wrong process for url " << entry->GetURL().spec();
...
void NavigationControllerImpl::GoBack() {
  ...
  LOG(INFO)<<"NavigationControllerImpl::GoBack() called"  ;
#else
  LOG(INFO)<<"NavigationControllerImpl::GoBack() url" ;
#endif  
  NavigateToPendingEntry(NO_RELOAD);
}
...
void NavigationControllerImpl::LoadURL(
    ...
    PageTransition transition,
    const std::string& extra_headers,
    int ua_override_option) {
  LoadURLParams params(url);
  params.referrer = referrer;
  params.transition_type = transition;
  params.extra_headers = extra_headers;
  if(ua_override_option == UA_OVERRIDE_INHERIT){
