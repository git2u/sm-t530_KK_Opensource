...
class GPU_EXPORT Texture : public base::RefCounted<Texture> {
 public:
  ...
  bool IsDefined() const {
    ...
    void setFileDescriptors(int fd_count, int* fd_array) 
	{
      if (fd_array_) 
	  {
      ...
      fd_count_ = fd_count;
      fd_array_ = new int[fd_count];

	  for (int i=0; i < fd_count; i++)
	  {
		  fd_array_[i] = fd_array[i];
	  }
  }
  ...
  int32 getFileDescriptorAt(int index)
  {
	  if (index >= 0 && index < fd_count_ && fd_array_)
	  {
