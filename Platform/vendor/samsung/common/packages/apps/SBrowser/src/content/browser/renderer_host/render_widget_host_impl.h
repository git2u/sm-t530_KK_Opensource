...
struct ViewHostMsg_HideUrlBarCmd_Params;
...
class CONTENT_EXPORT RenderWidgetHostImpl : virtual public RenderWidgetHost,
 ...
 public:
  ...
  virtual void SetHoverHighLight(
	  const WebKit::WebGestureEvent& hover_highlight__event,bool high_light) OVERRIDE;
#endif
#if defined(SBROWSER_IMAGEDRAG_IMPL)
virtual void GetBitmapForCopyImage(int x,int y) OVERRIDE;
#endif
#if defined(SBROWSER_SBR_NATIVE_CONTENTVIEW)  && defined(SBROWSER_ENTERKEY_LONGPRESS)
  virtual void performLongClickOnFocussedNode(const WebKit::WebGestureEvent& longpress_event) OVERRIDE;
#endif
  ...
  // for closing the webselectpop to go down when no focus is there
#if defined(SBROWSER_FORM_NAVIGATION)
  void OnSelectPopupClose();
...
#if defined(SBROWSER_HIDE_URLBAR)
  void OnHideUrlBarEventNotify(ViewHostMsg_HideUrlBarCmd_Params params);
#endif
