...
#endif
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
 MSC_METHOD("RVHM","RVH","Create RenderWidgetHostImpl","");
#endif
}
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_MESSAGE("RVH","RENDERER","ViewMsg_UpdateScreenRects","");
#endif
...
bool RenderWidgetHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP_EX(RenderWidgetHostImpl, msg, msg_is_ok)
	...
	IPC_MESSAGE_HANDLER(ViewHostMsg_OnCloseSelectPopup,OnSelectPopupClose)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_HideUrlBarCmd,OnHideUrlBarEventNotify)
#endif
#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE)
   IPC_MESSAGE_HANDLER(ViewMsg_FlingAnimationCompleted, OnFlingAnimationCompleted)
#endif   
...
void RenderWidgetHostImpl::WasHidden() {
  ...
  #if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_MESSAGE("RVH","RENDERER","ViewMsg_WasHidden","");
  #endif
...
void RenderWidgetHostImpl::WasShown() {
  ...
  {
  	gfx::AcceleratedWidget    q_window = GpuSurfaceTracker::Get()->GetNativeWidget(surface_id_);
  	if(!(SurfaceViewManager::GetInstance()->HasActiveWindow(q_window)))
  	{
  	  	LOG(INFO)<<"Single_Compositor:scheduled Delayed Shown";
  	 MessageLoop::current()->PostDelayedTask(
          FROM_HERE,
          base::Bind(&RenderWidgetHostImpl::WasShown,
                     weak_factory_.GetWeakPtr()),base::TimeDelta::FromMilliseconds(30));
	 return;
  	}
	 LOG(INFO)<<"Single_Compositor:Executed Delayed Shown";
  }
#endif
  ...
  #if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_MESSAGE("RVH","RENDERER","ViewMsg_WasShown","");
  #endif
...
void RenderWidgetHostImpl::WasResized() {
  ...
  gfx::Size new_physical_backing_size = view_->GetPhysicalBackingSize();
  //physical_backing_size_ = view_->GetPhysicalBackingSize();

  if(new_size.IsEmpty() || new_physical_backing_size.IsEmpty()) {
    LOG(INFO)<<" RenderWidgetHostImpl- WasResized - new_size w=" 
      << new_size.width() << " , h =" << new_size.height() << ", w="
      << new_physical_backing_size.width() << ", h = " << new_physical_backing_size.height();
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_MESSAGE_2("RVH","RENDERER","ViewMsg_Resize",new_size.width(),new_size.height());
#endif
...
void RenderWidgetHostImpl::SetHoverHighLight(const WebKit::WebGestureEvent& hover_highlight_event,bool high_light){
	const WebKit::WebInputEvent&  hover_event = hover_highlight_event;
	Send(new ViewMsg_HoverHighLight(routing_id_, &hover_event, high_light));
}
#endif
#if defined(SBROWSER_IMAGEDRAG_IMPL)
void RenderWidgetHostImpl::GetBitmapForCopyImage(int x,int y){
Send(new ViewMsg_GetBitmapForCopyImage(routing_id_, x,y));
}
#endif
#if defined(SBROWSER_SBR_NATIVE_CONTENTVIEW)  && defined(SBROWSER_ENTERKEY_LONGPRESS)
void RenderWidgetHostImpl::performLongClickOnFocussedNode(const WebKit::WebGestureEvent& longpress_event){
	const WebKit::WebInputEvent&  lp_event = longpress_event;
	Send(new ViewMsg_LongPressOnFocussed(routing_id_, &lp_event));
}
#endif
...
void RenderWidgetHostImpl::ForwardGestureEvent(
  ...
  if (
#if defined(SBROWSER_SKIP_GESTURE_EVENT_FILTER)
    !WebInputEvent::isScrollAndPinchZoomEventType(gesture_event.type) && 
#endif
...
#if defined(SBROWSER_SKIP_GESTURE_EVENT_FILTER)
  if(!WebInputEvent::isScrollAndPinchZoomEventType(input_event.type))
#endif
...
void RenderWidgetHostImpl::ForwardInputEvent(const WebInputEvent& input_event,
                                             int event_size,
                                             bool is_keyboard_shortcut) {
  TRACE_EVENT1("renderer_host", "RenderWidgetHostImpl::ForwardInputEvent", "type", input_event.type);
  if (!process_->HasConnection())
    return;
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_MESSAGE("RENDERER","RVH","RendererExited",status);
#endif
...
void RenderWidgetHostImpl::Destroy() {
  ...
  {
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
     MSC_RETURN("RVH","RWHVA","Destroy",view_);
#endif
    view_->Destroy();
  }
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_CALLBACK("RENDERER","RVH","UpdateScreenRects Done","");
#endif
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_DATA_CALLBACK("RENDERER","RVH",__FUNCTION__,"");
#endif
  if (view_) {
...
#if defined(SBROWSER_GRAPHICS_UI_COMPOSITOR_REMOVAL)
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  bool is_restore_ack = ViewHostMsg_UpdateRect_Flags::is_restore_ack(params.flags);
  if(is_restore_ack)
     MSC_CALLBACK("RENDERER","RVH","Renderer_Restore_ACK","");
#endif
#endif
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
     MSC_CALLBACK("RENDERER","RVH","Resize ACK",current_size_.ToString());
#endif
    resize_ack_pending_ = false;
    in_flight_size_.SetSize(0, 0);
  }
...
void RenderWidgetHostImpl::OnInputEventAck(
  ...
  if (
#if defined(SBROWSER_SKIP_GESTURE_EVENT_FILTER)
    !WebInputEvent::isScrollAndPinchZoomEventType(event_type) && 
#endif
    decrement_in_flight_event_count() == 0)
    StopHangMonitorTimeout();
...
void RenderWidgetHostImpl::OnSelectRangeAck() {
  LOG(INFO)<<"PERF: OnSelectRangeAck";
  select_range_pending_ = false;
  if (next_selection_range_) {
    scoped_ptr<SelectionRange> next(next_selection_range_.Pass());
	LOG(INFO)<<"PERF: OnSelectRangeAck start = "<<next->start.x()<<"  "<<next->start.y();
...
#if defined(SBROWSER_FORM_NAVIGATION)
void RenderWidgetHostImpl::OnSelectPopupClose(){
if(view_)
	view_->selectPopupClose();

}
void RenderWidgetHostImpl::OnSelectPopupCloseZero(){
if(view_)
	view_->selectPopupCloseZero();
}
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_CALLBACK("RENDERER","RVH",__FUNCTION__,activated);
#endif
  if (view_)
...
void RenderWidgetHostImpl::SelectRange(const gfx::Point& start,
                                       const gfx::Point& end) {
LOG(INFO)<<"PERF: SelectRange start = "<<start.x()<<"  "<<start.y();
...
void RenderWidgetHostImpl::SetPasswordEcho(bool pwdEchoEnabled)
{
    Send(new ViewMsg_PasswordEchoEnabled(GetRoutingID(),pwdEchoEnabled));
}
#endif
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_DATA_CALLBACK_NORMAL("RVH","RENDERER",__FUNCTION__,"");
#endif
  if (host)
...
void RenderWidgetHostImpl::GetSelectionContext(const bool shouldUpdateSelectionContext)
{
  LOG(ERROR) << "Sending message to render view impl to get selection context.";
  Send(new ViewMsg_GetSelectionContext(GetRoutingID(), shouldUpdateSelectionContext));
}

void RenderWidgetHostImpl::UpdateSelection(const std::string& oldSelection, const std::string& newSelection)
{
  Send(new ViewMsg_UpdateSelection(GetRoutingID(), oldSelection, newSelection));
}

void RenderWidgetHostImpl::RestoreSelection()
{
  Send(new ViewMsg_RestoreSelection(GetRoutingID()));
}

void RenderWidgetHostImpl::ClearSelection()
{
  Send(new ViewMsg_ClearSelection(GetRoutingID()));
}
#endif
// ---------------------------  kikin Modification ends  ---------------------------
#if defined(SBROWSER_HIDE_URLBAR)
void RenderWidgetHostImpl::OnHideUrlBarEventNotify(ViewHostMsg_HideUrlBarCmd_Params params) {
  if(view_){
    view_->OnHideUrlBarEventNotify(params.cmd_type, params.cmd_data1, params.cmd_data2);
  }
}
#endif

#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE)
void RenderWidgetHostImpl::OnFlingAnimationCompleted(bool page_end){
  if(view_){
  	view_->OnFlingAnimationCompleted(page_end);
  }
}
#endif
}  // namespace content
