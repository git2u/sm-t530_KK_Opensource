...
FrameView::FrameView(Frame* frame)
    ...
    , m_savedCompositingUpdateType(0)
#endif
#if defined (SBROWSER_SOFTBITMAP_IMPL)
    , m_softBitmapValidCapture(true)
#endif
#if defined(SBROWSER_CSS_TOUCHMOVE_EVENT_FIX)
    , m_shouldUpdateTouchEventTargetRects(false)
#endif
{
    init();
...
#if defined(SBROWSER_CSS_ANIMATION_PERFORMANCE_IMPROVEMENT_3)
void FrameView::updateCompositingLayerIfNeeded()
{
	
        
    CompositingUpdateType updateType = static_cast<CompositingUpdateType>(m_savedCompositingUpdateType);
    m_savedCompositingUpdateType = 0;
...
#if defined(SBROWSER_CSS_ANIMATION_PERFORMANCE_IMPROVEMENT_3)
    m_savedCompositingUpdateType |= CompositingUpdateAfterStyleChange;
#else
...
#if defined(SBROWSER_CSS_TOUCHMOVE_EVENT_FIX)
    if(m_shouldUpdateTouchEventTargetRects){    
        if (m_frame && m_frame->page()) {
            if (ScrollingCoordinator* scrollingCoordinator = m_frame->page()->scrollingCoordinator()){
                scrollingCoordinator->touchEventTargetRectsDidChange(m_frame->document());
            }
        }
        m_shouldUpdateTouchEventTargetRects = false;
    }
...
#if defined(SBROWSER_CSS_ANIMATION_PERFORMANCE_IMPROVEMENT_5)
    if(m_frame && m_frame->animation() && m_frame->animation()->isSetDelayLayoutAndUpdateStyle()) {    
        //setNeedsLayout();
        return;
    }
#endif
...
LayoutRect FrameView::viewportConstrainedVisibleContentRect() const
{
    ...
    viewportRect.setHeight(frameRect().height());
#endif
...
#if defined(SBROWSER_PLM_P130706_1198)
...
void FrameView::paintContents(GraphicsContext* p, const IntRect& rect)
{
    ...
    if(document->isWMLDocument()){
           p->fillRect(rect, Color::white, ColorSpaceDeviceRGB);
          return; 
    }
    #endif
