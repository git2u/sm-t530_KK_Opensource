...
GLRenderer::GLRenderer(RendererClient* client,
    ...
    : DirectRenderer(client, resource_provider),
      ...
      skiped_sending_frame_info_(false),
#endif
...
#if defined(SBROWSER_BLOCK_SENDING_FRAME_MESSAGE)
void GLRenderer::SetGestureInProgress(bool gesture) {
    if(gesture_in_progress_ == gesture)
        return;
    gesture_in_progress_ = gesture;
    if(!gesture_in_progress_){
        if (Settings().compositor_frame_message && skiped_sending_frame_info_) {
...
#if defined(SBROWSER_RESIZE_OPTIMIZATION)
            output_surface_->SendFrameUpdateInfo();
#endif
            skiped_sending_frame_info_ = false;
        }
    }
...
void GLRenderer::BeginDrawingFrame(DrawingFrame* frame) {
  ...
  if (is_viewport_changed_) {
	...
	//Rotation of device during scroll/zoom events results in updateframeinfo getting blocked
	//We provide one update message to application which removes the placeholder and 
	//again block updateframe messages in subsequent scroll/zoom events after resize.
	gesture_in_progress_ = false;
  }
...
#if defined(SBROWSER_BLOCK_SENDING_FRAME_MESSAGE)
  if (Settings().compositor_frame_message && !gesture_in_progress_) {
    CompositorFrame compositor_frame;
    compositor_frame.metadata = client_->MakeCompositorFrameMetadata();
    output_surface_->SendFrameToParentCompositor(&compositor_frame);
    skiped_sending_frame_info_ = false;
  } else if(Settings().compositor_frame_message && gesture_in_progress_){
  	skiped_sending_frame_info_ = true;
  }
#else
