...
bool BufferedResourceHandler::SelectNextHandler(bool* defer) {
  ...
  if (!must_download) {
    if (net::IsSupportedMimeType(mime_type)){
	    if( mime_type.compare(0, 6, "audio/") == 0  || mime_type.compare(0, 6, "video/") == 0) {
			media_download = true;
	    }
		else{
            return true;
    ...
    if(!media_download){
#if defined(SBROWSER_PLM_130814_05501)
	  ...
	  if (mime_type == "application/x-shockwave-flash" && !info->IsMainFrame())
	      return true;
#endif
	    scoped_ptr<ResourceHandler> handler(
