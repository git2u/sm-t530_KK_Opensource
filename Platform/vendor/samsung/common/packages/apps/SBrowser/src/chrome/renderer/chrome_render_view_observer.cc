...
#if defined (SBROWSER_SOFTBITMAP_IMPL)
void ChromeRenderViewObserver:: DidChangeLoadProgress(bool paint_full_image){
	 SkBitmap snapshot;
	if(CaptureSoftBitmap(render_view()->GetWebView(), &snapshot, paint_full_image))
	{
		//send bitmap to render host
		Send(new ViewHostMsg_SoftBitmap(routing_id(), snapshot));
	}
}

bool ChromeRenderViewObserver::CaptureSoftBitmap(WebView* view,
                                               SkBitmap* snapshot, bool paint_full_image) {
  if (!view)
     return false;

  view->layout();
  const WebSize& size = view->size();
  float scale = view->deviceScaleFactor();
  int soft_bitmap_height = -1;

  base::StringToInt(CommandLine::ForCurrentProcess()->GetSwitchValueASCII(switches::kSoftBitmapHeight), &soft_bitmap_height);


  if (!view->isSoftwareRenderable()) {
      if (soft_bitmap_height == -1) {
        //assuming that content is having some 3d elements
        soft_bitmap_height = size.height * scale;
      }
  }
  ...
  LOG(INFO)<<"Chandra::ChromeRenderViewObserver::CaptureSoftBitmap onPageFinished Notification";

  skia::RefPtr<SkCanvas> canvas = skia::AdoptRef(
		skia::CreatePlatformCanvas(
		size.width * scale, soft_bitmap_height, true, NULL, skia::RETURN_NULL_ON_FAILURE));
	
  if (!canvas)
	return false;
  (canvas.get())->drawARGB(255, 255, 255, 255);
  if(paint_full_image && view->isSoftwareRenderable()) {
  	   //below api paints using hardware layers not software layer
  	    view->paintSoftBitmap(webkit_glue::ToWebCanvas(canvas.get()),
              WebRect(0, 0, size.width*scale, size.height*scale));
  }
  else {
  	 //below api paints using hardware layers not software layer
        view->paintSoftBitmapRootImage(webkit_glue::ToWebCanvas(canvas.get()),
              WebRect(0, 0, size.width*scale, size.height*scale));
   }
