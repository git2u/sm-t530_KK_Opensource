...
class WebToCCInputHandlerAdapter::HandlerAdapter
 ...
 public:
  ...
  virtual ScrollStatus flingScrollBegin(double timeStampSeconds) {
    return static_cast<WebInputHandlerClient::ScrollStatus>(
        handler_->FlingScrollBegin(timeStampSeconds));
  }
#else
  ...
  virtual void flingAnimationCompleted() {  handler_->FlingAnimationCompleted(); }
#endif
  ...
  virtual void pinchGestureBegin() { handler_->PinchGestureBegin(); }
   ...
   virtual void setCommitBlockOnImplThread(bool blockState)  {
		handler_->setCommitBlockOnImplThread(blockState);
	}
#endif
