...
#if defined (SBROWSER_TOUCH_PERFORMANCE)
,firstGestureUpdateRecieved(false) 
#endif
{
}
...
#if defined (SBROWSER_TOUCH_PERFORMANCE)
      if(scrolling_in_progress_ && gesture_event.type == WebInputEvent::GestureFlingStart && debouncing_deferral_queue_.empty()){
...
bool GestureEventFilter::ShouldForwardForCoalescing(
    const WebGestureEvent& gesture_event) {
#if defined (SBROWSER_TOUCH_PERFORMANCE)
	if(!firstGestureUpdateRecieved && (gesture_event.type == WebInputEvent::GestureScrollUpdate || gesture_event.type == WebInputEvent::GesturePinchUpdate )) {
		firstGestureUpdateRecieved = true;
		return true;
	}
#endif
  switch (gesture_event.type) {
