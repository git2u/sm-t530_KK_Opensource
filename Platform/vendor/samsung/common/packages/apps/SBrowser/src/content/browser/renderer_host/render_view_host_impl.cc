...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_MESSAGE("RVH","RENDERER","ViewMsg_New","");
#endif
...
#if defined(SBROWSER_DISABLE_PRECONNECTION_ON_BACKFORWARD)
//  FOR_EACH_OBSERVER(RenderViewHostObserver, observers_, Navigate(params.url));
  // SAMSUNG_CHANGE : disable_preconnection_on_back/forward
  FOR_EACH_OBSERVER(content::RenderViewHostObserver,
                    observers_, Navigate(params.url, params.transition));
#endif
}
...
void RenderViewHostImpl::moveToNextInput(){
	Send(new ViewMsg_MoveToNext(GetRoutingID()));
}

void RenderViewHostImpl::moveToPrevInput(){
	Send(new ViewMsg_MoveToPrev(GetRoutingID()));
}
#endif
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_MESSAGE("RVH","RENDERER","ViewMsg_SwapOut","");
#endif
  if (IsRenderViewLive()) {
...
void RenderViewHostImpl::OnSwapOutACK(bool timed_out) {
  ...
  has_timed_out_on_unload_ = timed_out;
    #if defined(SBROWSER_GRAPHICS_MSC_TOOL)
	MSC_METHOD_NORMAL("RVH", "WC","SwappedOut","");
#endif
...
bool RenderViewHostImpl::OnMessageReceived(const IPC::Message& msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP_EX(RenderViewHostImpl, msg, msg_is_ok)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_setBitmapForDragging,OnReceiveBitmapForDragging)
#endif
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_SetBingSearchEngineAsDefault, SetBingSearchEngineAsDefault)
    #endif
...
#if defined (SBROWSER_SAVEPAGE_IMPL)
    IPC_MESSAGE_HANDLER(ViewHostMsg_OnBitmapForScrap, OnBitmapForScrap) 
	IPC_MESSAGE_HANDLER(ViewHostMsg_OnGetBitmapFromCachedResource,OnReceiveBitmapFromCache) 
#endif
   ...
   IPC_MESSAGE_HANDLER(ViewHostMsg_setSelectionVisible,OnReceiveSelectionVisibility)
   IPC_MESSAGE_HANDLER(ViewHostMsg_updateSelectionRect,OnUpdateSelectionRect)
#endif
#if defined(SBROWSER_FP_SUPPORT)
   IPC_MESSAGE_HANDLER(ViewHostMsg_NotifyAutofillBegin, OnNotifyAutofillBegin)
#endif
#if defined (SBROWSER_SCROLL_INPUTTEXT)
    IPC_MESSAGE_HANDLER(ViewHostMsg_UpdateTextFieldBounds, OnTextFieldBoundsChanged)
#endif
...
// SAMSUNG : SmartClip >>
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_SmartClipDataExtracted, OnSmartClipDataExtracted)
#endif
// SAMSUNG : SmartClip <<
#if defined(SBROWSER_READER_NATIVE)
    IPC_MESSAGE_HANDLER(ViewHostMsg_OnRecognizeArticleResult, OnRecognizeArticleResult) // SAMSUNG: Reader
#endif
#if defined (SBROWSER_SOFTBITMAP_IMPL)
   IPC_MESSAGE_HANDLER(ViewHostMsg_SoftBitmap,OnSoftBitmap)
   IPC_MESSAGE_HANDLER(ViewHostMsg_UpdateReCaptureSoftBitmap,OnUpdateReCaptureSoftBitmap)
#endif
#if defined (SBROWSER_PRINT_IMPL)
	IPC_MESSAGE_HANDLER(ViewHostMsg_OnBitmapForPrint, OnBitmapForPrint)
	IPC_MESSAGE_HANDLER(ViewHostMsg_OnPrintBegin, OnPrintBegin)
#endif

#if defined(SBROWSER_FLUSH_PENDING_MESSAGES)
   IPC_MESSAGE_HANDLER(ViewHostMsg_DidFlushPendingMessages, OnDidFlushPendingMessages)
#endif
...
void RenderViewHostImpl::OnRenderViewGone(int status, int exit_code) {
	LOG(INFO)  << "TouchLockup RenderViewHostImpl::OnRenderViewGone";
...
#if defined(SBROWSER_GRAPHICS_MSC_TOOL)
  MSC_CALLBACK_2("RENDERER","RVH","Navigated",validated_params.transition,"");
#endif
...
#if defined(SBROWSER_IMAGEDRAG_IMPL)
void RenderViewHostImpl::OnReceiveBitmapForDragging(const SkBitmap& bitmap,int width, int height){
 RenderViewHostDelegateView* view = delegate_->GetDelegateView();
  if (!view)
    return;	
    view->SetBitmapForDragging(bitmap,width,height);
}
#endif
...
#if defined (SBROWSER_BINGSEARCH_ENGINE_SET_VIA_JAVASCRIPT)
void RenderViewHostImpl::SetBingSearchEngineAsDefault(bool enable) {

  RenderViewHostDelegateView* view = delegate_->GetDelegateView();
  if (view)
      view->SetBingSearchEngineAsDefault(enable);
...
void RenderViewHostImpl::OnFocusedNodeChanged(bool is_editable_node) {
  NotificationService::current()->Notify(
	  ...
	  // plm fixed for webdialogBox
	#if defined(SBROWSER_FORM_NAVIGATION)
	  if(!is_editable_node){
	  RenderWidgetHostImpl::OnSelectPopupClose();
	  }
	#endif
}
...
#endif
#if defined(SBROWSER_ENABLE_ADBLOCKER)
	ResourceDispatcherHostImpl::setEnableAdBlocker(prefs.enable_adblocker);
#endif
...
#if defined (SBROWSER_SAVEPAGE_IMPL)
void RenderViewHostImpl::OnBitmapForScrap(const SkBitmap& bitmap, bool response) {
    RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
     if (view)
     view->OnBitmapForScrap(bitmap, response);
}

void RenderViewHostImpl::ScrapPageSavedAs(const base::FilePath::StringType& pure_file_name) 
{
    RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
     if (view)
       view->ScrapPageSavedFileName(pure_file_name);
}
void RenderViewHostImpl::ApplyContentAccessSetting() { //ramya
  const WebPreferences webkit_prefs = delegate_->GetWebkitPrefs();
  if (webkit_prefs.allow_content_url_access) {
    ChildProcessSecurityPolicy::GetInstance()->GrantScheme(
        GetProcess()->GetID(), chrome::kContentScheme);
  } else {
...
#endif
#if defined (SBROWSER_SOFTBITMAP_IMPL)
void RenderViewHostImpl::ReCaptureSoftBitmap(int x, int y, int height,float pageScaleFactor, bool fromPageLoadFinish){
    Send(new ViewMsg_ReCaptureSoftBitmap(GetRoutingID(), x, y, height, pageScaleFactor, fromPageLoadFinish));
}

void RenderViewHostImpl::OnUpdateReCaptureSoftBitmap(const SkBitmap& bitmap, bool fromPageLoadFinish) {
LOG(INFO) << "Chandra soft RenderViewHostImpl::OnReCaptureSoftBitmap";
   RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
   if (view)
       view->UpdateReCaptureSoftBitmap(bitmap, fromPageLoadFinish);
}

void RenderViewHostImpl::OnSoftBitmap(const SkBitmap& bitmap) {
LOG(INFO) << "ramya soft RenderViewHostImpl::OnSoftBitmap";
   RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
   if (view)
       view->UpdateSoftBitmap(bitmap);
}
#endif

#if defined(SBROWSER_FPAUTH_IMPL)
bool RenderViewHostImpl :: OnCheckFPAuth(){


	bool ret = false;
	RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
	if (view)
		ret = view->OnCheckFPAuth();

	return ret;
}

void RenderViewHostImpl :: OnLaunchFingerPrintActivity(){
	RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
	if (view)
		view->OnLaunchFingerPrintActivity();

}

 bool RenderViewHostImpl::getFPAuthStatusFlag() {
	 return fpAuthStatusFlag_;
}


 void RenderViewHostImpl::setFPAuthStatusFlag(bool fpAuthStatusFlag) {
	 fpAuthStatusFlag_ = fpAuthStatusFlag;
  }
  
#endif

#if defined (SBROWSER_PRINT_IMPL)
void RenderViewHostImpl::OnBitmapForPrint(const SkBitmap& bitmap, int responseCode) {
	RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
	if (view)
		view->OnBitmapForPrint(bitmap, responseCode);
}

void RenderViewHostImpl::OnPrintBegin(int totalPageCount) {
	RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
	if (view)
		view->OnPrintBegin(totalPageCount);
}
...
void RenderViewHostImpl::OnReceiveSelectionVisibility(bool isVisible)
{
    RenderViewHostDelegateView* view = delegate_->GetDelegateView();	
    if (!view)
        return;
    view->SetSelectionVisibilty(isVisible);
}
...
#if defined(SBROWSER_FP_SUPPORT)
void RenderViewHostImpl::OnNotifyAutofillBegin()
{
  RenderViewHostDelegateView* view = delegate_->GetDelegateView();
  if (view)
  	view->NotifyAutofillBegin();
  return;
}
#endif

#if defined (SBROWSER_SCROLL_INPUTTEXT)
void RenderViewHostImpl::ScrollEditWithCursor(int scrollX) {
    Send(new ViewMsg_ScrollEditWithCursor(GetRoutingID(), scrollX));
}

void RenderViewHostImpl::OnTextFieldBoundsChanged(const gfx::Rect&  input_edit_rect, bool isScrollEnable)
{
    RenderViewHostDelegateView* view = delegate_->GetDelegateView();
    if (!view)
        return;
    view->OnTextFieldBoundsChanged(input_edit_rect, isScrollEnable);
}
#endif
...
void RenderViewHostImpl::OnSelectedMarkup(const string16& markup)
{
   RenderViewHostDelegateView* view =  delegate_->GetDelegateView();
   if (view) 
...
void RenderViewHostImpl::HandleSelectionDrop(int x, int y, string16& text)
{
...
void RenderViewHostImpl::OnOpenUrlInNewTab(const string16& mouseClickUrl)
{
    RenderViewHostDelegateView* view = delegate_->GetDelegateView();
    if (!view)
        return;
    view->OnOpenUrlInNewTab(mouseClickUrl);
}
#endif
...
// SAMSUNG: Reader >>
#if defined(SBROWSER_READER_NATIVE)
void RenderViewHostImpl::OnRecognizeArticleResult(bool isArticle) {
    if (view_)
        view_->OnRecognizeArticleResult(isArticle);
}
#endif
// SAMSUNG: Reader <<
...
{
	LOG(INFO)<<"SBRContent ::RenderViewHostImpl::LoadDataWithBaseUrl";
	Send(new ViewMsg_LoadDataWithBaseUrl(GetRoutingID(), data,baseurl,mimetype,encoding,historyurl));
}
#endif
...
#if defined(SBROWSER_FLUSH_PENDING_MESSAGES)
void RenderViewHostImpl::FlushPendingMessages(MessageFlushObserver* observer) {
	msgFlushObserver_ = observer;
	Send(new ViewMsg_FlushPendingMessages(GetRoutingID()));
       LOG(INFO) <<"Sent ViewMsg_FlushPendingMessages";
}

void RenderViewHostImpl::OnDidFlushPendingMessages() {
       LOG(INFO) <<"Received ViewHostMsg_DidFlushPendingMessages";
	if (msgFlushObserver_)
		msgFlushObserver_->DidFlushPendingMessages();
