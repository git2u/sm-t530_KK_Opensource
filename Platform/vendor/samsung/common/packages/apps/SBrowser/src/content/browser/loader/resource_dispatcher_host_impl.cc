...
// static
#if defined(SBROWSER_ENABLE_ADBLOCKER)
bool ResourceDispatcherHostImpl::s_EnableAdBlocker = false;
#endif
...
#if defined(SBROWSER_SESSION_CACHE)
	net::SessionCacheInterceptor::EnsureRegistered();
#endif
...
void ResourceDispatcherHostImpl::BeginRequest(
  ...
  if(SecNativeFeature::getInstance()->getInteger(CscFeature_Web_MaxConnectionPerHost) > 0) {
    if (sync_result != NULL && pending_loaders_.size() > 0) {
      std::string url = request_data.url.spec();
      const char* PAGE_READY_URL = "PageReady.html?check";
      const char* COMPLETE_PAGE_URL = "CompletePage.html?check";
      if (url.find(PAGE_READY_URL) !=  std::string::npos
          || url.find(COMPLETE_PAGE_URL) !=  std::string::npos) {
        VLOG(0) << "BeginRequest(): ignore XHR";
...
#if defined(SBROWSER_ENABLE_ADBLOCKER)
	if(s_EnableAdBlocker)
	{
		bool userfilterBlocked = false;
		String contentType;
		static String docDomain;
		static String host;
		...
		*/		
		...
		if(WebAdBlocker::isAd_Banner(request_data.url.spec(),contentType, docDomain, &userfilterBlocked,referrer.url.spec(),request_data.url.host()) )
		{     
			LOG(INFO)<<"Vishal  ResourceDispatcherHostImpl::URL BLOCKED :"<<request_data.url.spec().c_str();
			AbortRequestBeforeItStarts(filter_, sync_result, route_id, request_id);
			return;
		}
	}
...
#if defined(SBROWSER_SESSION_CACHE)
  request->set_back_forward_navigation(request_data.is_back_forward_navigation);
#endif
...
#if defined(SBROWSER_SESSION_CACHE)
	// Make the session cache set extra data with the request, just in case we need to service it.
	// Pass data which will help in creating the SessionCacheRequestHandler
  	 net::SessionCacheInterceptor::SetExtraRequestInfo(request);
#endif
...
#if defined(SBROWSER_SESSION_CACHE)
  if (route_id != -1)
    net::SessionCacheStorage::GetInstance()->
            DoomCacheEntries(net::SessionCacheID(route_id, child_id));
#endif
...
void ResourceDispatcherHostImpl::UpdateLoadStates() {
  ...
  for (i = pending_loaders_.begin(); i != pending_loaders_.end(); ++i) {
    ...
    countUpdateLoadStates_ = countUpdateLoadStates_+1;
    if(countUpdateLoadStates_ > 100 ) {
        LOG(INFO) << "UpdateLoadStates: " << pending_requests()  << " pending requests exist";
        countUpdateLoadStates_ = 0;
    }
...
#if defined(SBROWSER_ENABLE_ADBLOCKER)
void ResourceDispatcherHostImpl::setEnableAdBlocker(bool enabled)
{
	s_EnableAdBlocker = enabled;
}
#endif
}  // namespace content
