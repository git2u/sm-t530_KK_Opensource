...
#if defined(SBROWSER_CSC_FEATURE)
	  if (SecNativeFeature::getInstance()->getEnableStatus(CscFeatureTagWeb_AddOptionToTerminate))
	  {    // We've gotten our first entry for this batch, fire off the timer.
		  // kCommitIntervalMs value should be passed here but for china model
		  // cookie should be flushed immediately to data base hence writing every 1 sec
		  // instead of 30 sec
		  int tmp_kCommitIntervalMs = 1* 1000;
		  if (!background_task_runner_->PostDelayedTask(
					  FROM_HERE, base::Bind(&Backend::Commit, this),
					  base::TimeDelta::FromMilliseconds(tmp_kCommitIntervalMs))) {
