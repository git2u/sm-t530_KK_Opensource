...
#if defined(SBROWSER_COMMIT_BLOCK_ONTOUCH)
void Scheduler::setNeedsCommitBlock(bool blockState)
{
   if(m_setCommitBlock == blockState)
        return;
   m_setCommitBlock = blockState;
   state_machine_.setNeedsCommitBlock(blockState);
   ProcessScheduledActions();
}
#endif
