...
#if ENABLE(SBROWSER_TEXT_CODEC_MEMORY_REDUCTION)
    enum UseExternalBuffer { useExternalBuffer };
    StringImpl(const LChar* characters, unsigned length, UseExternalBuffer)
        : m_data8(characters)
        , m_buffer(0)
        , m_refCount(s_refCountIncrement)
        , m_length(length)
        , m_hashAndFlags(s_hashFlag8BitBuffer | BufferExternal)
    {
        ASSERT(m_data8);
        ASSERT(m_length);
    }
...
#if ENABLE(SBROWSER_TEXT_CODEC_MEMORY_REDUCTION)
    static PassRefPtr<StringImpl> adopt(StringBufferConst<LChar>&);
#endif
