...
class BookmarkNode : public ui::TreeNode<BookmarkNode> {
 public:
  ...
  int64 Editable() const { return editable_; }
  void set_editable(int64 editable) { editable_ = editable; }
  
  int64 OperatorBookmark() const { return operator_bookmark_; }
  void set_operator_bookmark(int64 operator_bookmark) { operator_bookmark_ = operator_bookmark; }
...
class BookmarkModel : public content::NotificationObserver,
 ...
 public:
  ...
  //Samsung feature begin - OperatorBookmarks
  // Update the bookmark Node 
#if defined(SBROWSER_OPERATOR_BOOKMARKS)
  void UpdateBookmarkNode(const BookmarkNode* node);
#endif
  //Samsung feature end
...
#if defined(SBROWSER_BOOKMARKS)
//SAMSUNG Change - Get Count of Bookmarks under node of given type >>
 int GetBookmarkNodeCountOfType(const BookmarkNode * node, BookmarkNode::Type type);
//SAMSUNG Change - Get Count of Bookmarks under node of given type <<
#endif//#if defined(SBROWSER_BOOKMARKS)
...
#if defined(SBROWSER_BOOKMARKS)
  void set_my_device_id(const int64 my_device_id) {
    my_device_id_ = my_device_id;
  }

  const BookmarkNode* my_device_node() {
    if(my_device_id_ == 0)
      return NULL;
    return GetNodeByID(my_device_id_);
  }
 #endif //#if defined(SBROWSER_BOOKMARKS)
