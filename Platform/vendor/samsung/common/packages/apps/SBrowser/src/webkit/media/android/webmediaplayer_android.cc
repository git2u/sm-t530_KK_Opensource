...
#if defined(SBROWSER_DATA_COMPRESSION) || defined(SBROWSER_MEDIAPLAYER_CURRENT_CONSUMPTION_FIX)
...
void WebMediaPlayerAndroid::play() {
  ...
  content::RenderViewImpl* render_view = web_view ? content::RenderViewImpl::FromWebView(web_view) : NULL;
  if (render_view && render_view->is_hidden() && hasVideo()) {
      OnMediaPlayerPause();
      return;
  }
#endif
...
#if defined(SBROWSER_MEDIAPLAYER_SETVOLUME_IMPL)
...
void WebMediaPlayerAndroid::OnPlaybackComplete() {
  ...
  client_->timeChanged();
#ifdef SBROWSER_MEDIAPLAYER_LOOP
  is_playing_ = false;
#endif
}
...
void WebMediaPlayerAndroid::OnVideoSizeChanged(int width, int height) {
  ...
  ReallocateVideoFrame();
#if defined(SBROWSER_MEDIAPLAYER_POSTER_UPDATE_FIX)
  client_->sizeChanged();
#endif
}
