...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
    KeywordSearchTermRow keyword_row;
    LOG(INFO)<<"SmartSuggest last ids first: "<<last_ids.first<<" second "<<last_ids.second;
    history::URLRow referrer_row;
    if(db_->GetRowForURL(request.referrer, &referrer_row)){
    	db_->GetKeywordSearchTermRow(referrer_row.id_, &keyword_row);
    	LOG(INFO)<<"referrer id: "<<referrer_row.id_;
    	LOG(INFO)<<"Terms: "<<keyword_row.term;
		LOG(INFO)<<"from_visit_id = referrer id: "<<from_visit_id;
		LOG(INFO)<<"id of result url: "<<last_ids.first;
		LOG(INFO)<<"result url: "<<request.url;
		if(!keyword_row.term.empty()) {
			LOG(INFO)<<"Entry found: Inserting into db";
			db_->SetSearchTermsForURL(last_ids.first, keyword_row.term);
			KeywordSearchTermDetails* details = new KeywordSearchTermDetails;
			details->url = request.url;
			details->keyword_id = 0;
			details->term = keyword_row.term;
			BroadcastNotifications(
					chrome::NOTIFICATION_HISTORY_SEARCH_TERMS_RESULTS_UPDATED,
					details);
		}
		else{
			LOG(ERROR)<<"No search term entry for this URL"<<request.referrer;
		}
    }
    else if(db_->GetRowForURL(redirects[0], &referrer_row)){
    	LOG(INFO)<<"Redirect[0] id is not present: "<<request.referrer;
    	db_->GetKeywordSearchTermRow(referrer_row.id_, &keyword_row);
    	LOG(INFO)<<"Redirect[0] id: "<<referrer_row.id_;
    	LOG(INFO)<<"Terms: "<<keyword_row.term;
    	LOG(INFO)<<"from_visit_id = referrer id: "<<from_visit_id;
    	LOG(INFO)<<"id of result url: "<<last_ids.first;
    	LOG(INFO)<<"result url: "<<request.url;
    	if(!keyword_row.term.empty()) {
    		...
    		db_->SetSearchTermsForURL(last_ids.first, keyword_row.term);
    		KeywordSearchTermDetails* details = new KeywordSearchTermDetails;
    		details->url = request.url;//result url to get its id
    		details->keyword_id = 0;//unused
    		details->term = keyword_row.term;//search terms
    		BroadcastNotifications(
				chrome::NOTIFICATION_HISTORY_SEARCH_TERMS_RESULTS_UPDATED,
				details);
    	}
    	else{
    		LOG(ERROR)<<"No search term entry for this URL"<<redirects[0];
		}
    }
    else{
    	LOG(ERROR)<<"redirect id is not present: "<<redirects[0];
    }
#endif
...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  LOG(INFO)<<"SmartSuggest term: "<<term<< "url id: "<<url_row.id();
#endif
