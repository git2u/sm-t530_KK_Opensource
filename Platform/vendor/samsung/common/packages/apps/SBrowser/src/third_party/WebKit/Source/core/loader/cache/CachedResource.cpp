...
void CachedResource::load(CachedResourceLoader* cachedResourceLoader, const ResourceLoaderOptions& options)
{
	...
	//we need this info for session cache implementation which loads items from 
	// session cache only if its back forward navigation.
	//frame can be NULL!! But lets be extra cautious.
	if( cachedResourceLoader->document() 
			&& cachedResourceLoader->document()->frame() 
			&& cachedResourceLoader->document()->frame()->loader()){
		switch(cachedResourceLoader->document()->frame()->loader()->loadType()){
			case FrameLoadTypeBack :
			case FrameLoadTypeForward:
			case FrameLoadTypeIndexedBackForward:{
				m_resourceRequest.setIsBackForwardNavigation(true);
			}
		}
	}
#endif
