...
class CONTENT_EXPORT RenderViewImpl
 ...
 public:
  ...
  virtual void setFontBoostingModeEnabled(bool enable);
#endif // SBROWSER_FONT_BOOSTING_PREFERENCES
  // SAMSUNG_CHANGE : FontBoosting mode
  #if defined(SBROWSER_BINGSEARCH_ENGINE_SET_VIA_JAVASCRIPT)
  virtual void setBingSearchEngineAsDefaultSearch(bool enable);
  #endif
  
// SAMSUNG_CHANGE : Overview mode
#if defined(SBROWSER_OVERVIEW_MODE)
  void setIsReload(bool val) {m_isreload = val;}
  virtual bool getIsReload();
...
// SAMSUNG_CHANGE : Overview mode
  ...
  virtual void destroyExternalPopupMenu(
      WebKit::WebExternalPopupMenuClient* popup_menu_client);
#endif
  ...
  virtual void detectLinkOnTouch( const WebKit::WebElement& );
  ...
  virtual void setUserAgentOverride(bool shouldOverride) OVERRIDE;
  #endif
...
// SAMSUNG CHANGE : For keylagging perfomance >>
  ...
  virtual void DidKeyDownEvent() OVERRIDE;
#endif
// SAMSUNG CHANGE : For keylagging perfomance <<
 ...
 private:
  ...
  void OnDirectPaste(const string16& text);
  // SAMSUNG CHANGE : Add paste by secClipboardEx <<
...
void OnSetPasswordEcho(bool pwdEchoEnabled);
#endif
...
// --------------------------- kikin Modification starts ---------------------------
  ...
  void OnGetSelectionContext(const bool shouldUpdateSelectionContext);
  void OnUpdateSelection(const std::string& oldSelection, const std::string& newSelection);
...
// ---------------------------  kikin Modification ends  ---------------------------
   ...
   void OnCheckBelongToSelection(int x, int y);
   void OnGetSelectionVisibleRequest();
   void onGetCurrentSelectionRect();
 #endif
#if defined(SBROWSER_TABLET_SWITCH)
    void  TabletSwitchEnabled();
#endif
#if defined (SBROWSER_SOFTBITMAP_IMPL)  
  void OnReCaptureSoftBitmap(int x, int y, int height, float pageScaleFactor, bool fromPageLoadFinish);
#endif 

#if defined(SBROWSER_FP_SUPPORT)
   void OnNotifyAutofillBeginMain();
#endif

#if defined (SBROWSER_SCROLL_INPUTTEXT)
void OnScrollEditWithCursor(int scrollX);
void updateTextFieldBounds(WebKit::WebRect inputFieldBounds, bool isScrollEnable,bool focusOn);
#endif

#if defined (SBROWSER_HANDLE_MOUSECLICK_CTRL)
void OpenUrlInNewTab(const string16& mouseClickUrl);
#endif
   ...
   void OnHandleSelectionDrop(int x, int y, const string16& text);
#endif
#if defined (SBROWSER_HANDLE_MOUSECLICK_CTRL)
   void OnHandleMouseClickWithCtrlkey(int x, int y);
#endif
 // SAMSUNG : SmartClip >>
#if defined(SBROWSER_SMART_CLIP)  
    void OnSmartClipDataRequest(int x, int y, int w, int h); 
#endif
// SAMSUNG : SmartClip <<

#if defined(SBROWSER_READER_NATIVE)
    void OnRecognizeArticle(int mode); // SAMSUNG: Reader
#endif
	...
	void messageToClosePopup();
    int  getPrivateIMEOptionsValue() { return m_private_ime_options;}
#endif 
  ...
  void OnHoverHighLight(const WebKit::WebInputEvent* hover_event,bool high_light);
#endif

#if defined(SBROWSER_FPAUTH_IMPL)
 void FPAuthStatus(bool enabled, std::string usrName);
 void TriggerDidFinishDocumentLoad( WebKit::WebFrame* frame);
 void SubmitForm(WebKit::WebInputElement& pwd);
#endif
  ...
  void OnGetScrapBitmap( int bitmapWidth, int bitmapHeight); 
	void OnGetBitmapFromCachedResource(const std::string imageUrl);
#endif  

#if defined (SBROWSER_SBR_NATIVE_CONTENTVIEW)
   void OnLoadDataWithBaseUrl(const std::string& data,
...
// SAMSUNG CHANGE : For keylagging perfomance >>
#if defined (SBROWSER_KEYLAGGING_PERFORMANCE)
void ResetUpdateStyleIfNeeded();
void SetUpdateStyleIfNeeded(bool updateStyleIfNeeded);
#endif
// SAMSUNG CHANGE : For keylagging perfomance <<
...
// SAMSUNG_CHANGE : Overview mode
  ...
  // SAMSUNG CHANGE : For keylagging perfomance >>
#if defined (SBROWSER_KEYLAGGING_PERFORMANCE)
  base::OneShotTimer<RenderViewImpl> check_deferred_input_timer_;
  base::TimeTicks pre_down_time;
#endif
  // SAMSUNG CHANGE : For keylagging perfomance <<

#if defined(SBROWSER_FORM_NAVIGATION)
  int m_private_ime_options;
#endif

 std::string last_touch_link_href_;
