...
class CC_EXPORT LayerTreeHost : NON_EXPORTED_BASE(public RateLimiterClient) {
 public:
  ...
  void UpdateContentSize(bool hScroll, bool vScroll, gfx::SizeF size);
  void ReleaseEdgeGlow(int axis);
  void AddEdgeGlowLayer();
  bool IsGlowAnimationActive();
#endif
  ...
  void UpdateLayers(ResourceUpdateQueue* queue,
    ...
    void setGestureState(bool gestureActive);
#endif
#if defined(SBROWSER_COMMIT_BLOCK_ONTOUCH)
    virtual void  setCommitBlock(bool blockState) ;
#endif
...
void FlingAnimationCompleted(bool page_end);
  ...
  void UpdateCompositingLayersIfNeeded();
#endif
  
  HeadsUpDisplayLayer* hud_layer() const { return hud_layer_.get(); }

#if defined(SBROWSER_HIDE_URLBAR)
  Layer* urlbar_layer() const { return static_cast<Layer*>(urlbar_layer_.get()); }
  Layer* web_root_layer() const { return web_root_layer_.get(); }
  int urlbar_height() { return urlbar_height_; }
  void SetUrlBarBitmap(SkBitmap* bitmap);
  void SetUrlBarActive(bool active, bool override_allowed);
  void DidHideUrlBarEvent(int event, int event_data1, int event_data2);
  void SetBitmapSyncPending(bool sync) {bitmap_sync_pending_ = sync;}
  bool BitmapSyncPending() {return bitmap_sync_pending_;}
#endif
 ...
 private:
  ...
  #if  defined(SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
    void setGestureStateRecursive(Layer* current, bool gestureActive);
#endif

#if defined(SBROWSER_HIDE_URLBAR)
  void UpdateUrlBarLayer();
#endif
  bool animating_;
  ...
  scoped_ptr<content::OverscrollGlow> overscroll_effect_;
  bool set_hScrollbar_previous;
  bool set_vScrollbar_previous;
  gfx::Size set_size_previous;
  float WebLayerY_previous;
  enum Axis { AXIS_X = 0, AXIS_Y, AXIS_XY };
#endif

#if defined(SBROWSER_HIDE_URLBAR)
  scoped_refptr<Layer> web_root_layer_;
  scoped_refptr<ImageLayer> urlbar_layer_;
  int urlbar_height_;
  bool bitmap_sync_pending_;
#endif
