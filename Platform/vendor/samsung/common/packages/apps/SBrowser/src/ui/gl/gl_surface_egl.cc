...
bool NativeViewGLSurfaceEGL::Initialize() {
  ...
  if (!surface_) {
    ...
    EGLBoolean ret = eglSurfaceAttrib(GetDisplay(), surface_, EGL_SWAP_BEHAVIOR, EGL_BUFFER_PRESERVED);
    LOG(INFO)<<__FUNCTION__<<"set swap attr: EGL_BUFFER_PRESERVED:"<<"ret val: "<<ret;
#endif
  EGLint surfaceVal;
...
void NativeViewGLSurfaceEGL::Destroy() {
  if (surface_) {
/*#if defined(SBROWSER_GRAPHICS_GETBITMAP)
    LOG(INFO)<<"Setting NO_CONTEXT before deletion";
   // SAMSUNG GRAPHICS CHANGE +
  	if (!eglMakeCurrent(GetDisplay(), EGL_NO_SURFACE,
...
//  VLOG(0)<<"NativeViewGLSurfaceEGL::Resize   size =  "<<size.ToString()<<   "    GetSize()=  "<<GetSize().ToString();
if (size == GetSize() 
#if defined(SBROWSER_GRAPHICS_UI_COMPOSITOR_REMOVAL)
&& (prev_size_ == size)
#endif
...
{
// VLOG(0)<<"NativeViewGLSurfaceEGL::Resize   prev_size_ == size  return";
    return true;
}
