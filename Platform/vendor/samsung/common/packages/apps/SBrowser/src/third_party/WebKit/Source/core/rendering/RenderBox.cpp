    ...
    BackgroundBleedAvoidance bleedAvoidance, CompositeOperator op, RenderObject* backgroundObject)
{
    ...
    if (frame){
		...
		if(settings && settings->isBgColorOffWhiteEnabled()){
			newColor = Color::offwhite;
			applyColor = true;
		}
		else if(settings && settings->isBgColorLightGreyEnabled()){
			newColor = Color::lightGrey;
			applyColor = true;
		}
		else if(settings && settings->isBgColorGreyEnabled()){
			newColor = Color::gray;
			applyColor = true;
		}
		if(applyColor && (c.rgb()>=newColor.rgb() && c.rgb()<=white.rgb())){
			c1 = newColor;
		}
    }
#endif
...
#if defined(SBROWSER_POWER_SAVER_MODE_IMPL)		
        paintFillLayer(paintInfo, c1, *it, rect, bleedAvoidance, op, backgroundObject);
#else
