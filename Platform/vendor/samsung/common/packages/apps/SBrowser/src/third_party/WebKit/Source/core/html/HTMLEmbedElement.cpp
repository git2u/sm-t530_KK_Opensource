...
void HTMLEmbedElement::updateWidget(PluginCreationOption pluginCreationOption)
{
    ...
    if (!renderer()) // Do not load the plugin if beforeload removed this element or its renderer.
        return;
#if defined(SBROWSER_DISABLE_LOAD_FOR_EMBED_MIMETYPES) //disallow loading for unsupported mimetypes in embed element
  if(!(getAttribute(typeAttr)).contains( "audio/mpeg")){
#endif
    SubframeLoader* loader = document()->frame()->loader()->subframeLoader();
    // FIXME: beforeLoad could have detached the renderer!  Just like in the <object> case above.
    loader->requestObject(this, m_url, getNameAttribute(), m_serviceType, paramNames, paramValues);
#if defined(SBROWSER_DISABLE_LOAD_FOR_EMBED_MIMETYPES)
  }
#endif
}
