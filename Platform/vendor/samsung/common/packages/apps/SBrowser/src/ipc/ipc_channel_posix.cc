...
void Channel::ChannelImpl::OnFileCanReadWithoutBlocking(int fd) {
  ...
  if (fd == server_listen_pipe_) {
    ...
    if (pipe_ != -1) {
	  ...
	  LOG(ERROR) <<"[IO][IPC] close pipe : " << pipe_name_;
	...
	LOG(ERROR) <<"[IO][IPC] Unable to query client euid";
        ResetToAcceptingConnectionState();
        return;
      }
      if (client_euid != geteuid()) {
	 LOG(ERROR) <<"[IO][IPC] Client euid is not authorised";
    ...
    if (!ProcessIncomingMessages()) {
      ...
      LOG(ERROR) <<"[IO][IPC] Failed - ProcessIncomingMessages";
      send_server_hello_msg = false;
      ClosePipeOnError();
    }
  } else {
    LOG(ERROR) <<"[IO][IPC] Failed - Unknown pipe";
...
void Channel::ChannelImpl::ClosePipeOnError() {
  LOG(ERROR) <<"[IO][IPC] [ChannelImpl][ClosePipeOnError]";
...
Channel::ChannelImpl::ReadState Channel::ChannelImpl::ReadData(
    ...
    int* bytes_read) {
  if (pipe_ == -1){
    LOG(ERROR) <<"[IO][IPC] [ChannelImpl][ReadData] pipe_ = -1";
    return READ_FAILED;
  }
  ...
  if (*bytes_read < 0) {
    if (errno == EAGAIN) {
      ...
      LOG(ERROR) <<"[IO][IPC] [ChannelImpl][ReadData] errno = " << errno;
      return READ_FAILED;
    } else {
      PLOG(ERROR) << "pipe error (" << pipe_ << ")";
      LOG(ERROR) <<"[IO][IPC] [ChannelImpl][ReadData] pipe error >> " << pipe_;
      return READ_FAILED;
    }
  } else if (*bytes_read == 0) {
    // The pipe has closed...
    LOG(ERROR) <<"[IO][IPC] [ChannelImpl][ReadData] The pipe has closed.";
    return READ_FAILED;
  }
  DCHECK(*bytes_read);
  ...
  if (!ExtractFileDescriptorsFromMsghdr(&msg)){
    LOG(ERROR) <<"[IO][IPC] [ChannelImpl][ReadData] ExtractFileDescriptorsFromMsghdr failed.";
    return READ_FAILED;
  }
