...
bool MediaPlayerListener::RequestAudioFocus(){
  ...
  CHECK(env);
  if (!j_media_player_listener_.is_null()) {
    return Java_MediaPlayerListener_requestAudioFocus(env, j_media_player_listener_.obj());
  }
  return false;
}

void MediaPlayerListener::ShowAudiofocusFailMessage(){
  JNIEnv* env = AttachCurrentThread();
  CHECK(env);
  if (!j_media_player_listener_.is_null()) {
    Java_MediaPlayerListener_showAudiofocusFailMessage(env, j_media_player_listener_.obj());
  }
}
#endif

#if defined(SBROWSER_MEDIAPLAYER_WAKELOCK_FIX)
void MediaPlayerListener::SetWakeLock(jobject context, jobject media_player){
  JNIEnv* env = AttachCurrentThread();
  CHECK(env);
  if (!j_media_player_listener_.is_null()) {
    Java_MediaPlayerListener_setWakeLock(env, j_media_player_listener_.obj(), context, media_player);
  }
}
#endif
