...
#include "ui/gl/interface_shared_memory_pool.h"
#endif
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
AsyncPixelTransferDelegateEGL::AsyncPixelTransferDelegateEGL(SharedMemoryPoolInterface* decoder) {
#else
...
#if defined(SBROWSER_IPC_SHARED_MEMORY_TRACKING)
// TODO: Revisit this patch in furture <pritam.nikam@samsung.com>
// Sometimes while duplicating the shared memory handle fails in mmap() (MAP_FAILED) causing browser to abort
// its execution. And user has to restart to browser again.
// To overcome this problem, we have added below logic so that user does not require the browser, instead recover the
// ref-counted ashmmem by closing renderers.

   scoped_ptr<ScopedSafeSharedMemory> shm_mem(ScopedSafeSharedMemory::create(safe_shared_memory_pool(), mem_params.shared_memory, mem_params.shm_size));
   if (!shm_mem.get()) {
        // mmap() would have failed

        // TODO: post message to renderer that no more shared memory will be alocated.
        LOG(ERROR)<<"[AsyncPixelTransferDelegateEGL][AsyncNotifyCompletion] Failed mmap()";
        gles2Decoder->NotifyMMapFailure();
        return;
    }
...
// ref-counted ashmmem by closing renderers.
   ...
   if (!shm_mem.get()) {
        ...
        // TODO: post message to renderer that no more shared memory will be alocated.
        LOG(ERROR)<<"[AsyncPixelTransferDelegateEGL][AsyncTexImage2D] Failed mmap()";
        gles2Decoder->NotifyMMapFailure();
        return;
    }
...
// ref-counted ashmmem by closing renderers.
   ...
   if (!shm_mem.get()) {
        ...
        // TODO: post message to renderer that no more shared memory will be alocated.
        LOG(ERROR)<<"[AsyncPixelTransferDelegateEGL][AsyncTexSubImage2D] Failed mmap()";
        gles2Decoder->NotifyMMapFailure();
        return;
    }
