...
//	VLOG(0)<<"Mayur"<<"Login Database"<<__FUNCTION__;	
...
bool LoginDatabase::AddLogin(const PasswordForm& form) {
  std::string encrypted_password;
  	  ...
  	  mSbrPasswdMgrUtility = SbrPasswordManagerUtility::getSbrPasswdMgrUtility();
	  if(mSbrPasswdMgrUtility)
	  {
	  	//VLOG(0)<<"Mayur"<<__FUNCTION__;	

		//mSbrPasswdMgrUtility->setLoginDB(this);
		if (!mSbrPasswdMgrUtility->getEncryptedPassword(form.password_value, &encrypted_password))
			return false;
	  }
	  else 
	  {
	  //	VLOG(0)<<"Mayur"<<__FUNCTION__<<"mSbrPasswdMgrUtility is NULL";	
	   	return false;
	  ...
	  if (!EncryptedString(form.password_value, &encrypted_password))
	    return false;
  #endif
//VLOG(0)<<"Mayur"<<__FUNCTION__<<"Coming Here";
  ...
  bool ret = s.Run();
  if(ret)
  	mSbrPasswdMgrUtility->notifyDBChange(form.username_value,form.action.spec(),
  						static_cast<SbrPasswordManagerUtility::dbChangeType>(0));
  return ret;
  #else
 	 return s.Run();
  #endif
...
//VLOG(0)<<"Mayur"<<__FUNCTION__<<"Coming Here";	
  ...
  #ifdef SBROWSER_FP_SUPPORT_PART2
  	mSbrPasswdMgrUtility->notifyDBChange(form.username_value,form.action.spec(),
  				static_cast<SbrPasswordManagerUtility::dbChangeType>(1));
  #endif
...
bool LoginDatabase::RemoveLogin(const PasswordForm& form) {
 ...
 // VLOG(0)<<"Mayur"<<__FUNCTION__;
  ...
  s.BindString(5, form.signon_realm);
	...
	if(ret)
		mSbrPasswdMgrUtility->notifyDBChange(form.username_value,form.action.spec(),
		static_cast<SbrPasswordManagerUtility::dbChangeType>(2));
	return ret;
  #else
	  return s.Run();
  #endif
...
bool LoginDatabase::InitPasswordFormFromStatement(PasswordForm* form,
  ...
  string16 decrypted_password;
	  ...
	  if(mSbrPasswdMgrUtility)
	  {
	  	//Uncomment if reqd.
		//mSbrPasswdMgrUtility.setLoginDB(this);
		if (!mSbrPasswdMgrUtility->getDecryptedPassword(encrypted_password, &decrypted_password))
			return false;
	  }
	  else 
	  	return false;
...
bool LoginDatabase::DeleteAndRecreateDatabaseFile() {
  ...
  #ifdef SBROWSER_FP_SUPPORT_PART2
     string16 empty ;
  	mSbrPasswdMgrUtility->notifyDBChange(empty,"",
  			static_cast<SbrPasswordManagerUtility::dbChangeType>(3));
