...
#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE) || defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
      scroll_page_end(false),
#endif
#if defined(SBROWSER_GENERATING_FAKE_SCROLL_EVENT)
      m_fakeScrollUpdateEvent(gfx::Vector2dF()),
      fake_viewport_point(gfx::Point()),
#endif
...
#if defined(SBROWSER_HIDE_URLBAR)
  urlbar_active_ = false;
  urlbar_active_override_ = false;
  pending_urlbar_active_ = false;
  scroll_inprogress_ = false;
  sync_urlbar_active_ = false;
  pending_urlbar_update_ = false;
#endif
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
  defer_commit_ = false;
#endif
#if defined(SBROWSER_SKIP_MODIFYING_TILE_PRIORITY)
  skip_modifying_tile_priority_ = false;
#endif
#if defined(SBROWSER_BLOCK_SENDING_FRAME_MESSAGE)
  need_to_update_frame_info_ = false;
#endif
...
if (settings_.enable_edge_gloweffect) {
  set_hScrollbar_status = false;
  set_vScrollbar_status = false;
  set_size_current.set_width(0);
  set_size_current.set_height(0);
  deactive_glow_on_TouchStart = false;
  block_glow_on_pinch = false;
  event_type = LayerTreeHostImpl::Undefined;
  accumulated_root_overscroll_y_current = 0;
  accumulated_root_overscroll_y_previous = 0;
  accumulated_root_overscroll_x_current = 0;
  accumulated_root_overscroll_x_previous = 0;
  WebLayerY_previous = settings_.top_controls_height;
  glow_effect_active_ = false;
}  
#endif
...
bool LayerTreeHostImpl::CanDraw() {
  ...
  if (!active_tree_->root_layer()) {
    ...
    VLOG(0)<<"cc_logs: "<<"LayerTreeHostImpl::CanDraw no root layer";
    return false;
  }
  if (device_viewport_size_.IsEmpty()) {
    TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw empty viewport",
                         TRACE_EVENT_SCOPE_THREAD);
    VLOG(0)<<"cc_logs: "<<"LayerTreeHostImpl::CanDraw empty viewport";
    return false;
  }
  if (active_tree_->ViewportSizeInvalid()) {
    TRACE_EVENT_INSTANT0(
        ...
        TRACE_EVENT_SCOPE_THREAD);
    VLOG(0)<<"cc_logs: "<<"LayerTreeHostImpl::CanDraw viewport size recently changed";
    return false;
  }
  if (!renderer_) {
    TRACE_EVENT_INSTANT0("cc", "LayerTreeHostImpl::CanDraw no renderer",
                         TRACE_EVENT_SCOPE_THREAD);
    VLOG(0)<<"cc_logs: "<<"LayerTreeHostImpl::CanDraw no renderer";
    return false;
  }
  if (active_tree_->ContentsTexturesPurged()) {
    TRACE_EVENT_INSTANT0(
        ...
        TRACE_EVENT_SCOPE_THREAD);
    VLOG(0)<<"cc_logs: "<<"LayerTreeHostImpl::CanDraw contents textures purged";
    return false;
  }
  return true;
}
...
void LayerTreeHostImpl::SetHighPriorityToRasterWorker(bool flag) { 
    TRACE_EVENT2("cc","LayerTreeHostImpl::SetHighPriorityToRasterWorker"
        , "isImplSidePainting", (settings_.impl_side_painting ? "true" : "false")
        , "tile_manager_", (tile_manager_ ? "true" : "false")
        );
    if(settings_.impl_side_painting)        
        TileManager::SetHighPriorityToRasterWorker(flag, tile_manager());
}
#endif
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)	
  if (settings_.enable_edge_gloweffect)
      deactive_glow_on_TouchStart = true;
#endif	
	
#if defined(SBROWSER_HIDE_URLBAR)
 if(settings_.enable_hide_urlbar)
 {
   viewport_point.set_y(viewport_point.y() + settings_.top_controls_height); 
 }
...
#if defined(SBROWSER_CQ_KoreaHerald)
...
bool LayerTreeHostImpl::CalculateRenderPasses(FrameData* frame) {
  ...
  {
    VLOG(0)<<"cc_logs: "<<"Any problem with CanDraw or active_tree..?";
    return false;
  }
...
bool LayerTreeHostImpl::PrepareToDraw(FrameData* frame,
...
#if defined(SBROWSER_HIDE_URLBAR)
  if(settings_.enable_hide_urlbar && !scroll_inprogress_){
    if(sync_urlbar_active_){
      if(urlbar_active_ != pending_urlbar_active_){
        urlbar_active_ = pending_urlbar_active_;
      }
      if(!urlbar_active_){
        sync_urlbar_active_ = false;
      }
      VLOG(0)<<"HideUrlBar: "<<__FUNCTION__<<" Syncing active: urlbar_active_: "<<urlbar_active_<<" urlbar_active_override_: "<<urlbar_active_override_;
  ...
  {
    VLOG(0)<<"cc_logs: "<<"Any problem with CalculateRenderPasses..?";
    return false;
  }
...
bool LayerTreeHostImpl::ShouldForceTileUploadsRequiredForActivationToComplete() const {
...
#if defined(SBROWSER_BLOCK_SENDING_FRAME_MESSAGE)
void LayerTreeHostImpl::NeedToUpdateFrameInfo(bool enable) {
  need_to_update_frame_info_ = enable;
}
#endif
...
CompositorFrameMetadata LayerTreeHostImpl::MakeCompositorFrameMetadata() const {
  ...
  if(settings_.enable_hide_urlbar && client_ && top_controls_manager_) {
     client_->DidHideUrlBarEventOnImplThread(cc::CONTENT_OFFSET_CHANGE_NOTIFY, (int)(top_controls_manager_->content_top_offset()), (int)(top_controls_manager_->controls_top_offset()));
  }
#endif
...
gfx::SizeF LayerTreeHostImpl::VisibleViewportSize() const {
  ...
  gfx::Size ceiled_dip_size= gfx::ToCeiledSize(dip_size);
  dip_size = gfx::SizeF((float)ceiled_dip_size.width(), (float)ceiled_dip_size.height());
...
void LayerTreeHostImpl::SetVisible(bool visible) {
  ...
  TRACE_EVENT2("cc", "LayerTreeHostImpl::SetVisible","visible",visible,"page_scale_animation_active",page_scale_animation_active());
  VLOG(0)<<"LayerTreeHostImpl::SetVisible visible="<<visible;

  if (visible_ == visible)
    return;

  if(visible && page_scale_animation_active()){
    TRACE_EVENT0("cc", "LayerTreeHostImpl::SetVisible page_scale_animation_active  reset anim");
    VLOG(0)<<"LayerTreeHostImpl::SetVisible page_scale_animation_active  reset anim";
...
#if defined(SBROWSER_BLOCK_SENDING_FRAME_MESSAGE)
    renderer_ ->SetGestureInProgress(false);
#endif

#if defined(SBROWSER_GL_OUT_OF_MEMORY_PATCH)
  //Patch to avoid GL_OUT_OF_MEMORY
  if((!visible) && (tile_manager_ != NULL))
  {
  	if(settings_.is_imagination_)
  	{
  		LOG(INFO)<<this<<" "<< __FUNCTION__<< " AbortPendingTileUploads() Called" ;
  		tile_manager_->AbortPendingTileUploads() ;
  	}
	else
	{
  		LOG(INFO)<<this<<" "<< __FUNCTION__<< " CheckForCompletedTileUploads() Called" ;
  		tile_manager_->CheckForCompletedTileUploads() ;
	}
  }
...
if (settings_.enable_edge_gloweffect) {
	block_glow_on_pinch = false;
	if(event_type != LayerTreeHostImpl::GestureScrollBegin)
	    event_type = LayerTreeHostImpl::GestureScrollBegin;
}	
#endif  
  ...
  if(settings_.enable_hide_urlbar)
  {
    int content_top_offset = (int)(top_controls_manager_->content_top_offset());
    viewport_point.set_y(viewport_point.y() + content_top_offset); 
  }
#endif 
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
if (settings_.enable_edge_gloweffect) {
	accumulated_root_overscroll_y_previous = accumulated_root_overscroll_y_current  = 0;
	accumulated_root_overscroll_x_previous = accumulated_root_overscroll_x_current  = 0;
}	
#endif	
#if defined(SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
         client_->setGestureStateOnImplThread(true);
#endif
#if defined(SBROWSER_HIDE_URLBAR)
  if(settings_.enable_hide_urlbar && !scroll_inprogress_) {
    if(active_tree_->urlbar_layer() && urlbar_active_override_){
      client_->DidHideUrlBarEventOnImplThread(cc::SCROLL_BEGIN_NOTIFY, 1, 0);
      urlbar_active_ = true;
      VLOG(0)<<"HideUrlBar: "<<__FUNCTION__<<" ScrollBegin, Notified App ";
    }
    else{
      urlbar_active_ = false;
    }
    scroll_inprogress_ = true;
  }
#endif
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
...
#endif
#if defined(SBROWSER_SKIP_MODIFYING_TILE_PRIORITY)
    SetSkipModifyingTilePriority(true);
    accumulated_scroll_delta = 0.f;
#endif
#if defined(SBROWSER_BLOCK_SENDING_FRAME_MESSAGE)
    if (renderer_ && !need_to_update_frame_info_)
        renderer_ ->SetGestureInProgress(true);
#endif
...
bool LayerTreeHostImpl::ScrollBy(gfx::Point viewport_point,
...
#endif
#if  defined(SBROWSER_SKIP_MODIFYING_TILE_PRIORITY)
    SetSkipModifyingTilePriority(false);
#endif
    return false;
  }
...
#endif
#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE) || defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
  gfx::Vector2dF cached_pending_delta;
#endif

  bool did_scroll_x = false;
  bool did_scroll_y = false;
  
  //bool top_controls_update = false;
  bool consume_by_top_controls = top_controls_manager_ &&
      (CurrentlyScrollingLayer() == RootScrollLayer() || scroll_delta.y() < 0);

#if defined(SBROWSER_GENERATING_FAKE_SCROLL_EVENT) || defined(SBROWSER_SKIP_MODIFYING_TILE_PRIORITY)
    ...
    if (settings_.enable_edge_gloweffect) {
        if (layer_impl == RootScrollLayer()) {
          ...
          if (consume_by_top_controls && layer_impl == RootScrollLayer()) {
               ...
               UpdateMaxScrollOffset();
#if defined(SBROWSER_HIDE_URLBAR)
               VLOG(0)<<"HideUrlBar: "<<__FUNCTION__<<" scroll consumed by top_controls_manager ";
#endif
           }
 ...
 #if defined(SBROWSER_FLING_END_NOTIFICATION_CANE) || defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
     cached_pending_delta=pending_delta;
#endif
...
#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE) || defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
    if(!scroll_page_end && layer_impl == RootScrollLayer()){
      cached_pending_delta.Subtract(applied_delta);
    }
#endif
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
if (settings_.enable_edge_gloweffect) {
...
#if  defined(SBROWSER_SKIP_MODIFYING_TILE_PRIORITY)
  if(accumulated_scroll_delta > (640 / device_scale_factor_)){
  	SetSkipModifyingTilePriority(false);
  } else {
  	accumulated_scroll_delta += std::abs(applied_delta.Length());
  }
#endif
  ...
  if (did_scroll) {
    ...
    client_->RenewTreePriority();
#if defined(SBROWSER_GENERATING_FAKE_SCROLL_EVENT)
    m_fakeScrollUpdateEvent= applied_delta;
    fake_viewport_point = viewport_point;
#endif
  }

#if defined(SBROWSER_FLING_END_NOTIFICATION_CANE) || defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
    if(!scroll_page_end){
      bool did_overscroll = !gfx::ToRoundedVector2d(cached_pending_delta).IsZero();
      if(did_overscroll && !did_scroll){
        scroll_page_end = true;
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
	 DeferCommitsOnGesture(false);
#endif
      }
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
//GlowEffect: Glow is not shown During Zoom-in/out 
if((event_type != LayerTreeHostImpl::GestureScrollUpdate) && settings_.enable_edge_gloweffect) {
    event_type = LayerTreeHostImpl::GestureScrollUpdate;
	return did_scroll;
}
if(block_glow_on_pinch && settings_.enable_edge_gloweffect)
    return did_scroll;
...
if (settings_.enable_edge_gloweffect) {
  ...
  if (did_scroll_y)
    ...
    gfx::Vector2dF  rounded_unused_root_delta = gfx::ToRoundedVector2d(unused_root_delta);
    bool did_overscroll = !rounded_unused_root_delta.IsZero();
	
    //Condition while scrolling only in vertical direction
    if(did_scroll_y && !did_scroll_x && did_overscroll && ((rounded_unused_root_delta.x()>0 ||rounded_unused_root_delta.x()<0) && rounded_unused_root_delta.y()==0))
  	return did_scroll;
	
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
   // ChangeDeferCommitsStatusForGlow(did_scroll_x, did_scroll_y);
#endif
#if defined(SBROWSER_HIDE_URLBAR)
   if(settings_.enable_hide_urlbar){
		if(WebLayerY() < settings_.top_controls_height &&  WebLayerY() > 0){
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
			if(glow_effect_active_)
			    DeferCommitsOnGesture(false);
#endif
			client_->DropEdgeGlowEffect(LayerTreeHostImpl::AXIS_XY);
			return did_scroll;
		}
	}
#endif	
    accumulated_root_overscroll_y_current = gfx::ToCeiledInt(accumulated_root_overscroll_.y());
    accumulated_root_overscroll_x_current = gfx::ToCeiledInt(accumulated_root_overscroll_.x());
    if( ((accumulated_root_overscroll_y_current ^ accumulated_root_overscroll_y_previous) < 0) ||
       (accumulated_root_overscroll_y_current == 0 && (accumulated_root_overscroll_y_previous > 0 || accumulated_root_overscroll_y_previous < 0)) ||
       ((accumulated_root_overscroll_x_current ^ accumulated_root_overscroll_x_previous) < 0) ||
       (accumulated_root_overscroll_x_current == 0 && (accumulated_root_overscroll_x_previous > 0 || accumulated_root_overscroll_x_previous < 0))	  
      ) {
	   // opposite direction
	   if(!did_overscroll && glow_effect_active_) {
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
		  DeferCommitsOnGesture(false);
#endif
		  client_->DropEdgeGlowEffect(LayerTreeHostImpl::AXIS_XY);
    ...
    if (did_overscroll) {
        const cc::CompositorFrameMetadata metadata = MakeCompositorFrameMetadata();
        set_hScrollbar_status = metadata.root_layer_size.width() != metadata.viewport_size.width();
        set_vScrollbar_status = metadata.root_layer_size.height() != metadata.viewport_size.height();
        if (active_tree_->root_layer() && active_tree_->root_layer()->render_surface())
            set_size_current = active_tree_->root_layer()->render_surface()->content_rect().size();
        accumulated_root_overscroll_y_previous = accumulated_root_overscroll_y_current;
	accumulated_root_overscroll_x_previous = accumulated_root_overscroll_x_current;
		float WebLayer_Y = top_controls_manager_ ? top_controls_manager_->content_top_offset() : 0.f;
        client_->OnOverscrollOnImplThread(accumulated_root_overscroll_, current_fling_velocity_, set_hScrollbar_status, set_vScrollbar_status, set_size_current, WebLayer_Y);
    }
}
#endif
  return did_scroll;
...
#if  defined(SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
    client_->setGestureStateOnImplThread(false);
#endif
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
  DeferCommitsOnGesture(false);
#endif
...
if (settings_.enable_edge_gloweffect) {
	block_glow_on_pinch = false;
	if(event_type != LayerTreeHostImpl::GestureScrollEnd)
	    event_type = LayerTreeHostImpl::GestureScrollEnd;
}	
#endif  
#if defined(SBROWSER_SCROLLBAR_ANIMATION_OPTIMIZATION)
  ...
  StartScrollbarAnimation(CurrentFrameTimeTicks());
   ...
   if(settings_.enable_hide_urlbar){
     if(urlbar_active_){
       client_->DidHideUrlBarEventOnImplThread(cc::SCROLL_END_NOTIFY, 0, 0);
       VLOG(0)<<"HideUrlBar: "<<__FUNCTION__<<" ScrollEnd, Notified App ";
     }     
     if(scroll_inprogress_) {
       scroll_inprogress_ = false;
     }
   }
#endif
...
#if defined(SBROWSER_SCROLLBAR_ANIMATION_OPTIMIZATION)
void LayerTreeHostImpl::incrcounter(){
	counter++;
}
void LayerTreeHostImpl::decrcounter(){
	counter--;
}
int LayerTreeHostImpl::getcounter(){
	return counter;
}
#endif/* SBROWSER_SCROLLBAR_ANIMATION_OPTIMIZATION */
...
#if defined(SBROWSER_GENERATING_FAKE_SCROLL_EVENT)
InputHandler::ScrollStatus LayerTreeHostImpl::FlingScrollBegin(double timeStampSeconds) {
  if (active_tree_->CurrentlyScrollingLayer()){
    TRACE_EVENT1("cc", "LayerTreeHostImpl::FlingScrollBegin","timeStampSeconds",timeStampSeconds);
    base::TimeTicks frame_time = base::TimeTicks::FromInternalValue(
	    timeStampSeconds * base::Time::kMicrosecondsPerSecond);
    ScrollBy(fake_viewport_point, m_fakeScrollUpdateEvent);
...
void LayerTreeHostImpl::FlingAnimationCompleted(){
  VLOG(0)<<"LayerTreeHostImpl::FlingAnimationCompleted Haptic scroll_page_end="<<scroll_page_end;
  client_->FlingAnimationCompletedOnImplThread(scroll_page_end);
  scroll_page_end=false;
}
#endif
...
if (settings_.enable_edge_gloweffect) {
    client_->DropEdgeGlowEffect(LayerTreeHostImpl::AXIS_XY);
	block_glow_on_pinch = true;
	if(event_type != LayerTreeHostImpl::GesturePinchBegin)
	    event_type = LayerTreeHostImpl::GesturePinchBegin;
}
#endif  
#if defined(SBROWSER_DEFER_COMMITS_ON_GESTURE)
  DeferCommitsOnGesture(true);
  ...
  #if  defined(SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
    client_->setGestureStateOnImplThread(pinch_gesture_active_);
#endif
#if defined(SBROWSER_BLOCK_SENDING_FRAME_MESSAGE)
  if (renderer_ && !need_to_update_frame_info_)
    renderer_ ->SetGestureInProgress(true);
...
if (settings_.enable_edge_gloweffect) {
    client_->DropEdgeGlowEffect(LayerTreeHostImpl::AXIS_XY);
	block_glow_on_pinch = true;
	if(event_type != LayerTreeHostImpl::GesturePinchUpdate)
		event_type = LayerTreeHostImpl::GesturePinchUpdate;
}	
#endif  
...
#if defined(SBROWSER_SUPPORT_GLOW_EDGE_EFFECT)
if (settings_.enable_edge_gloweffect) {
  if(event_type != LayerTreeHostImpl::GesturePinchEnd)
	    event_type = LayerTreeHostImpl::GesturePinchEnd;
}		
#endif  
  #if  defined(SBROWSER_PAUSE_ANIMATION_ON_SCROLL_ZOOM)
    client_->setGestureStateOnImplThread(pinch_gesture_active_);
...
#if defined(SBROWSER_HIDE_URLBAR)
void LayerTreeHostImpl::SetUrlBarActive(bool active, bool override_allowed) {
    if(settings_.enable_hide_urlbar){
      VLOG(0)<<"HideUrlBar: "<<__FUNCTION__<<" Enters Function: active: "<<active<<" overrideAllowed: "<<override_allowed;
      if(pending_urlbar_active_ != active || urlbar_active_ != active 
	  	|| urlbar_active_override_ != override_allowed) {
        VLOG(0)<<"HideUrlBar: "<<__FUNCTION__<<" Updating Pending Values: active: "<<active<<" overrideAllowed: "<<override_allowed;
        pending_urlbar_active_ = active;
	urlbar_active_override_ = override_allowed;
        sync_urlbar_active_ = true;
        if(active){
         pending_urlbar_update_ = true;
        }
        client_->SetNeedsRedrawOnImplThread();
      }
    }
...
float LayerTreeHostImpl::WebLayerY() {
  return top_controls_manager_ ? top_controls_manager_->content_top_offset() : 0.f;
}

float LayerTreeHostImpl::UrlBarLayerY() {
  return top_controls_manager_ ? top_controls_manager_->controls_top_offset() : 0.f;
}

void LayerTreeHostImpl::SendUrlBarActiveAckIfNeeded() {
  if(settings_.enable_hide_urlbar /*&& !scroll_inprogress_*/){
    if(sync_urlbar_active_ && urlbar_active_){
      sync_urlbar_active_ = false;
      client_->DidHideUrlBarEventOnImplThread(cc::URLBAR_ACTIVE_ACK, 0, 0);
      VLOG(0)<<"HideUrlBar: "<<__FUNCTION__<<" UrlBar Active, Notified App ";
    }
  }
}
#endif
...
void LayerTreeHostImpl::ChangeDeferCommitsStatusForGlow(bool did_scroll_x, bool did_scroll_y) {
    if(did_scroll_x || did_scroll_y){
        DeferCommitsOnGesture(true);
        return;
    }
    if(RootScrollLayer()->max_scroll_offset().y() > 0 && RootScrollLayer()->max_scroll_offset().x() == 0){
        if (!did_scroll_y && (RootScrollLayer()->scroll_offset().y()== 0 || RootScrollLayer()->scroll_offset().y()== RootScrollLayer()->max_scroll_offset().y()))
            DeferCommitsOnGesture(false);
        else
            DeferCommitsOnGesture(true);
    }else if(RootScrollLayer()->max_scroll_offset().x() > 0 && RootScrollLayer()->max_scroll_offset().y() == 0){
        if (!did_scroll_x && (RootScrollLayer()->scroll_offset().x()== 0 || RootScrollLayer()->scroll_offset().x()== RootScrollLayer()->max_scroll_offset().x()))
            DeferCommitsOnGesture(false);
        else
            DeferCommitsOnGesture(true);
    }else if(RootScrollLayer()->max_scroll_offset().x() > 0 && RootScrollLayer()->max_scroll_offset().y() > 0){
        if (!did_scroll_x && (RootScrollLayer()->scroll_offset().x()== 0 || RootScrollLayer()->scroll_offset().x()== RootScrollLayer()->max_scroll_offset().x()))
            DeferCommitsOnGesture(false);
        else if (!did_scroll_y && (RootScrollLayer()->scroll_offset().y()== 0 || RootScrollLayer()->scroll_offset().y()== RootScrollLayer()->max_scroll_offset().y()))
            DeferCommitsOnGesture(false);
        else
            DeferCommitsOnGesture(true);
    }else{
...
void LayerTreeHostImpl::setCommitBlock(bool blockState)
{
...
void LayerTreeHostImpl::setCommitBlockOnImplThread(bool blockState)
{
     client_->setCommitBlockOnImplThread(blockState);
}
#endif
}  // namespace cc
