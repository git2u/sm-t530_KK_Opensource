...
#if defined(SBROWSER_KEYLAG)
void Page::suspendJavaScript()
{
    // Old deferrer could block different object then new one.
    // We need to "reblock" all active object once again.
    if( m_deferrer )
        delete m_deferrer;
		   
    m_deferrer = new PageGroupLoadDeferrer(this, true);
   // LOG(INFO)<<"KeyLag: suspendJavaScriptt";   
}

void Page::resumeJavaScript()
{
   if( m_deferrer )
    {    		    
        delete m_deferrer;
   ...
   // LOG(INFO)<<"KeyLag: resumeJavaScript";
