...
class ClipboardMap {
 ...
 private:
  ...
  #if defined (SBROWSER_CLIPBOARDEX_IMPL)
  ScopedJavaGlobalRef<jobject> clipboard_manager_Ex;
  #endif
};
...
ClipboardMap::ClipboardMap() {
  ...
  ScopedJavaLocalRef<jobject> local_ref =
      Java_ClipboardEx_create(env, context);
	  DCHECK(local_ref.obj());
	  clipboard_manager_Ex.Reset(env, local_ref.Release());
#else
  ScopedJavaLocalRef<jobject> local_ref =
...
void ClipboardMap::Set(const std::string& format, const std::string& data) {
...
#if defined(SBROWSER_CLIPBOARDEX_IMPL)
    Java_ClipboardEx_setText(env, clipboard_manager_Ex.obj(), str.obj());
#else
  ...
  {
      // P130827-02658 : convert htpl type string due to emotion icon display issue
      ScopedJavaLocalRef<jstring> str = ConvertUTF8ToJavaString(env, data);
      //ScopedJavaLocalRef<jstring> str(
      //  env, env->NewStringUTF(data.c_str()));
      DCHECK(str.obj() && !ClearException(env));
      Java_ClipboardEx_setHTML(env,clipboard_manager_Ex.obj(),str.obj());
  }
  #endif
}
...
//Commented this to avoid setting null item to clipboard. As clipboard is Samsung 
//feature, we cannot clear clipboard before setting new item. 

//#if defined (SBROWSER_CLIPBOARDEX_IMPL) 
//  Java_ClipboardEx_setText(env, clipboard_manager_Ex.obj(), NULL);
//#else
...
void ClipboardMap::SyncWithAndroidClipboard() {
...
#if defined(SBROWSER_CLIPBOARDEX_IMPL)
  if (!Java_ClipboardEx_hasPlainText(env, clipboard_manager_Ex.obj())) {
    if (it != map_.end())
  ...
  ScopedJavaLocalRef<jstring> java_string =
      Java_ClipboardEx_getCoercedText(env, clipboard_manager_Ex.obj());
#else
...
// See clipboardEx_android_initialization.h for more information.
#if defined(SBROWSER_CLIPBOARDEX_IMPL)
bool RegisterClipboardExAndroid(JNIEnv* env) {
return RegisterNativesImpl(env);
}
#endif
