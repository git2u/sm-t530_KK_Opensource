...
const char* BookmarkCodec::kEditableKey = "editable";
const char* BookmarkCodec::kOperatorBookmarkKey = "operator_bookmark";
#endif
...
#if defined(SBROWSER_OPERATOR_BOOKMARKS)
      std::string editable = base::Int64ToString(node->Editable());
      value->SetString(kEditableKey, editable);
      std::string operator_bookmark = base::Int64ToString(node->OperatorBookmark());
      value->SetString(kOperatorBookmarkKey, operator_bookmark);
#endif
...
bool BookmarkCodec::DecodeNode(const DictionaryValue& value,
  ...
  base::Time date_added = base::Time::FromInternalValue(internal_time);
    ...
    int64 editable; 
    std::string operator_bookmark_string;
    int64 operator_bookmark = 0;
 
    value.GetString(kEditableKey, &editable_string);
    if(!editable_string.empty())
      base::StringToInt64(editable_string, &editable);
    else
      editable = 1;
    
    value.GetString(kOperatorBookmarkKey, &operator_bookmark_string);
    base::StringToInt64(operator_bookmark_string, &operator_bookmark);
  ...
  if (type_string == kTypeURL) {
    ...
    node->set_operator_bookmark(operator_bookmark);
#endif
  node->SetTitle(title);
