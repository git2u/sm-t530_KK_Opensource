...
RenderViewImpl::RenderViewImpl(RenderViewImplParams* params)
    ...
    fpAuthStatusCalled_(false),
     ...
     m_private_ime_options(0)
#endif
 {
#if defined(SBROWSER_OVERVIEW_MODE)
	gfx::DeviceDisplayInfo info;
	if(device_dpi_scale_ < 0) 
		device_dpi_scale_ = info.GetDIPScale();
#endif

#if defined(SBROWSER_PRINT_IMPL)
	print_begin_called_ = false;
#endif
...
// ------------------------- kikin Modification Starts -------------------------------
       ...
       IPC_MESSAGE_HANDLER(ViewMsg_GetSelectionContext,
                                               OnGetSelectionContext)
       IPC_MESSAGE_HANDLER(ViewMsg_UpdateSelection,
                                               OnUpdateSelection)
       IPC_MESSAGE_HANDLER(ViewMsg_RestoreSelection,
                                               OnRestoreSelection)
       IPC_MESSAGE_HANDLER(ViewMsg_ClearSelection,
                                               OnClearSelection)
#endif
// ------------------------- kikin Modification Ends -------------------------------

#if defined(SBROWSER_SAVEPAGE_IMPL)   
    IPC_MESSAGE_HANDLER(ViewMsg_GetScrapBitmap, OnGetScrapBitmap)
    IPC_MESSAGE_HANDLER(ViewMsg_GetBitmapFromCachedResource, OnGetBitmapFromCachedResource)
#endif
	...
	IPC_MESSAGE_HANDLER(ViewMsg_getSelectionVisiblity,OnGetSelectionVisibleRequest)
	IPC_MESSAGE_HANDLER(ViewMsg_getSelectionrect,onGetCurrentSelectionRect)
#endif
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_ScrollEditWithCursor,OnScrollEditWithCursor)
#endif
	...
	IPC_MESSAGE_HANDLER(ViewMsg_PasswordEchoEnabled,OnSetPasswordEcho)
#endif
 
// SAMSUNG : SmartClip >>
#if defined(SBROWSER_SMART_CLIP)  
    IPC_MESSAGE_HANDLER(ViewMsg_RequestSmartClipData, OnSmartClipDataRequest)
#endif
// SAMSUNG : SmartClip <<
...
#if  defined(SBROWSER_FORM_NAVIGATION) 
	IPC_MESSAGE_HANDLER(ViewMsg_MoveToPrev,OnMovePrev)
	IPC_MESSAGE_HANDLER(ViewMsg_MoveToNext,OnMoveNext)
#endif
#if defined(SBROWSER_PRINT_IMPL) 
	IPC_MESSAGE_HANDLER(ViewMsg_PrintBegin, PrintBegin)
	IPC_MESSAGE_HANDLER(ViewMsg_PrintPage, PrintPage)
	IPC_MESSAGE_HANDLER(ViewMsg_PrintEnd, PrintEnd)
#endif
...
#if defined(SBROWSER_FPAUTH_IMPL)
	IPC_MESSAGE_HANDLER(ViewMsg_FPAuthStatus, FPAuthStatus)
#endif

#if defined (SBROWSER_SOFTBITMAP_IMPL)
        IPC_MESSAGE_HANDLER(ViewMsg_ReCaptureSoftBitmap, OnReCaptureSoftBitmap)
#endif
...
#if defined(SBROWSER_UA_NEW_WINDOW_FROM_JS)
	setUserAgentOverride(params.is_overriding_user_agent);
  #endif
...
#if defined(SBROWSER_HOVER_HIGHLIGHT)
void RenderViewImpl::OnHoverHighLight(const WebKit::WebInputEvent* hover_event,bool high_light){
...
void RenderViewImpl::OnDirectPaste(const string16& text) {
  if (!webview())
      return;
...
void RenderViewImpl::OnSelectAll() {
  if (!webview())
    return;
  //Webkit does not paint selection colour When composition ranges overlap with selection ranges.Please refer to webviewImpl::SetComposition
  //sending an OnImeConfirmComposition call with an empty string to make sure that composition mode is end.
#if defined(SBROWSER_PLM_P131231_00002)
  ui::Range range(ui::Range::InvalidRange());
  OnImeConfirmComposition(string16(),range);
#endif
...
#if defined (SBROWSER_ENABLE_ECHO_PWD)
void RenderViewImpl::OnSetPasswordEcho(bool pwdEchoEnabled){
	webview()->SetPasswordEchoEnabled(pwdEchoEnabled);
}
#endif
...
// --------------------------- kikin Modification starts ---------------------------
#if defined (SBROWSER_KIKIN)
void RenderViewImpl::OnGetSelectionContext(const bool shouldUpdateSelectionContext)
{
       // Fetching selection context from WebFrame:
       std::map<std::string, std::string> selectionContext;
       webview()->mainFrame()->getSelectionContext(shouldUpdateSelectionContext, &selectionContext);

       // Sending message with extracted selection context:
       Send(new ViewHostMsg_SelectionContextExtracted(routing_id_, selectionContext));
}

void RenderViewImpl::OnUpdateSelection(const std::string& oldSelection, const std::string& newSelection)
{
       webview()->focusedFrame()->updateSelection(oldSelection, newSelection);
}

void RenderViewImpl::OnRestoreSelection()
{
  webview()->mainFrame()->restoreSelection();
  SyncSelectionIfRequired();
}

void RenderViewImpl::OnClearSelection()
{
  webview()->mainFrame()->clearSelection();
}
#endif
// ---------------------------  kikin Modification ends  ---------------------------
...
void RenderViewImpl::UpdateURL(WebFrame* frame) {
  ...
  if (!frame->parent()) {
    ...
    webview()->adjustWorkerThreadPriority(params.url.spec());
#endif
...
WebView* RenderViewImpl::createView(
  ...
  if (!request.isNull())
  {
    ...
    params.target_href = last_touch_link_href_;
    last_touch_link_href_ = "";
  }
...
#if defined(SBROWSER_FORM_NAVIGATION) 
//Added to Show Keypad at the end of performing Prev/Next IME Option
void RenderViewImpl::performMouseClick(int x, int y){
	//SLOG(INFO) <<__FUNCTION__<<"x : " << x <<" y : "<<y;
	SendTouchEndAck();
}
...
WebExternalPopupMenu* RenderViewImpl::createExternalPopupMenu(
  ...
  if (external_popup_menu_)
    return NULL;
  #if defined(SBROWSER_FORM_NAVIGATION)
  WebPopupMenuInfo& newPopupMenuInfo = const_cast<WebPopupMenuInfo&>(popup_menu_info);
  //newPopupMenuInfo.privateIMEOptions = webview()->getIMEOptions();
  newPopupMenuInfo.privateIMEOptions = m_private_ime_options;
  //SLOG(INFO) <<__FUNCTION__<<"setting privateimeoptions as "<<newPopupMenuInfo.privateIMEOptions;
#endif
...
//API added to avoid invalid popup_menu_client pointer access 
#if defined(SBROWSER_FORM_NAVIGATION)
void RenderViewImpl::destroyExternalPopupMenu(
    ...
    if(external_popup_menu_->IsSamePopupMenuClient(popup_menu_client)) {
	    ...
	    external_popup_menu_.reset();
	    //hide ui should have happened from focus node change
    }
...
//m.edaily.co.kr -> Page is loaded when the Ad content in the page is changed. In this scenario, handlers are hidden but
//selection is visible. Clearing the selection and behavior is same as Chrome.
#if defined (SBROWSER_PLM_P130731_5016) && defined(SBROWSER_TEXT_SELECTION)
  WebFrame* main_frame = webview()->mainFrame();
  if(main_frame != NULL)
    main_frame->OnClearTextSelection();
#endif
...
// SAMSUNG CHANGE : focus url is empty by urlbarview is hided >>
// This is for showed autoShowedUrlbar
#if defined(SBROWSER_HIDE_URLBAR)
  if(focus_url_.is_empty())
      return;
#endif
// SAMSUNG CHANGE : focus url is empty by urlbarview is hided <<
...
void RenderViewImpl::setFontBoostingModeEnabled(bool enable) {
      Send(new ViewHostMsg_SetFontBoostingModeEnabled(routing_id_, enable));
}
#endif // SBROWSER_FONT_BOOSTING_PREFERENCES
// SAMSUNG_CHANGE : FontBoosting mode
#if defined(SBROWSER_BINGSEARCH_ENGINE_SET_VIA_JAVASCRIPT)
void RenderViewImpl::setBingSearchEngineAsDefaultSearch(bool enable){  
      Send(new ViewHostMsg_SetBingSearchEngineAsDefault(routing_id_,enable));
}
#endif
...
#if defined(SBROWSER_FORM_NAVIGATION)
  m_private_ime_options = 0;
  if (webview() && (IsEditableNode(node) || webview()->isFocusedNodeSelectInput())) 
    m_private_ime_options = webview()->getIMEOptions();
...
#if defined(SBROWSER_OVERVIEW_MODE)
bool RenderViewImpl::getIsReload() {return m_isreload;}
#endif
...
void RenderViewImpl::DidKeyDownEvent() {
  int delay = 180;
  base::TimeTicks now;
  ...
  now = base::TimeTicks::NowFromSystemTraceTime();

  if (check_deferred_input_timer_.IsRunning()) {
  	//LOG(INFO) << __FUNCTION__<<" RenderViewImpl::timer(reset) "<<(now -pre_down_time).InSecondsF();
	if((now -pre_down_time).InSecondsF() > 0.08f)
            SetUpdateStyleIfNeeded(false);
	else
	    SetUpdateStyleIfNeeded(true);
    check_deferred_input_timer_.Reset();
  }else{
	SetUpdateStyleIfNeeded(true);
	//LOG(INFO) << __FUNCTION__<<" RenderViewImpl::timer(start)";
	check_deferred_input_timer_.Start(FROM_HERE,
                        TimeDelta::FromMilliseconds(delay), this,
                        &RenderViewImpl::ResetUpdateStyleIfNeeded);
  }
  pre_down_time = now;
}

void RenderViewImpl::ResetUpdateStyleIfNeeded() {
    SetUpdateStyleIfNeeded(true);
}

void RenderViewImpl::SetUpdateStyleIfNeeded(bool updateStyleIfNeeded) {
  if (webview() && webview()->mainFrame())
    webview()->mainFrame()->document().setUpdateStyleIfNeeded(updateStyleIfNeeded);
...
#if defined (SBROWSER_SOFTBITMAP_IMPL)
	 if(/* is_hidden() &&*/ !frame->parent()) { // power consumption is more - so capturing only when main frame is fully loaded 
	  // inside chrome_render_view_observer.cpp, if page contains only some 3d content then only allow bitmap capture else ignore
	   // so no need of is hidden checks.
	   if(has_focus()) //paintFullImage for foreground tab
		  FOR_EACH_OBSERVER(RenderViewObserver, observers_, DidChangeLoadProgress(true));
	   else //paintRootImage for background tabs
		  FOR_EACH_OBSERVER(RenderViewObserver, observers_, DidChangeLoadProgress(false));
	 }
 #endif
...
#if defined(SBROWSER_UA_NEW_WINDOW_FROM_JS)
void RenderViewImpl::setUserAgentOverride(bool shouldOverride){
	if (!webview() || !webview()->mainFrame())
		return;
	...
	if (internal_data)
		  internal_data->set_is_overriding_user_agent(shouldOverride);
}
#endif
...
void RenderViewImpl::EvaluateScript(const string16& frame_xpath,
  ...
  if (notify_result) {
	...
	VLOG(0)<<"recognizeArticle javascript Time : "<<(1000.0*((double)(end - begin)/CLOCKS_PER_SEC))<<"ms";
#endif
...
void RenderViewImpl::detectLinkOnTouch( const WebElement& url_element )
{
  last_touch_link_href_ = "";
  ...
  {
      //WebString name = element.tagName();
      //if( name.length() != 0 )
        //LOG(INFO)<< "AddNewContents" << name.utf8().data();
      WebString hrefstr = element.getAttribute( "href" ) ;
      if( hrefstr.length() != 0 )
      {
        last_touch_link_href_ = hrefstr.utf8().data();
        break;
      }    
      element = element.parentNode().to<WebElement>();
  }
    
  //LOG(INFO)<< "RenderViewImpl::detectLinkOnTouch " << last_touch_link_href_;
...
void RenderViewImpl::OnSelectPopupMenuItems(
  ...
  if (!external_popup_menu_)
    return;
  #if defined(SBROWSER_FORM_NAVIGATION)
	  if(!webview()->isFocusedNodeSelectInput()){
		  //LOG(INFO)<<__FUNCTION__<<" cancel popup and return because focused node is not select input";
		  external_popup_menu_->DidSelectItems(true, selected_indices); // MPSG100010168 - required to clear state
		  external_popup_menu_.reset();
		  return;
	  }
...
#if defined(SBROWSER_POPUP_ZOOMER)
...
{
	SkBitmap SelectedRegion;
	webview()->OnGetSelectionBitmap(&SelectedRegion);
	Send(new ViewHostMsg_SelectedBitmap(routing_id_,SelectedRegion));
}
void RenderViewImpl::OnSelectClosestWord(int x , int y)
{
	...
	//CQ:MPSG100011296--
	WebFrame* main_frame = webview()->mainFrame();
	main_frame->OnSelectClosestWord(x,y);
}
void RenderViewImpl::OnCheckBelongToSelection(int x, int y)
{
	bool rv = false;
	WebFrame* main_frame = webview()->mainFrame();
	rv = main_frame->PointOnRegion( x, y);
	Send(new ViewHostMsg_PointOnRegion(routing_id_,rv));
}

void RenderViewImpl::OnGetSelectionVisibleRequest(){
     bool isSelectionVisible = webview()->IsSelectionWithinVisibleRect();
     Send( new ViewHostMsg_setSelectionVisible(routing_id_,isSelectionVisible));
}
void RenderViewImpl::onGetCurrentSelectionRect(){
	WebRect selectionrect = webview()->currentSelectionRect();
	gfx::Rect rect = gfx::Rect(selectionrect.x, selectionrect.y, selectionrect.width, selectionrect.height);
	Send( new ViewHostMsg_updateSelectionRect(routing_id_,rect));
}
#endif

#if defined(SBROWSER_TABLET_SWITCH)
void RenderViewImpl:: TabletSwitchEnabled()
{
    if (!webview())
		return;
    ...
    webview()->SetTabletModeEnabled(isTablet);
}
#endif

#if defined(SBROWSER_FP_SUPPORT)
void RenderViewImpl::OnNotifyAutofillBeginMain()
{
	Send(new ViewHostMsg_NotifyAutofillBegin(routing_id_));
}
#endif

#if defined (SBROWSER_SCROLL_INPUTTEXT)
void RenderViewImpl::OnScrollEditWithCursor(int scrollX)
{
    webview()->ScrollEditWithCursor(scrollX);
}

void RenderViewImpl::updateTextFieldBounds(WebKit::WebRect inputFieldBounds,bool isScrollEnable,bool focusOn)
{
	if(isScrollEnable || focusOn){
        gfx::Rect input_rect = gfx::Rect(inputFieldBounds.x, inputFieldBounds.y, inputFieldBounds.width, inputFieldBounds.height);
        Send(new ViewHostMsg_UpdateTextFieldBounds(routing_id_, input_rect, isScrollEnable));
	}
}
#endif

#if defined (SBROWSER_HANDLE_MOUSECLICK_CTRL)
void RenderViewImpl::OpenUrlInNewTab(const string16& mouseClickUrl) {
...
void RenderViewImpl::OnGetSelectionMarkup()
{
	...
	string16 markup = main_frame->selectionAsMarkup();
	Send(new ViewHostMsg_SelectedMarkup(routing_id_, markup));
}
#endif

#if defined (SBROWSER_MULTIINSTANCE_DRAG_DROP)
void RenderViewImpl::OnHandleSelectionDrop(int x, int y, const string16& text) {
...
// SAMSUNG : SmartClip >>
#if defined(SBROWSER_SMART_CLIP)  
void RenderViewImpl::OnSmartClipDataRequest(int x, int y, int w, int h) 
{
    LOG(INFO) << "SmartClip : OnSmartClipDataRequest called";

    string16 result;

    // Traverse the DOM tree
    WebString cropResult = webview()->extractSmartClipData(WebRect(x,y,w,h));
...
void RenderViewImpl::OnRecognizeArticle(int mode) {
	...
	begin = clock();
#endif
	bool isArticle = webview()->mainFrame()->RecognizeArticle(mode);
#if defined(SBROWSER_READER_DEBUG)
	end = clock();
	VLOG(0)<<"recognizeArticle native Time : "<<(1000.0*((double)(end - begin)/CLOCKS_PER_SEC))<<"ms";
#endif

    Send(new ViewHostMsg_OnRecognizeArticleResult(routing_id_, isArticle));
}
#endif
// SAMSUNG: Reader <<

#if defined(SBROWSER_FORM_NAVIGATION) 
void RenderViewImpl::OnMoveNext(){
	webview()->moveFocusToNext();
}

void RenderViewImpl::OnMovePrev(){
	webview()->moveFocusToPrev();
}
#endif
                                         ...
                                         const std::string& historyurl)
{
    ...
    if(main_frame)
        main_frame->loadData(WebData(data.data(),data.size())
                             ...
                             ,WebURL(GURL(baseurl.data()))
                             ,WebURL(GURL(historyurl.data())),true);
}
#endif

#if defined (SBROWSER_SOFTBITMAP_IMPL)
void RenderViewImpl::OnReCaptureSoftBitmap(int x, int y, int reqHeight, float pageScaleFactor, bool fromPageLoadFinish) { 
    WebView* view = webview();
    SkBitmap errBitmap;
    if (!view) {
      Send(new ViewHostMsg_UpdateReCaptureSoftBitmap(routing_id_, errBitmap, fromPageLoadFinish));
      return;
    }

    if (!view->isSoftwareRenderable()) {
       LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap WEBPAGE Contants 3D Contents, so return";
       Send(new ViewHostMsg_UpdateReCaptureSoftBitmap(routing_id_, errBitmap, fromPageLoadFinish));
       return;
    }
    //layout
    ...
    if (size.width == 0 || size.height == 0) {
    	LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap NOT A VALID WEBVIEW = VIEW WIDTH"<<size.width;
    	LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap NOT A VALID WEBVIEW = VIEW HEIGHT"<<size.height; 
        Send(new ViewHostMsg_UpdateReCaptureSoftBitmap(routing_id_, errBitmap, fromPageLoadFinish));
        return;
    }
    ...
    if (!view->mainFrame()) {
       LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap MainFrame NULL, so return";
       Send(new ViewHostMsg_UpdateReCaptureSoftBitmap(routing_id_, errBitmap, fromPageLoadFinish));
       return;
    }
    ...
    if (height > contentSize.height) {
       LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap Content Height is smaller than requested";
       height = contentSize.height;
    }
    skia::RefPtr<SkCanvas> canvas = skia::AdoptRef(
			skia::CreatePlatformCanvas(
                     size.width*deviceScale,  height*deviceScale, true, NULL, skia::RETURN_NULL_ON_FAILURE));		
    if (!canvas) {
        LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap CANVAS NULL, so return";
        Send(new ViewHostMsg_UpdateReCaptureSoftBitmap(routing_id_, errBitmap, fromPageLoadFinish));
	return;
    }
  
    if (pageScaleFactor == -1 || fromPageLoadFinish) {
        LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap Considering ENGINE PAGE SCALE FACTOR";
        pageScaleFactor = view->pageScaleFactor();
        WebSize offset = view->mainFrame()->scrollOffset();
        x = offset.width;
        y = offset.height; 
    }
     //apply scale factor
    (canvas.get())->scale(deviceScale*pageScaleFactor, deviceScale*pageScaleFactor);
     //translate page to scroll offset to map with screen
    (canvas.get())->translate(-x, -y);
     //clear canvas to avoid black screen
    (canvas.get())->drawARGB(255, 255, 255, 255);
    //draw canvas --frame view paint for root layer
    view->paintImage(webkit_glue::ToWebCanvas(canvas.get()),
	      WebRect(0, 0, contentSize.width, contentSize.height));

    if (!view->mainFrame()->isSoftBmpCaptured()) {
       LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap FrameView::PaintContents failed, so return";
       Send(new ViewHostMsg_UpdateReCaptureSoftBitmap(routing_id_, errBitmap, fromPageLoadFinish));
       return;
    }
    //get canvas device to access bitmap
    SkDevice* device = skia::GetTopDevice(*canvas);
    //get bitmap ARGB8888
    const SkBitmap& bitmap = device->accessBitmap(false);
    SkBitmap snapshot;
    //copy bitmap to RGB565
    if (!bitmap.copyTo(&snapshot, SkBitmap::kRGB_565_Config)) {
         LOG(INFO)<<"Chandra::RenderViewImpl::OnReCaptureSoftBitmap BITMAP COPY FAILED, so return";
	 return;
    }
           
     //send host message with updated soft bitmap
    Send(new ViewHostMsg_UpdateReCaptureSoftBitmap(routing_id_, snapshot, fromPageLoadFinish));
}
#endif

#if defined (SBROWSER_SAVEPAGE_IMPL)
typedef enum {
    ESCRAP_BITMAP_FAIL = 0,
    ESCRAP_BITMAP_SUCCESS
}ESCRAP_BITMAP_RESPONSE;

void RenderViewImpl::OnGetScrapBitmap(int bitmapWidth, int bitmapHeight) { 

    LOG(INFO) << " RenderViewImpl::OnGetScrapBitmap";
    ESCRAP_BITMAP_RESPONSE response = ESCRAP_BITMAP_SUCCESS;
    SkBitmap snapshot;

    if (!webview())
        response = ESCRAP_BITMAP_FAIL;
    else {
        //construct canvas and rect and pass to webkit to paint
        webview()->layout();
        float scale =webview()->pageScaleFactor();
	 float inverseScale = 1/scale;
	 
        skia::RefPtr<SkCanvas> canvasMain = skia::AdoptRef(
		 skia::CreatePlatformCanvas(
			 ceil(bitmapWidth/scale), ceil(bitmapHeight/scale), true, NULL, skia::RETURN_NULL_ON_FAILURE));
	 if (!canvasMain)
	 	 response = ESCRAP_BITMAP_FAIL;
	...
	if (response == ESCRAP_BITMAP_SUCCESS) {
	//  if (!canvas.initialize(bitmapWidth/scale, MAX_HEIGHT/scale, true))
	//	   response = ESCRAP_BITMAP_FAIL;
	skia::RefPtr<SkCanvas> canvas = skia::AdoptRef(
		 skia::CreatePlatformCanvas(
			 ceil(bitmapWidth/scale), ceil(MAX_HEIGHT/scale), true, NULL, skia::RETURN_NULL_ON_FAILURE));
	   if (!canvas)
	 	 response = ESCRAP_BITMAP_FAIL;
	  else {
		  (canvas.get())->scale(inverseScale,inverseScale); //draw contents at scale 1
		  do{
		   if(height > MAX_HEIGHT) {
		   	remaining=height - MAX_HEIGHT;
		   	height = MAX_HEIGHT;
		   }
		   else
		   	remaining =0;
		   
		   //Draw clips of webview to canvas of clip size
		   (canvas.get())->save();
		   (canvas.get())->translate(-x,-y);
		   SkRect clip = {x, y, x+width,y+ height};
		   (canvas.get())->clipRect(clip);
	          webview()->paintImage(webkit_glue::ToWebCanvas(canvas.get()), WebRect(x, y, width, height));  
		   (canvas.get())->restore();
		   SkDevice* device = skia::GetTopDevice(*canvas);
		   const SkBitmap& bitmap = device->accessBitmap(false);

		   //copy canvas bitmap to final canvas
		   (canvasMain.get())->save();
		   (canvasMain.get())->translate(ceil(x/scale),ceil(y/scale));
		   (canvasMain.get())->drawBitmap(bitmap,0,0);
		   (canvasMain.get())->restore();

		   y=y+height;
		   height = remaining;
		   }while(remaining >0);
		        SkDevice* device = skia::GetTopDevice(*canvasMain);
	    	    const SkBitmap& bitmap = device->accessBitmap(false);
    ...
    Send(new ViewHostMsg_OnBitmapForScrap(routing_id_, snapshot, response));
}
void RenderViewImpl::OnGetBitmapFromCachedResource(const std::string imageUrl){
	SkBitmap bitmap;
    webview()->getBitmapFromCachedResource(imageUrl,&bitmap);
...
#endif
#if defined(SBROWSER_FPAUTH_IMPL)
void RenderViewImpl::FPAuthStatus(bool enabled, std::string usrName){
LOG(INFO)<<"FP: FPAuthStatus called";
     if(usrName.size() != 0){
        LOG(INFO)<<"FP: FPAuthStatus DidFinishDocumentLoad";
        WebFrame* main_frame = webview()->mainFrame();
         FOR_EACH_OBSERVER(RenderViewObserver, observers_,
	                     DidFinishDocumentLoad(main_frame));
	  if(main_frame)
	      main_frame->CheckFramesWithForms();	
	  if(enabled)
	    fpAuthStatusCalled_ = true;
     }
}
void RenderViewImpl::TriggerDidFinishDocumentLoad( WebKit::WebFrame*  frame)
{

	if(frame){
    	LOG(INFO)<<"FP: FPAuthStatus TriggerDidFinishDocumentLoad ";
	    FOR_EACH_OBSERVER(RenderViewObserver, observers_,
						 DidFinishDocumentLoad(frame));
	}
}

void RenderViewImpl :: SubmitForm(WebKit::WebInputElement& pwd){
	LOG(INFO)<<"FP: SubmitForm called fpAuthStatusCalled_ = "<<fpAuthStatusCalled_;
	if(fpAuthStatusCalled_){
	  LOG(INFO)<<"FP: SubmitForm called";
	  webview()->generateEnterEvent(pwd);
	  fpAuthStatusCalled_= false;
	}
}
...
void RenderViewImpl::PrintBegin(){

	WebFrame* main_frame = webview()->mainFrame();
    LOG(INFO)<<"PrintBegin Called";
	//These are settings for 72  DPI A4 size paper.
	int pageWidth = 540;
	int pageHeight = 795;
	...
	if(main_frame){
		 	WebKit::WebPrintParams webkit_params(WebSize(pageWidth,pageHeight));
		 	WebNode constrainToNode;
			page_count = main_frame->printBegin(webkit_params,constrainToNode, NULL);
	}
	print_begin_called_ = true;
	Send(new ViewHostMsg_OnPrintBegin( routing_id_, page_count));
}

void RenderViewImpl::PrintPage(int pageNum){

	//The resulting bitmap height and width
	// This is the maximum bitmap size we can set.
	int ScaledPageWidth = 1620;
	int ScaledPageHeight = 2385;
    
	LOG(INFO)<<"PrintPage :: Creating BitMap";
	skia::RefPtr<SkCanvas> canvas = skia::AdoptRef(
		skia::CreatePlatformCanvas(
				ScaledPageWidth ,ScaledPageHeight , true, NULL, skia::RETURN_NULL_ON_FAILURE));
	if (!canvas)
		return;
	...
	(canvas.get())->scale(3,3);
	main_frame->printPage(pageNum,webkit_glue::ToWebCanvas(canvas.get()));
	...
	if (!bitmap.copyTo(&snapshot, SkBitmap::kARGB_8888_Config)){
		 LOG(INFO)<<"PrintPage :: False Bitmap";
		 Send(new ViewHostMsg_OnBitmapForPrint( routing_id_, snapshot, -1));
		 return;
	}
	
	LOG(INFO)<<"PrintPage :: Sending the bitmap back :: "<<"SKbitMap :: "<< &snapshot;
	Send(new ViewHostMsg_OnBitmapForPrint( routing_id_, snapshot, 0));
}


void RenderViewImpl::PrintEnd(){

	WebFrame* main_frame = webview()->mainFrame();

	if(main_frame){
		if(print_begin_called_){
			main_frame->printEnd();
			print_begin_called_ = false;
		}
	LOG(INFO)<<"PrintPage :: Ending Print Page";
	}
}
