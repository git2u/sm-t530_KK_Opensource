...
class AndroidBrowserBackend(browser_backend.BrowserBackend):
  ...
  def __init__(self, options, adb, package, is_content_shell,
    ...
    self._port = 9222#adb_commands.AllocateTestServerPort()
    ...
    if is_content_shell:
      ...
      self._profile_dir += 'app_sbrowser/' # chrome/'
    ...
    if is_content_shell:
      ...
      #pseudo_exec_name = 'sbrowser' #'chrome'
    ...
    #Check for SBrowser
    if 'sbrowser' in self._options.browser_type:
    #Check if cmdline_file exists
      out = self._adb.RunShellCommand('su -c ls %s'% self._cmdline_file);
      if "No such file or directory" in out[0]:
        self._adb.RunShellCommand('su -c mkdir %s'% ('/data/data/'+self._package+'/config'))
        self._adb.RunShellCommand('su -c touch %s'% self._cmdline_file);
        self._adb.RunShellCommand('echo "sbrowser --enable-gpu-benchmarking" > %s '% self._cmdline_file);
    ...
    if 'sbrowser' in self._options.browser_type:
      ...
      try:
        with open(os.devnull, 'w') as devnull:
          proc = subprocess.Popen(['adb', 'shell', 'ps', '|', 'grep', '-e', self._package, '|', 'grep', '-v', 'sandboxed'],
          ...
          pid = (stdout.split())[1]
          #self._devtools_remote_port = self._devtools_remote_port + pid
          self._devtools_remote_port = self._devtools_remote_port
          self._adb.Forward('tcp:%d' % self._port, self._devtools_remote_port)
      except OSError:
        print "Could not get PID of Portable Android Browser"
