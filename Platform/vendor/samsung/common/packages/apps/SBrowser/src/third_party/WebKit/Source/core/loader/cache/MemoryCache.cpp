...
#if defined(SBROWSER_DATA_COMPRESSION)
void MemoryCache::evictCompressedImages(int image_quality)
{
    Vector<CachedResource*> to_evict;

    for (CachedResourceMap::iterator i = m_resources.begin(); i != m_resources.end(); ++i) {
        int used_image_quality;
        if (i->value->response().isTurboCompressedImage(&used_image_quality) && used_image_quality < image_quality)
            to_evict.append(i->value);
    }

    for (Vector<CachedResource*>::iterator i = to_evict.begin(); i != to_evict.end(); ++i)
        evict(*i);
}
#endif
