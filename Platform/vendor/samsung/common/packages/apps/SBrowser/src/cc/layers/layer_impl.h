...
class CC_EXPORT LayerImpl : LayerAnimationValueObserver {
 public:
  ...
  void SetWebRoot(bool root);
  bool web_root() const { return web_root_; }
#endif
...
#if defined(SBROWSER_HIDE_URLBAR)
  void DumpLayerPropertiesEx(std::string* str, int indent);
  void DumpLayerEx(std::string* str, int indent) ;
#endif
