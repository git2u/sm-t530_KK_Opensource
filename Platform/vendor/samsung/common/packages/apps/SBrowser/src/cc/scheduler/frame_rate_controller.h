...
class CC_EXPORT FrameRateController {
 public:
  enum {
#if defined(SBROWSER_DEFAULT_MAX_FRAMES_PENDING)
    DEFAULT_MAX_FRAMES_PENDING = 3
#else
...
#if defined(SBROWSER_QC_VSYNC_PATCH)
  explicit FrameRateController(scoped_refptr<TimeSource> timer, Thread* = 0);
#else
 ...
 protected:
  ...
  void ManualTick();
#if defined(SBROWSER_QC_VSYNC_PATCH)
  void chooseManualOrTimeSourceThrottling();  
#endif
...
#if defined(SBROWSER_QC_VSYNC_PATCH)
  bool force_manual_tick_;
  double max_fps_;
  scoped_ptr<FPSAverage> fps_average_;  
#endif
  Thread* thread_;
