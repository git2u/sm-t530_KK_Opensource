...
void CommandBufferProxyImpl::SendManagedMemoryStats(
 ...
 void CommandBufferProxyImpl::SendFileDescriptors(
		 int32 texid, int32 fd_count, int32* fds) {

 	IPC::Message* msg = new GpuCommandBufferMsg_SendFileDescriptors(route_id_, texid);

 	// find out number of zero FDs
 	uint32 zero_count = 0;
 	...
 	// write zero FD count to message
 	msg->WriteUInt32(zero_count);

 	// write zero FD indices to message
 	for (int i = 0; i < fd_count; i++)
	{
		if (fds[i] == 0)
			msg->WriteUInt32((uint32)i);
	}
 	...
 	for (int i = 0; i < fd_count; i++)
 	{
 		if (fds[i] != 0)
 		{
			base::FileDescriptor temp_fd;
			temp_fd.fd = fds[i];
			msg->WriteFileDescriptor(temp_fd);
 		}
 	}
