...
void ExternalPopupMenu::show(const WebKit::WebRect& bounds) {
  ...
  popup_params.allow_multiple_selection = popup_menu_info_.allowMultipleSelection;
   ...
   popup_params.privateimeoptions = popup_menu_info_.privateIMEOptions;
  #endif
  render_view_->Send(
...
#if defined(SBROWSER_FORM_NAVIGATION)
bool ExternalPopupMenu::IsSamePopupMenuClient(WebKit::WebExternalPopupMenuClient* popup_menu_client) {
  if (!popup_menu_client_)
    return false;
  if (popup_menu_client_ == popup_menu_client)
    return true;
  else
    return false;
}
