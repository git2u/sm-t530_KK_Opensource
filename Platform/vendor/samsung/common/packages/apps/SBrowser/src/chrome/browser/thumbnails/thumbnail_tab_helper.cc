...
void UpdateThumbnail(const ThumbnailingContext& context,
	 ...
	 VLOG(0) << "LOG: Thumbnail " << " score to string = " << context.score.ToString();
#else
	 VLOG(0) << "LOG: Thumbnail taken for " << context.url << " score to string = "
...
void ThumbnailTabHelper::UpdateThumbnailEx(scoped_refptr<thumbnails::ThumbnailingContext> context,
    const SkBitmap& thumbnail) {
  VLOG(0) << " LOG: " << __FUNCTION__;
  Profile* profile =
      Profile::FromBrowserContext(web_contents()->GetBrowserContext());
...
#else
  if(thumbnail_service == NULL ||
      !thumbnail_service->ShouldAcquirePageThumbanil(url))
#endif
  {
    VLOG(0) << " LOG: " << " Page is not suitable for adding again.";
    return;
  }

  scoped_refptr<thumbnails::ThumbnailingAlgorithm> algorithm(
      thumbnail_service->GetThumbnailingAlgorithm());
  algorithm->ProcessBitmap(context, base::Bind(&UpdateThumbnail), thumbnail);
}
