...
#        }],
    ...
    # Use system harfbuzz_ng instead of bundled one.
    'use_system_harfbuzz_ng%' : 0,
    ...
    'conditions': [
      ...
      #included sbr_gyp_defines.gypi to include samsung features - Mohan Reddy
      ['OS=="android"', {
      'includes': ['sbr/sbr_gyp_defines.gypi'],
      }],
...
#             'android_ndk_root%': '<!(cd <(DEPTH) && pwd -P)/third_party/android_tools/ndk/',
		  ...
		  'android_ndk_root%': '<!(cd <(DEPTH) && pwd -P)/third_party/android_tools/ndk_4.8/',
	      },
	      {
		  'android_ndk_root%': '<!(cd <(DEPTH) && pwd -P)/third_party/android_tools/ndk/',
	     }],
...
#          'android_libstdcpp_root': '<(android_ndk_root)/sources/cxx-stl/gnu-libstdc++/4.6',
          ...
          'conditions': [
	    ['enable_ndktoolchain_48 == 1', {
	          'android_libstdcpp_root': '<(android_ndk_root)/sources/cxx-stl/gnu-libstdc++/4.8',
  	     },
	     {
	          'android_libstdcpp_root': '<(android_ndk_root)/sources/cxx-stl/gnu-libstdc++/4.6',
	    }],
            ['target_arch == "ia32"', {
              ...
              'conditions': [
              ['enable_ndktoolchain_48 == 1', {
	              'android_ndk_sysroot%': '<(android_ndk_root)/platforms/android-19/arch-x86',
...
#	      'conditions': [
	      ['enable_ndktoolchain_48 == 1', {
	              'android_gdbserver%': '<(android_ndk_root)/prebuilt/android-arm/gdbserver/gdbserver',
	              'android_ndk_sysroot%': '<(android_ndk_root)/platforms/android-19/arch-arm',
	      ...
	      },
	      {
            ...
            ['target_arch == "mipsel"', {
              ...
              'conditions': [
              ['enable_ndktoolchain_48 == 1', {
	              'android_ndk_sysroot%': '<(android_ndk_root)/platforms/android-19/arch-mips',
        	      'android_toolchain%': '<(android_ndk_root)/toolchains/mipsel-linux-android-4.8/prebuilt/<(host_os)-x86/bin',
		},
		{
...
#Mohan Notification need to be enabled for sbrowser apk, this get enable under enable_sbrowser_apk=1	
#        'notifications%': 0,
	...
	['enable_sbrowser_apk==1', {
	 'notifications%': 1,
	 'enable_webrtc%': 0,
         'grit_defines': ['-D', 'enable_sbrowser_apk'],
 	 'conditions': [
	 ['enable_so_reduction==1', {
          'use_system_expat%': 1, 
          'use_system_icu%': 1, 
  ...
  'target_defaults': {
    ...
    'conditions': [
      ...
      ['OS=="android"', {
        'includes': ['sbr/sbr_feature_defines.gypi'],
      }],
      ['proprietary_codecs==1', {
  ...
  'conditions': [
    ...
    ['os_posix==1 and OS!="mac" and OS!="ios"', {
      'target_defaults': {
        ...
        'variables': {
          'werror%': '-Wno-unused-but-set-variable -Wno-unused-variable', # -Wno-unused-result', 
		   # sonic: instead of -Werror, we suppress warnings as errors.
