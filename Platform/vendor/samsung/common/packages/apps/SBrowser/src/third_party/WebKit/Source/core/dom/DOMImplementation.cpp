...
PassRefPtr<Document> DOMImplementation::createDocument(const String& type, Frame* frame, const KURL& url, bool inViewSourceMode)
{
    ...
    // To make WML Parsing at Java application level, we need to create HTMLViewSourceDocument for WML raw source.
    if (type == "text/vnd.wap.wml"){
        return HTMLViewSourceDocument::create(frame, url, type,true);
    }
    if (inViewSourceMode){
        return HTMLViewSourceDocument::create(frame, url, type,false);
    }
    #else
    if (inViewSourceMode){
