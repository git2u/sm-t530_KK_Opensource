...
void PasswordManager::Autofill(
  ...
  #if defined(SBROWSER_FPAUTH_IMPL)
  LOG(INFO)<<" PasswordManager::Autofill";

  if( (web_contents_->GetRenderViewHost()->OnCheckFPAuth() ) && ! web_contents_->GetRenderViewHost()->getFPAuthStatusFlag() && !delegate_->GetProfile()->IsOffTheRecord() && !wait_for_username){
         //SBROWSER_FP_SUPPORT_PART2
         LOG(INFO)<<" FP: PasswordManager::Autofill:: OnLaunchFingerPrintActivity then RETURN";
         web_contents_->GetRenderViewHost()->OnLaunchFingerPrintActivity();
         //SBROWSER_FP_SUPPORT_PART2
         return;
  }

  web_contents_->GetRenderViewHost()->setFPAuthStatusFlag(false);

  LOG(INFO)<<" PasswordManager::Filling the forms";
  #endif
