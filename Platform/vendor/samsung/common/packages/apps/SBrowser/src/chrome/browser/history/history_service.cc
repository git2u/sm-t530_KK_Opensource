...
#if SBROWSER_OMNIBOX_SMART_SUGGEST
  LOG(INFO)<<"History Add Page Data:";
  LOG(INFO)<<"did_replace_entry: "<<add_page_args.did_replace_entry;
  LOG(INFO)<<"add_page_args.page_id: "<<add_page_args.page_id;

  LOG(INFO)<<"add_page_args.referrer.spec(): "<<add_page_args.referrer.spec();
  LOG(INFO)<<"add_page_args.time.ToDoubleT(): "<<add_page_args.time.ToDoubleT();
  LOG(INFO)<<"add_page_args.transition: "<<add_page_args.transition;
  LOG(INFO)<<"add_page_args.url.spec(): "<<add_page_args.url.spec();
  LOG(INFO)<<"add_page_args.visit_source: "<<add_page_args.visit_source;
  for(unsigned int i = 0; i < add_page_args.redirects.size(); i++){
	  LOG(INFO)<<"add_page_args.redirects["<<i<<"]: "<<add_page_args.redirects.at(i);
  }
#endif
...
bool HistoryService::CanAddURL(const GURL& url) {
  ...
  int savePageURILength = std::string(chrome::kSavePageURI).length();
  if (url.spec().compare(0, savePageURILength, chrome::kSavePageURI) == 0) {
	VLOG(1)<<"Cannot add SAVEPAGE URI in history returning false";
    return false;
  }
