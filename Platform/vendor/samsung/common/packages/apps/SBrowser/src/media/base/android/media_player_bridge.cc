...
MediaPlayerBridge::MediaPlayerBridge(
    ...
    : media_error_cb_(media_error_cb),
      ...
      ,
      motion_listener_(base::MessageLoopProxy::current(),
                weak_this_.GetWeakPtr())
#endif
      {
  Initialize();
...
#if defined(SBROWSER_MEDIAPLAYER_MOTION_LISTENER)
  motion_listener_.CreateMediaPlayerMotionListener(j_context, j_media_player_.obj());
#endif
}
...
#if defined(SBROWSER_MEDIAPLAYER_MOTION_LISTENER)
  motion_listener_.UnRegisterReceiver();
#endif
}
...
#if defined(SBROWSER_MEDIAPLAYER_MOTION_LISTENER)
  motion_listener_.RegisterReceiver();
#endif

#if defined(SBROWSER_MEDIAPLAYER_AUDIOFOCUS_FIX)
  if(url_.spec().find("https://ssl.gstatic.com/accessibility/javascript/android/") == std::string::npos){
      if(!listener_.RequestAudioFocus()){
        listener_.ShowAudiofocusFailMessage();
        return;
      }
  }
#endif

#if defined(SBROWSER_MEDIAPLAYER_WAKELOCK_FIX)
  if(width_ > 0 && height_ > 0){
    jobject j_context = base::android::GetApplicationContext();
    DCHECK(j_context);
    listener_.SetWakeLock(j_context, j_media_player_.obj());
  }
#endif
