...
#if defined (SBROWSER_CHROME_NATIVE_PREFERENCES)
void BrowsingDataCookieHelper::FetchCookiesOnIOThreadwithUrl(std::string curl) {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::IO));
  scoped_refptr<net::CookieMonster> cookie_monster =
      request_context_getter_->GetURLRequestContext()->
      cookie_store()->GetCookieMonster();
...
#if defined (SBROWSER_CHROME_NATIVE_PREFERENCES)
void BrowsingDataCookieHelper::NotifyInUIThreadForUrl(
   const net::CookieList&  cookies) {
  DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  DCHECK(is_fetching_);
  is_fetching_ = false;
...
void BrowsingDataCookieHelper::OnGetCurrentCookieCount(const net::CookieList& cookies) {
     DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
	 m_observer->OnGetCurrentCookieCount(cookies.size());
}

void  BrowsingDataCookieHelper::GetCookiesForUrl(std::string curl) { 
	LOG(INFO) << " check BrowsingDataCookieHelper::GetCookiesForUrl entered";
...
void BrowsingDataCookieHelper::OnGetCookiesForUrl(const net::CookieList& cookies) {
     DCHECK(BrowserThread::CurrentlyOn(BrowserThread::UI));
  	...
  	std::string cookie_line = BuildCookieLine(cookies);
		m_observer->OnGetCookiesForUrl(cookie_line);
}
