...
void ChromeContentBrowserClient::OverrideWebkitPrefs(
  ...
  web_prefs->kikin_enabled = prefs->GetBoolean(prefs::kWebKitKikinEnabled);
  #endif
  ...
  web_prefs->loads_images_automatically =
	...
	// SAMSUNG_CHANGES : FontBoosting mode
#if defined(SBROWSER_FONT_BOOSTING_PREFERENCES)
    web_prefs->fontboosting_mode_enabled =
          prefs->GetBoolean(sbrprefs::kWebKitFontBoostingModeEnabled);
...
#if defined(OS_ANDROID)
#if defined(SBROWSER_FONT_BOOSTING_PREFERENCES)
   web_prefs->text_autosizing_enabled = web_prefs->fontboosting_mode_enabled;
#endif /* SBROWSER_FONT_BOOSTING_PREFERENCES */

#if defined(SBROWSER_IMIDEO_PREFERENCES) 
    web_prefs->imideo_debug_mode = prefs->GetInteger(prefs::kWebkitImideoDebugMode);
#endif
...
#endif
#if defined(SBROWSER_CSC_FEATURE)
	if (SecNativeFeature::getInstance()->getEnableStatus(CscFeature_Web_Bool_Disable_User_Gesture_required))
		web_prefs->user_gesture_required_for_media_playback = false;
#endif
  ...
  web_prefs->use_wide_viewport =
      prefs->GetBoolean(sbrprefs::kWebKitUseWideViewport);
#endif
#if defined (SBROWSER_SAVEPAGE_IMPL)
  web_prefs->allow_content_url_access = prefs->GetBoolean(sbrprefs::kWebKitAllowContentURLAccess);
#endif
  ...
  web_prefs->hyperlink_auditing_enabled =
    ...
    web_prefs->is_bing_as_default_search_engine = 
          prefs->GetBoolean(prefs::kWebKitIsBingAsDefaultSearchEngine);
#endif
  ...
  if (content::IsForceCompositingModeEnabled())
    web_prefs->force_compositing_mode = true;
	  ...
	  web_prefs->enable_appnap = prefs->GetBoolean(sbrprefs::kEnableAppNap);
#endif
#if defined(SBROWSER_POWER_SAVER_MODE_IMPL)
	  web_prefs->enable_bgcolor_Offwhite = prefs->GetBoolean(sbrprefs::kEnableBgColorOffWhite);   
	  web_prefs->enable_bgcolor_lightgrey = prefs->GetBoolean(sbrprefs::kEnableBgColorLightGrey);
	  web_prefs->enable_bgcolor_grey = prefs->GetBoolean(sbrprefs::kEnableBgColorGrey);
	  web_prefs->disable_gif_animation= prefs->GetBoolean(sbrprefs::kDisableGifAnimation);
	  web_prefs->disable_progressive_image_decoding = prefs->GetBoolean(sbrprefs::kDisableProgressiveImageDecode);
	  web_prefs->disable_filter_bitmap= prefs->GetBoolean(sbrprefs::kDisableFilterBitmap);
#endif
#if defined(SBROWSER_ENABLE_ADBLOCKER)
	  web_prefs->enable_adblocker= prefs->GetBoolean(sbrprefs::kEnableAdBlocker);
#endif
  ...
  if (url.spec() == chrome::kChromeUIFileManagerURL) {
	...
	web_prefs->overview_mode_enabled = prefs->GetBoolean(sbrprefs::kWebkitOverviewModeEnabled);
